...
- [第八部分：上下文管理](#第八部分上下文管理) ⭐ NEW
  - [8.1 上下文管理架构](#81-上下文管理架构)
  - [8.2 Token 监控与警告](#82-token-监控与警告)
  - [8.3 分层压缩策略](#83-分层压缩策略)
  - [8.4 compact_context 工具](#84-compact_context-工具)
  - [8.5 配置选项](#85-配置选项)
  - [8.6 使用示例](#86-使用示例)
  - [7.5 配置](#75-配置)
  - [7.6 使用示例](#76-使用示例)
...
⭐ **NEW** - 智能对话压缩系统，防止 token 上限导致对话中断

### 8.1 上下文管理架构

**核心问题**：长对话（128k-256k tokens）最终会触及上下文限制

**解决方案**：渐进式警告 + 分层压缩 + 降级策略

```
Token 使用 0-75%:   正常运行
Token 使用 75-85%:  💡 温和提示 + 加载 compact_context 工具
Token 使用 85-95%:  ⚠️ 强烈警告 + 建议立即压缩
Token 使用 95%+:    🚨 危险状态（记录日志，由 agent 决定）
```

**四个核心组件**：

1. **TokenTracker** - Token 监控器
   - 从 API 响应提取精确 token 使用量
   - 支持 20+ 模型（DeepSeek, Kimi, GLM, GPT, Claude）
   - 动态决定压缩策略（compact vs summarize）

2. **ContextCompressor** - 上下文压缩器
   - 三层消息分区（System/Old/Middle/Recent）
   - 两种压缩策略（Compact 详细 / Summarize 极简）
   - LLM 压缩失败时自动降级到简单截断

3. **ContextManager** - 统一管理器
   - 协调所有组件
   - 生成用户可见的压缩报告
   - 提供防御性错误处理

4. **compact_context** - Agent 工具
   - Agent 可调用的压缩接口
   - 动态加载（仅在需要时）
   - 返回 Command 更新 state
...
**监控机制**（`generalAgent/context/token_tracker.py`）：

```python
# Planner 节点每次 LLM 调用后提取 token
...
# status.level: "normal" | "info" | "warning" | "critical"
```
...
**三层分区策略**（`generalAgent/context/compressor.py`）：
...
# 分层
partitioned = {
    "system": [SystemMessage],      # 保留
    "old": messages[0:100],          # 压缩 (Compact or Summarize)
    "middle": messages[100:130],     # 压缩 (总是 Compact)
    "recent": messages[130:140],     # 完整保留
}

# 压缩后
compressed = [
    SystemMessage(...),
    SystemMessage(content="# 对话历史摘要（系统自动生成）\n..."),  # Old 压缩
    SystemMessage(content="# 近期对话摘要（系统自动生成）\n..."),  # Middle 压缩
    ...recent 10 messages (完整)
]
```
...
| 策略 | 用途 | 输出长度 | 内容 |
|------|------|---------|------|
| **Compact** | 详细摘要 | ~1000 chars | 用户请求、文件操作、工具调用、错误修复、TODO 状态 |
| **Summarize** | 极简摘要 | <200 chars | 主要任务、关键文件、工具名称、当前状态 |

**动态策略选择**：

```python
# 规则 1: 上次压缩率 > 40% → 切换到 summarize
if last_compression_ratio > 0.4:
    return "summarize"

# 规则 2: 连续 3 次 compact → 切换到 summarize
if compact_count % 3 == 0:
    return "summarize"
...
### 8.4 compact_context 工具

**工具定义**（`generalAgent/tools/builtin/compact_context.py`）：
...
async def compact_context(
    ...
    Returns:
        Command: 更新 state (messages, compact_count, etc.)
    """
    # 执行压缩
    ...
    return Command(update={
        ...
        "cumulative_prompt_tokens": 0  # 重置计数器
    })
```
...
try:
    ...
    # 降级：Kimi-style 简单截断
    truncator = MessageTruncator(settings)
    compressed = truncator.truncate(messages, max_messages=150)

    # 保留 System + 最近 150 条消息
```
...
# 示例:
#   - 激进: 5 (仅保留最近 5 条，最大化 token 节省)
#   - 保守: 20 (保留最近 20 条，保留更多细节)
#   - 平衡: 10 (默认值，适合大多数场景)
...
# 示例:
#   - 激进: 20 (中间 20 条详细摘要，其余极简)
#   - 保守: 50 (中间 50 条详细摘要，保留更多技术上下文)
#   - 平衡: 30 (默认值)
...
# 压缩率阈值（决定策略切换）
# 默认: 0.4 (40%)
# 约束: ge=0.2, le=0.8 (有效范围 0.2-0.8)
# 说明: 当上次压缩率 > 此值时，说明 compact 效果不佳，下次切换为 summarize
# 影响:
#   - 调低: 更容易切换到 summarize（激进，更快节省 tokens）
#   - 调高: 更倾向使用 compact（保守，保留更多细节）
# 示例:
#   - 激进: 0.3 (压缩率 > 30% 就切换到 summarize)
#   - 保守: 0.5 (压缩率 > 50% 才切换到 summarize)
#   - 平衡: 0.4 (默认值)
# 注意: 压缩率 = 压缩后 tokens / 压缩前 tokens，越低说明压缩效果越好

CONTEXT_COMPACT_CYCLE_LIMIT=3
# 连续 compact 次数限制
# 默认: 3
# 约束: ge=1, le=10 (有效范围 1-10)
# 说明: 连续使用 compact 策略 N 次后，强制切换为 summarize（防止压缩效果递减）
# 影响:
#   - 调低: 更快切换到 summarize（激进，防止压缩效果递减）
#   - 调高: 延长 compact 周期（保守，尽量保留详细摘要）
# 示例:
#   - 激进: 2 (compact 两次后立即切换)
#   - 保守: 5 (compact 五次后才切换)
#   - 平衡: 3 (默认值)
# 注意: 结合 compression_ratio_threshold 一起工作
