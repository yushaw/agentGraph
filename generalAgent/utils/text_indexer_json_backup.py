...
def _index_exists_json(file_path: Path) -> bool:
    """检查文件是否已索引（且索引有效）"""
...
def _create_index_json(file_path: Path) -> Path:
    """创建文档搜索索引
...
def _load_index_json(file_path: Path) -> Dict:
    """加载文档索引
...
def _cleanup_old_indexes_json(days: int = 30, remove_orphans: bool = True):
    """清理旧索引文件
...
def _get_index_stats_json() -> Dict:
    """获取索引统计信息"""
    if not INDEXES_DIR.exists():
        return {
            "total_indexes": 0,
...
def _search_in_index_json(file_path: Path, query: str, max_results: int = 5) -> List[Dict]:
    """搜索索引（自动路由到配置的后端）"""
    settings = get_settings()
